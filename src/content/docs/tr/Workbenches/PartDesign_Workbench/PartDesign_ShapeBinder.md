---
title: Parça Tasarım Tezgahı - Şekil bağlayıcı oluştur
---
|  |
| --- |
| Şekil bağlayıcı oluştur |
| Menü konumu |
| Parça tasarım → Şekil bağlayıcı oluştur |
| Tezgahlar |
...
| 0.17 |
| Ayrıca bkz |
| [klonla](/PartDesign_Clone/tr "PartDesign Clone/tr") |
|  |
...
A ShapeBinder will track the relative placement of the referenced geometry, which is useful in the context of creating [assemblies](/Assembly "Assembly"), if its Veri**Trace Support** property is set to `true`. See the [Example](#Example) below to understand how this works.
...
2. ![](/images/PartDesign_ShapeBinder.png) **Şekil bağlayıcı oluştur**'a basın.
3. Nesne veya Geometri ekle tuşlarına basın.
...
* A PartDesign SubShapeBinder always tracks the relative placement of the referenced geometry. For a PartDesign ShapeBinder this behavior is optional through its Veri**Trace Support** property.
...
* Veri**Label**:Nesneye verilen isimdir. Bu isim daha sonra değiştirilebilir.
* Veri**Trace Support**: Bu seçeneği true olarak ayarlayarak, Şekil bağlayıcı parçaların ve cisimlerin göreceli yerleşimlerini gözlemler.[introduced in 0.18](/Release_notes_0.18 "Release notes 0.18")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=PartDesign_ShapeBinder/tr&oldid=1458298>"