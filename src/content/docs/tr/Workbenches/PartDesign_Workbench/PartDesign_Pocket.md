...
* Veri**Type** (`Enumeration`): Defines how the pocket will be extruded, see [Options](#Options).
* Veri**Length** (`Length`): Defines the length of the pocket, see [Options](#Options).
* Veri**Length2** (`Length`): Second pocket length in case the Veri**Type** is **TwoLengths**, see [Options](#Options).
* Veri**Use Custom Vector** (`Bool`): If checked, the pocket direction will not be the normal vector of the sketch but the given vector, see [Options](#Options).
* Veri**Direction** (`Vector`): Vector of the pocket direction if Veri**Use Custom Vector** is used.
* Veri**Reference Axis** (`LinkSub`)
* Veri**Along Sketch Normal** (`Bool`): If *true*, the pocket length is measured along the sketch normal. Otherwise and if Veri**Use Custom Vector** is used, it is measured along the custom direction.
* Veri**Up To Face** (`LinkSub`): A face the pocket will extrude up to, see [Options](#Options).
* Veri**Offset** (`Length`)
...
Retrieved from "<http://wiki.freecad.org/index.php?title=PartDesign_Pocket/tr&oldid=1485491>"