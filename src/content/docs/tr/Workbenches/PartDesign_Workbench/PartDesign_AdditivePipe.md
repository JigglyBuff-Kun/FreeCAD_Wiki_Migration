---
title: PartDesign AdditivePipe/tr
---
|  |
| --- |
...
* Veri (Hidden)**Base Feature** (`Link`):
* Veri (Hidden)**\_Body** (`LinkHidden`):
...
* Veri**Profile** (`LinkSub`): reference to sketch.
* Veri**Midplane** (`Bool`): extrude symmetrically to sketch face.
* Veri**Reversed** (`Bool`): reverses extrusion direction.
* Veri**Up To Face** (`LinkSub`): face where feature will end.
* Veri**Allow Multi Face** (`Bool`): allow multiple faces in profile.

Sweep

* Veri**Sections** (`LinkSubList`): lists the sections used.
* Veri**Spine** (`LinkSub`): spine (path) to sweep along.
* Veri**Spine Tangent** (`Bool`): true or false (default). True extends the spine to include tangent edges.
* Veri (Hidden)**Auxiliary Spine** (`LinkSub`): secondary spine (path) to orient the Sweep.
* Veri**Auxiliary Spine Tangent** (`Bool`): true or false (default). True extends the auxiliary spine to include tangent edges.
* Veri**Auxiliary Curvelinear** (`Bool`): true or false (default). True calculates the normal between equidistant points on both spines.
* Veri**Mode** (`Enumeration`): profile mode. Options are *Fixed*, *Frenet*, *Auxiliary* or *Binormal*. See [Options](#Options).
* Veri**Binormal** (`Vector`): binormal vector for corresponding orientation mode.
* Veri**Transition** (`Enumeration`): transition mode. Options are *Transformed*, *Right Corner* or *Round Corner*.
* Veri**Transformation** (`Enumeration`): *Constant* uses a single cross-section. *Multisection* uses two or more cross-sections. *Linear*, *S-shape* and *Interpolation* are currently not functional.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=PartDesign_AdditivePipe/tr&oldid=1554024>"