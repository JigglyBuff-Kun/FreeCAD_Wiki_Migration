...
| Arch → Structure |
| Tezgahlar |
| [Arch](/Arch_Workbench/tr "Arch Workbench/tr") |
| Varsayılan kısayol |
| S T |
| Versiyonda tanıtıldı |
| - |
| Ayrıca bkz |
| [Arch Wall/tr](/Arch_Wall/tr "Arch Wall/tr"), [Arch Rebar/tr](/index.php?title=Arch_Rebar/tr&action=edit&redlink=1 "Arch Rebar/tr (page does not exist)") |
|  |
...
* Veri**Tool**: an optional extrusion path, which can be any type of wire. If this property is empty, the extrusion will be straight, and happen in the direction given by the Normal property
* Veri**Normal**: specifies the direction in which the base face of this structure will be extruded. If this property is kept to (0,0,0), the direction will be automatically set to the normal direction of the base face.
* Veri**Face Maker**: specifies the type of face generation algorithm to use to build the profile. The options are:
  + `None`
...
* Veri**Length**: specifies the length of the structure. This is only used if the structure is not based on a profile.
* Veri**Width**: specifies the width of the structure. This is only used if the structure is not based on a profile.
* Veri**Height**: specifies the height of the structure, or the extrusion length when based on a profile. If no height is given, and the structure is inside an [Arch Floor](/Arch_Floor "Arch Floor") object with its height defined, the structure will automatically take the value of the floor height.
* Veri**Nodes Offset**: specifies an optional offset between the centerline and the nodes line.

### View

* Görünüm**Nodes Type**: The type of structural nodes of this object, linear or area.
* Görünüm**Show Nodes**: Shows or hides the structural nodes.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Structure/tr&oldid=1539774>"