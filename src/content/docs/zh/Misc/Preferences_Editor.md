...
FreeCAD 的功能被分为了不同的模块，每个模块负责某个具体[工作台](/Workbenches/zh "Workbenches/zh")的相关工作。FreeCAD 还使用了一种称之为后加载的观念，意即相关组件只有在需要时才被加载进来。或许你已经注意到，当你在 FreeCAD 工具栏中选择一个工作台时，此工作台和它的所有组件都同时被加载进来。这其中包含了它的首选项设置。
...
[A2plus首选项](/A2plus_Workbench#Preferences "A2plus Workbench")
...
如果设置，缩放操作将在鼠标指针的中心位置。
如果未设置，缩放操作将在当前视图的中心位置。
...
| **Two-side rendering** | If checked, the default value for the View**Lighting** property is `Two side` instead of `One side`. `Two side` means the color of the interior side of faces is the same as the color of the exterior side. `One side` means their color is either the [backlight color](#3D_View), if enabled, or black. |
...
[建筑工作台首选项](/Arch_Preferences "Arch Preferences")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Preferences_Editor/zh&oldid=1542511>"