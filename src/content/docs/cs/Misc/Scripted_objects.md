...
Je třeba pochopit jednu zvláštnost, tyto objekty jsou ukládány ve FcStd souborech FreeCADu s pythonovským modulem [json](http://docs.python.org/2/library/json.html). Tento modul převede pythonovský objekt do řetězce, který je pak možno uložit v souboru. Při načítání naopak tento modul použije uložený řetězec ke znovuvytvoření původního objektu, při tom musí mít přístup ke zdrojovému kódu, který vytvoří objekt. To znamená, že když uložíte takový uživatelský objekt a pak jej otevíráte na počítači kde není pythonovský kód, tak nebude tento objekt vytvořen. Když tedy distribuujete takový objekt někomu jinému, musíte společně s ním distribuovat i pythonovský skript, který objekt vytváří.
...
Tento příklad používá [Modul Díl](/Part_Workbench/cs "Part Workbench/cs") k vytvoření osmistěnu a potom vytvoří pomocí Pivy jeho reprezentaci v Coinu.
...
See [FeaturePython methods](/FeaturePython_methods "FeaturePython methods") for the complete reference.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Scripted_objects/cs&oldid=1499122>"