---
title: Editor předvoleb
---
## Introduction

Systém předvoleb ve FreeCADu je umístěn v menu Úpravy → Předvolby.

Funkcionalita FreeCADu je rozdělena do několika modulů, kde je každý modul zodpovědný za práci specifické [pracovní plochy](/Workbenches "Workbenches"). FreeCAD také využívá koncept tzv. pozdní volání, což znamená, že potřebné komponenty jsou natahovány až když jsou potřeba. Můžete si všimnout, že když spouštíte nějakou pracovní plochu z pruhu nástrojů FreeCADu, jsou všechny její komponenty natahovány až v tomto okamžiku. Zahrnuje to i nastavení předvoleb.
...
Bez natažení nějakého modulu máte přístup ke dvěma konfiguračním sekcím, zodpovědným za základní nastavení aplikace a nastavení zobrazení na dispeji.
...
Pokud spustíte FreeCAD bez natažení některé z pracovních ploch, máte jen minimální okno předvoleb. Při natažení přídavného modulu se objeví nová sekce v okně předvoleb, které umožňuje konfigurovat detaily příslušných pracovních ploch.
...
| **Two-side rendering** | If checked, the default value for the Pohled**Lighting** property is `Two side` instead of `One side`. `Two side` means the color of the interior side of faces is the same as the color of the exterior side. `One side` means their color is either the [backlight color](#3D_View), if enabled, or black. |
| **Show bounding-box for highlighted or selected meshes** | If checked, the default value for the Pohled**Selection Style** property is `BoundBox` instead of `Shape`. `BoundBox` means a highlighted bounding box is displayed if meshes are highlighted or selected. `Shape` means the shape itself is then highlighted. |
...
Návrhový modul má obrazovku [Předvolby](/Draft_Preferences/cs "Draft Preferences/cs")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Preferences_Editor/cs&oldid=1542513>"