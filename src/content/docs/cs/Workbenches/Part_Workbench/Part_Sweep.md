...
| - |
| Viz také |
| [Part Loft](/Part_Loft/cs "Part Loft/cs") |
|  |
...
* Údaje**Spine** (`LinkSub`): spine (path) to sweep along.
* Údaje**Solid** (`Bool`): true or false (default). True creates a Solid.
* Údaje**Frenet** (`Bool`): true or false (default). True uses Frenet algorithm.
* Údaje**Transition** (`Enumeration`): transition mode. Options are *Transformed*, *Right corner* or *Round corner*.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Part_Sweep/cs&oldid=1494781>"