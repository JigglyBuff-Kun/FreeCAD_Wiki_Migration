...
FreeCAD has several different [navigation modes](/Mouse_Model "Mouse Model") available, that change the way you use your mouse to interact with the objects in the 3D view and the view itself. One of them is specifically made for [touchpads](/Mouse_Model#Touchpad_Navigation "Mouse Model"), where the middle mouse button is not used. The following table describes the default mode, called **CAD Navigation** (You can quickly change the current navigation mode by right-clicking on an empty area of the 3D view):
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Getting_started/zh-tw&oldid=1480479>"