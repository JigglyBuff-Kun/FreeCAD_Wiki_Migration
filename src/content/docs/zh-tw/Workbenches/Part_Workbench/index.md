...
FreeCAD 的電腦輔助設計功能建構於 [OpenCasCade](http://en.wikipedia.org/wiki/Open_CASCADE) 核心。物件坊讓 FreeCAD 取用 OpenCasCade 物件與功能。OpenCasCade 是一專業級電腦輔助設計核心，善於空間幾何控制與物件。不同於 [網狀模組](/Mesh_Workbench/zh-tw "Mesh Workbench/zh-tw") 物件，物體物件更加複雜，因此容許更多進階操作，例如一致布林操作、修改歷史和參數行為。
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Part_Workbench/zh-tw&oldid=1500783>"
