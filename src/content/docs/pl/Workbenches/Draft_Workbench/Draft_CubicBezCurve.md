---
title: Rysunek Roboczy Sześcienna krzywa Béziera
---
|  |
| --- |
| Rysunek Roboczy: Sześcienna krzywa Béziera |
| Lokalizacja w menu |
| Kreślenie → Narzędzia krzywych Béziera → Sześcienna krzywa Béziera Kreślenie 2D → Sześcienna krzywa Béziera |
| Środowisko pracy |
...
| 0.19 |
| Zobacz także |
| [Krzywa Béziera](/Draft_BezCurve/pl "Draft BezCurve/pl"), [Krzywa złożona](/Draft_BSpline/pl "Draft BSpline/pl") |
|  |

## Opis

Polecenie ![](/images/Draft_CubicBezCurve.svg) **Sześcienna krzywa Béziera** tworzy [krzywą Béziera](http://en.wikipedia.org/wiki/Bezier_curve) trzeciego stopnia *(wymagane są cztery punkty)*.

Krzywa Béziera jest jedną z najczęściej używanych krzywych w grafice komputerowej. Polecenie to pozwala utworzyć ciągły splajn składający się z kilku segmentów Béziera trzeciego stopnia, w sposób podobny do narzędzia Béziera w [Inkscape](https://inkscape.org/). Ogólną krzywą Béziera dowolnego stopnia można utworzyć za pomocą polecenia [Krzywa Beziera](/Draft_BezCurve/pl "Draft BezCurve/pl").

Polecenia [Krzywa Béziera](/Draft_BezCurve/pl "Draft BezCurve/pl") i Sześcienna krzywa Béziera używają *punktów kontrolnych* do zdefiniowania położenia i krzywizny odcinka krzywej. Z kolei polecenie [Krzywa złożona](/Draft_BSpline/pl "Draft BSpline/pl") określa *dokładne punkty*, przez które będzie przechodzić krzywa.
...
1. Polecenie można wywołać na kilka sposobów:
   * Naciśnij przycisk ![](/images/Draft_CubicBezCurve.svg) **Sześcienna krzywa Béziera**.
   * [Środowisko pracy Rysunek Roboczy](/Draft_Workbench/pl "Draft Workbench/pl"): Wybierz z menu **Kreślenie → Narzędzia krzywych Béziera → ![](/images/Draft_CubicBezCurve.svg) Sześcienna krzywa Béziera**.
   * [Środowisko pracy BIM](/BIM_Workbench/pl "BIM Workbench/pl"): Wybierz opcję **Kreślenie 2D → ![](/images/Draft_CubicBezCurve.svg) Sześcienna krzywa Béziera** option from the menu.
2. Otworzy się panel zadań **Sześcienna krzywa Béziera**. Więcej informacji można znaleźć w sekcji [Opcje](#Opcje).
3. Nie jest możliwe wprowadzanie punktów za pomocą panelu zadań.
4. W przypadku następujących modeli [Profil nawigacji myszką](/Mouse_navigation/pl "Mouse navigation/pl") należy przytrzymać klawisz klawiatury:
   * W przypadku korzystania z profilu [OpenInventor](/Mouse_navigation/pl#OpenInventor "Mouse navigation/pl") klawisz Ctrl musi być wciśnięty przez cały czas trwania polecenia.
   * Jeśli korzystasz z profilu [Gesture](/Mouse_navigation/pl#Gesture "Mouse navigation/pl"), klawisz Alt musi być wciśnięty dla każdej sekwencji kliknięcia i przytrzymania, ale możliwe jest również trzymanie tego klawisza wciśniętego przez cały czas trwania polecenia.
5. Wybierz pierwszy punkt w oknie [widoku 3D](/3D_view/pl "3D view/pl") i przytrzymaj przycisk myszki *(1)*, jest to pierwszy punkt końcowy.
6. Przeciągnij kursor do innego punktu w oknie [widoku 3D](/3D_view/pl "3D view/pl") i zwolnij przycisk myszki *(2)*, jest to pierwszy punkt kontrolny.
7. Przesuń kursor do innego punktu w oknie [widoku 3D](/3D_view/pl "3D view/pl"), wybierz ten punkt i przytrzymaj przycisk myszki *(3)*, jest to drugi punkt końcowy.
8. Przesuń kursor do innego punktu w oknie [widoku 3D](/3D_view/pl "3D view/pl"), aby dostosować końcową krzywiznę segmentu i zwolnij przycisk myszki *(4)*, jest to drugi punkt kontrolny.
...
Zapoznaj się z informacjami na stronie [Krzywa Beziera](/Draft_BezCurve/pl#Options "Draft BezCurve/pl").

## Uwagi

* Sześcienną krzywą Béziera środowiska Rysunek Roboczy można edytować za pomocą polecenia [Edycja](/Draft_Edit/pl "Draft Edit/pl").

## Właściwości

Zapoznaj się z informacjami na stronie [Krzywa Béziera](/Draft_BezCurve/pl#Właściwości "Draft BezCurve/pl").

## Tworzenie skryptów

Zobacz również stronę: [Dokumentacja API generowana automatycznie](https://freecad.github.io/SourceDoc/) oraz [Podstawy pisania skryptów dla FreeCAD](/FreeCAD_Scripting_Basics/pl "FreeCAD Scripting Basics/pl").

Aby uzyskać ogólne informacje, zobacz stronę [Krzywa Béziera](/Draft_BezCurve/pl "Draft BezCurve/pl"). Poprzez przekazanie opcji tworzony jest sześcienna krzywa Béziera `degree=3` to `makeBezCurve()`.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Draft_CubicBezCurve/pl&oldid=1473852>"