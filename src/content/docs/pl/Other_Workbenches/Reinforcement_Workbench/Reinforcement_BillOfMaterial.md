---
title: Architektura Zestawienie zbrojenia
---
|  |
| --- |
| Zbrojenie: Zestawienie zbrojenia |
| Lokalizacja w menu |
| *brak* |
| Środowisko pracy |
...
1. Wybierz obiekty ![](/images/Arch_Rebar.svg) [Prętów](/Arch_Rebar/pl "Arch Rebar/pl") i Prętów2, które chcesz uwzględnić w zestawieniu zbrojenia. Lub wybierz obiekty ![](/images/Arch_Structure.svg) [konstrukcji](/Arch_Structure/pl "Arch Structure/pl") do uwzględnienia obiektów ![](/images/Arch_Rebar.svg) [Prętów](/Arch_Rebar/pl "Arch Rebar/pl") w zestawieniu zbrojenia. Jeśli nic nie zostanie zaznaczone, wygenerowane zostanie zestawienie zbrojenia dla wszystkich obiektów ![](/images/Arch_Rebar.svg) [Prętów](/Arch_Rebar/pl "Arch Rebar/pl") i Prętów2 obecnych w modelu.

2. Następnie wybierz ![](/images/Reinforcement_BillOfMaterial.svg)
...
5. Aby edytować konfiguracje SVG ![](/images/Reinforcement_BillOfMaterial.svg) **Zestawienia zbrojenia**, kliknij przycisk Edytuj konfiguracje SVG. Pojawi się okno dialogowe, jak pokazano poniżej.

:   ![](/images/BOMDialog_SVG.png)
:   Okno dialogowe edycji konfiguracji SVG zestawienia zbrojenia.

6. Modyfikuj konfiguracje SVG ![](/images/Reinforcement_BillOfMaterial.svg) **Zestawienia zbrojenia**, a następnie kliknij OK, aby zastosować zmiany.
...
* DANE**Jednostki kolumn**: Słownik z kluczami: "Diameter", "RebarsLength", "RebarsTotalLength" i odpowiadającymi im jednostkami jako wartością.
* DANE**Mapa wagi średnicy**: Słownik ze średnicą jako kluczem i odpowiadającą jej wagą jako wartością.
* DANE**Typ długości pręta zbrojeniowego**: Typ długości pręta zbrojeniowego określa typ długości pręta zbrojeniowego używany do obliczeń BOM, tj. "RealLength" lub "LengthWithSharpEdges".
* DANE**Obiekty prętów zbrojeniowych**: Lista obiektów ArchRebar i / lub rebar2 i / lub struktur *(aby wybrać ArchRebar w tej konstrukcji)*.

**SVG:**

* DANE**Rodzina czcionki**: Rodzina czcionki tekstu w BOM SVG.
* DANE**Nazwa czcionki**: Nazwa pliku czcionki odpowiadająca rodzinie czcionki wymaganej w trybie konsoli.
* DANE**Rozmiar czcionki**: Rozmiar czcionki w mm.
* DANE**Szerokość kolumny**: Szerokość każdej kolumny w tabeli BOM SVG.
* DANE**Wysokość wiersza**: Wysokość każdego wiersza w tabeli BOM SVG.
* DANE**Lewy odsunięcie**: Lewe przesunięcie tabeli BOM SVG.
* DANE**Górne odsunięcie**: Górne przesunięcie tabeli BOM SVG.
* DANE**Minimalne przesunięcie w prawo**: Minimalne przesunięcie w prawo tabeli BOM SVG.
* DANE**minimalne odsunięcie dolne**: Minimalne dolne przesunięcie tabeli BOM SVG.
* DANE**Szerokość maksymalna**: Maksymalna szerokość tabeli BOM SVG.
* DANE**Wysokość maksymalna**: Maksymalna wysokość tabeli BOM SVG.
* DANE**Plik szablonu**: Plik szablonu SVG dla BOM SVG.
* DANE**Plik wyjściowy**: Plik wyjściowy BOM SVG.
...
Narzędzie **Zestawienie zbrojenia** może być używane w [makrodefinicjach](/Macros/pl "Macros/pl") i z konsoli [Python](/Python/pl "Python/pl") za pomocą następującej funkcji:
...
* `column_headers` jest słownikiem z kluczami: "Host", "Mark", "RebarsCount", "Diameter", "RebarsLength", "RebarsTotalLength", a wartościami są krotki column\_header i ich numery porządkowe.
...
* Tworzy i zwraca obiekt RebarBillOfMaterial\_SVG `BOMContent` dla danego `rebar_objects`.
...
* `font_filename` określa nazwę pliku czcionki lub pełną ścieżkę do pliku czcionki odpowiadającego font\_family. Jest to wymagane, jeśli pracujesz w trybie konsoli, bez GUI.
...
* `template_file` określa plik szablonu używany do umieszczenia na nim wygenerowanej tabeli zestawienia materiałów. Musi to być prawidłowy plik szablonu Rysunku Technicznego z polami przedstawionymi na stronie [Przestrzeń nazw SVG](/Svg_Namespace/pl "Svg Namespace/pl").
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Reinforcement_BillOfMaterial/pl&oldid=1441512>"