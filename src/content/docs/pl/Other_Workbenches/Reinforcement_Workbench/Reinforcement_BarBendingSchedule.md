---
title: Zbrojenie Schemat gięcia prętów
---
|  |
| --- |
...
| brak |
| Workbenches |
| [Architektura](/Reinforcement_Workbench/pl "Reinforcement Workbench/pl"), [BIM](/BIM_Workbench/pl "BIM Workbench/pl") |
| Default shortcut |
| *None* |
| Introduced in version |
| 0.19 |
| See also |
| [Zbrojenie](/Reinforcement_Workbench/pl "Reinforcement Workbench/pl"), [Zestawienie zbrojenia](/Arch_Rebar_BOM/pl "Arch Rebar BOM/pl"), [Wymiarowanie rysunku zbrojenia](/Arch_Rebar_Drawing_Dimensioning/pl "Arch Rebar Drawing Dimensioning/pl") |
|  |
...
1. Wybierz obiekty ![](/images/Arch_Rebar.svg) [Prętów](/Arch_Rebar/pl "Arch Rebar/pl") i Prętów2, które chcesz uwzględnić w harmonogramie gięcia prętów. Lub wybierz obiekty ![](/images/Arch_Structure.svg) [konstrukcji](/Arch_Structure/pl "Arch Structure/pl") do uwzględnienia obiektów ![](/images/Arch_Rebar.svg) [Pręta](/Arch_Rebar/pl "Arch Rebar/pl") i Pręta2 w harmonogramie gięcia prętów. Jeśli nic nie zostanie zaznaczone, wygenerowany zostanie harmonogram gięcia prętów dla wszystkich ![](/images/Arch_Rebar.svg) obiektów [Prętów](/Arch_Rebar/pl "Arch Rebar/pl") i Prętów2 obecnych w modelu.

2. Następnie wybierz ![](/images/Reinforcement_BarBendingSchedule.svg) **Schemat gięcia prętów** z narzędzi prętów zbrojeniowych.
...
* DANE**Grupowanie zbrojenia według**: określa sposób grupowania obiektów zbrojenia w zestawieniu gięcia prętów, tj. "Obiekt" lub "Oznaczenie".
* DANE**Typ długości pręta**: określa typ długości pręta zbrojeniowego używany do obliczeń BOM, tj. "Długość rzeczywista" lub "Długość z ostrymi krawędziami".
* DANE**Nagłówki kolumn**: Słownik z column\_data jako kluczem i krotką *(column\_display\_header, column\_sequence)* jako wartością.
* DANE**Jednostki kolumn**: Słownik z kluczami: "Średnica", "Długość pręta zbrojeniowego", "Długość całkowita pręta zbrojeniowego" i odpowiadającymi im jednostkami jako wartością.
* DANE**Rodzaj czcionki**: Rodzina czcionki tekstu w zestawieniu gięcia prętów SVG.
* DANE**Rozmiar czcionki**: Rozmiar czcionki w mm.
* DANE**Szerokość kolumny**: Szerokość każdej kolumny w harmonogramie gięcia prętów SVG.
* DANE**Wysokość wiersza**: Wysokość każdego wiersza w harmonogramie gięcia prętów SVG.
* DANE**Plik wyjściowy SVG**: Plik wyjściowy SVG, do zapisu wygenerowanej listy cięć kształtów prętów zbrojeniowych.

**Dane kolumny Kształt pręta zbrojeniowego:** Dane związane z kolumną Kształt pręta zbrojeniowego w Zestawieniu gięcia prętów.

* DANE**Nagłówek kolumny**: definiuje nagłówek kolumny dla kolumny kształtu pręta zbrojeniowego.
* DANE**Przesunięcie przedłużonej krawędzi strzemienia**: określa przesunięcie przedłużonych krawędzi końcowych strzemion, tak aby krawędzie końcowe strzemion o kącie zagięcia 90 stopni nie pokrywały się z krawędziami strzemion.
* DANE**Szerokość skoku prętów zbrojeniowych**: definiuje szerokość obrysu prętów zbrojeniowych w kolumnie kształtu prętów zbrojeniowych..
* DANE**Styl koloru prętów zbrojeniowych**: definiuje styl koloru prętów zbrojeniowych.
...
* DANE**Uwzględnij wymiary**: Jeśli opcja ta ma wartość `Prawda`, wówczas każdy wymiar krawędzi pręta zbrojeniowego i wymiary kąta gięcia zostaną uwzględnione na liście cięć kształtu pręta zbrojeniowego.
...
* DANE**Jednostki wymiaru krawędzi prętów zbrojeniowych**: Jednostki używane dla wymiarów długości krawędzi prętów zbrojeniowych.
* DANE**Dokładność wymiarowania krawędzi prętów zbrojeniowych**: Liczba miejsc dziesiętnych, które powinny być wyświetlane dla długości krawędzi pręta zbrojeniowego jako etykieta wymiaru.
* DANE**Rodzina czcionki wymiaru**: Rodzina czcionki tekstu wymiaru.
* DANE**Rozmiar czcionki wymiaru**: Rozmiar czcionki tekstu wymiaru.
* DANE**Lista wykluczeń wymiaru kąta zagięcia**: Lista kątów giętych, których wymiary nie będą uwzględniane.
* DANE**Format etykiety wymiaru spiralnego pręta zbrojeniowego**: Format etykiety wymiaru spiralnego pręta zbrojeniowego. np. "%L,r=%R,pitch=%P", gdzie %L → Długość spiralnego pręta zbrojeniowego, %R → Promień spirali spiralnego pręta zbrojeniowego, %P → Skok spirali spiralnego pręta zbrojeniowego.
...
Narzędzie **Schemat gięcia prętów** może być używane w [makrodefinicjach](/Macros/pl "Macros/pl") i z konsoli [Python](/Python/pl "Python/pl") za pomocą następującej funkcji:
...
* `rebar_shape_view_directions` to lista kierunków widoku dla każdego kształtu pręta zbrojeniowego. Może być typu `FreeCAD.Vector` lub `WorkingPlane.Plane`. LUB ich lista. Zachowaj `FreeCAD.Vector(0, 0, 0)`, aby automatycznie wybrać view\_directions.
...
* `rebar_shape_color_style` określa styl koloru prętów zbrojeniowych. Może to być "kolor kształtu" lub "color\_name lub hex\_value\_of\_color". "kolor kształtu" oznacza wybór koloru kształtu pręta zbrojeniowego.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Reinforcement_BarBendingSchedule/pl&oldid=1374747>"