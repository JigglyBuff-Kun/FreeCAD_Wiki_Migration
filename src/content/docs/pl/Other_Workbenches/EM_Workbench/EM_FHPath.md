---
title: Elektromagnetyka Ścieżka FastHenry
---
...
- The default orientation of the FHPath segment cross-sections is the FastHenry default one: the vector along the width is parallel to the XY plane; if the width is along the Z direction, the width vector is aligned to the X axis. You can change the orientation of the cross-section of the first segment of the FHPath specifying the DANE**ww** vector property. This is done in the base Placement coordinate system, i.e. changes in the Placement preserve the relative cross-connection orientation without changing DANE**ww**. Subsequent segments are automatically oriented applying the rotations corresponding to the angle between each pair of segments in turn. The first segment is identified by the first node of the FHPath object as shown in the tree (the topmost is the first node, regardless of its name / numbering), or equivalently as the first node in the DANE**Nodes** FHPath property list.
...
- Curved edges will be discretized into a set of segments according to the DANE**Discr** FHPath property.

## Properties

- DANE**Base**: The base object this component is built upon
- DANE**Nodes**: (read-only) The list of [FHNode](/EM_FHNode "EM FHNode") along the path. Not for direct user modification.
- DANE**Width**: the FHPath segments width ('w' segment parameter in FastHenry)
- DANE**Height**: the FHPath segments height ('h' segment parameter in FastHenry)
- DANE**Discr**: the max number of segments into which curved edges will be discretized
- DANE**Sigma**: the FHPath segments conductivity ('sigma' segment parameter in FastHenry)
- DANE**ww**: the cross-section direction along the width for the first segment of the FHPath ('wx', 'wy', 'wz' segment parameter in FastHenry)
...
Retrieved from "<http://wiki.freecad.org/index.php?title=EM_FHPath/pl&oldid=1077911>"
