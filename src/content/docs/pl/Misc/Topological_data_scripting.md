---
title: Skrypty danych topologicznych
---
## Wprowadzenie

Tutaj wyjaśnimy Ci jak kontrolować środowisko [Część](/Part_Workbench/pl "Part Workbench/pl") bezpośrednio z interpretera FreeCAD Python, lub z dowolnego zewnętrznego skryptu. Przejrzyj sekcję  [o skryptach](/Power_users_hub/pl "Power users hub/pl") oraz strony [Podstawy tworzenia skryptów FreeCAD](/FreeCAD_Scripting_Basics/pl "FreeCAD Scripting Basics/pl"), jeśli potrzebujesz więcej informacji na temat działania skryptów Pythona w FreeCAD. Jeśli jesteś początkującym użytkownikiem środowiska Python, dobrze jest najpierw przeczytać [Wprowadzenie do środowiska Python](/Introduction_to_Python/pl "Introduction to Python/pl").
...
To jest [Unified Modeling Language *(UML)*](http://en.wikipedia.org/wiki/Unified_Modeling_Language) przegląd najważniejszych klas modułu Część:
![klasy Python, modułu Część](/images/Part_Classes.jpg)
...
Zobacz stronę [skrypty środowiska Część](/Part_API/pl "Part API/pl") lub [dokumentacja API środowiska Część generowana automatycznie](https://freecad-python-stubs.readthedocs.io/en/latest/autoapi/Part/) aby zobaczyć pełną listę dostępnych metod modułu Część.
...
Uwaga: `makePlane()` akceptuje tylko `App.Vector()` dla start\_pnt i dir\_normal, a nie krotki.
...
Jest kilka sposobów na zapisanie swojej pracy. Możesz oczywiście zapisać swój dokument FreeCAD, ale możesz również zapisać obiekty typu [Część](/Part_Workbench/pl "Part Workbench/pl") bezpośrednio do popularnych formatów CAD, takich jak BREP, IGS, STEP i STL.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Topological_data_scripting/pl&oldid=1312343>"