...
**IfcOpenShell** to biblioteka oprogramowania typu open source *(LGPL 3)*, która pomaga programistom pracować z formatem plików [industry foundation classes](http://www.buildingsmart-tech.org/specifications/ifc-overview) *([IFC](/Arch_IFC/pl "Arch IFC/pl"))*. Format pliku IFC może być używany do opisywania danych budowlanych. Format ten jest powszechnie używany do [modelowania informacji o budynku](https://en.wikipedia.org/wiki/Building_information_modeling) *(BIM)*, na przykład do analizy obciążeń mechanicznych oraz badań termicznych i efektywności energetycznej. IfcOpenShell jest przede wszystkim zbiorem bibliotek C++, jednak ponieważ posiada powiązania ze środowiskiem [Python](/Python/pl "Python/pl"), może być zintegrowany z programami takimi jak FreeCAD i Blender.

IfcOpenShell używa [OpenCASCADE](/OpenCASCADE/pl "OpenCASCADE/pl") wewnętrznie do konwersji niejawnej geometrii w plikach IFC na geometrię jawną, którą mogą zrozumieć inne pakiety CAD, na przykład STEP, [OBJ](/Arch_OBJ/pl "Arch OBJ/pl") i [DAE](/Arch_DAE/pl "Arch DAE/pl").
...
Retrieved from "<http://wiki.freecad.org/index.php?title=IfcOpenShell/pl&oldid=1433910>"