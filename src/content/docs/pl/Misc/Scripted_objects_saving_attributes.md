---
title: Obiekty tworzone skryptami, zapis atrybutów
---
## Wprowadzenie

[Obiekty tworzone skryptami](/Scripted_objects/pl "Scripted objects/pl") są odbudowywane przy każdym otwarciu dokumentu [FCStd](/File_Format_FCStd/pl "File Format FCStd/pl"). W tym celu dokument przechowuje referencję do modułu i klasy Python, które zostały użyte do stworzenia obiektu, wraz z jego właściwościami.
...
Obiekt może zostać utworzony przy użyciu tej klasy i zapisany do pliku my\_document.FCstd. Jeśli żaden konkretny [dostawca widoku](/Viewprovider/pl "Viewprovider/pl") nie jest przypisany do nowego obiektu, jego klasa proxy jest po prostu ustawiana na wartość inną niż `None`, w tym przypadku na `1`.
...
Zwykle [obiekty tworzone skryptami](/Scripted_objects/pl "Scripted objects/pl") powinny używać [właściwości](/Property/pl "Property/pl") do przechowywania informacji, ponieważ są one automatycznie przywracane po otwarciu dokumentu.
...
Na przykład, większość obiektów środowiska pracy [Rysunek Roboczy](/Draft_Workbench/pl "Draft Workbench/pl") ustawia atrybut `Type`, który może być użyty do określenia typu używanego obiektu.
...
Wszystkie obiekty mają właściwość `TypeId`, ale dla [obiektów tworzonych skryptami](/Scripted_objects "Scripted objects") ta właściwość nie jest użyteczna, ponieważ zawsze odnosi się do nadrzędnej klasy C++, na przykład [`Part::Part2DObjectPython`](/Part_Part2DObject/pl "Part Part2DObject/pl") lub [`Part::FeaturePython`](/Part_Feature/pl "Part Feature/pl"). Dlatego posiadanie tego dodatkowego atrybutu `Proxy.Type` w klasie jest przydatne do traktowania każdego obiektu w określony sposób.
...
* [Obiekty tworzone skryptami](/Scripted_objects "Scripted objects")
* [Migracja obiektów tworzonych skryptami](/Scripted_objects_migration "Scripted objects migration")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Scripted_objects_saving_attributes/pl&oldid=1491200>"