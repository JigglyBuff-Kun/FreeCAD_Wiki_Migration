...
Ten przykład wykorzystuje środowisko pracy [Część](/Part_Workbench/pl "Part Workbench/pl") do utworzenia ośmiościanu, a następnie tworzy jego reprezentację "coin" za pomocą Pivy.
...
Zobacz stronę [Metody FeaturePython](/FeaturePython_methods "FeaturePython methods"), aby zapoznać się z pełnym opisem.
...
Zobacz [Właściwości niestandardowe funkcji Python](/FeaturePython_Custom_Properties/pl "FeaturePython Custom Properties/pl"), aby uzyskać więcej informacji.
...
Atrybuty ustawiane przy pomocy `setEditorMode` mogą zostać usunięte przez użytkownika. Zobacz [Edytor właściwości](/Property_editor/pl#Menu_kontekstowe "Property editor/pl"). Zauważ, że właściwości tylko do odczytu mogą być nadal zmienione z poziomu Pythona.
...
* `group`: Subsekcja właściwości (używane w [Edytorze właściwości](/Property_editor/pl "Property editor/pl")).
* `doc`: Podpowiedź narzędzia (to samo).
* `attr`: Atrybut, zobacz wyżej.
* `read_only`: Zobacz wyżej.
* `hidden`: Zobacz wyżej.
* `enum_vals`: Wartości wyliczenia (lista ciągów znaków), ma znaczenie tylko jeśli typ to `"App::PropertyEnumeration"`.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Scripted_objects/pl&oldid=1499248>"