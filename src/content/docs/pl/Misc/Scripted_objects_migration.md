---
title: Obiekty tworzone skryptami, migracja
---

## Wprowadzenie

[Obiekty tworzone skryptami](/Scripted_objects/pl "Scripted objects/pl") są odbudowywane przy każdym otwarciu dokumentu [FCStd](/File_Format_FCStd/pl "File Format FCStd/pl"). W tym celu dokument przechowuje referencję do modułu i klasy Pythona, które zostały użyte do stworzenia obiektu, wraz z jego właściwościami.
...
Obiekt może zostać utworzony przy użyciu tej klasy i zapisany do pliku my_document.FCstd. Jeśli żaden konkretny [dostawca widoku](/Viewprovider/pl "Viewprovider/pl") nie jest przypisany do nowego obiektu, jego klasa proxy jest po prostu ustawiana na wartość inną niż `None`, w tym przypadku na `1`.
...
Sesja [konsoli Python](/Python_console/pl "Python console/pl") z pominiętymi podstawowymi właściwościami.
...
Weźmy teraz pod uwagę, że środowisko pracy jest zrestrukturyzowane tak, że klasy nie znajdują się tylko w katalogu głównym, ale zamiast tego znajdują się wewnątrz katalogu objects. Złożone środowiska pracy, które mają wiele różnych typów obiektów, powinny być zorganizowane w katalogach zawierających obiekty, [dostawcy widoku](/Viewprovider/pl "Viewprovider/pl"), [polecenia Gui](/Command/pl "Command/pl"), interfejsy [panela zadań](/Task_panel/pl "Task panel/pl") itd.
...
Jeśli otworzymy dokument i sprawdzimy właściwości obiektu w [konsoli Python](/Python_console/pl "Python console/pl"), zobaczymy, że starsze właściwości zostały zachowane, ale obiekt ma nową klasę Proxy.
...
- [24px](/index.php?title=Plik:Edit_OK.svg&action=edit&redlink=1 "Plik:Edit OK.svg (page does not exist)") Jest to najprostsza metoda, która wymaga jedynie przekierowania starej klasy do nowej.

* ![](/images/Edit_OK.svg) Stare właściwości są zachowywane, o ile nowa klasa ich nie nadpisuje.
* ![](/images/Edit_OK.svg) Jest to dobre rozwiązanie, jeśli stara i nowa klasa mają te same właściwości _(obsługują ten sam typ danych)_, ale różnią się tylko nazwą modułu lub klasy.

**Wady**

- ![](/images/Edit_Cancel.svg) Nowa klasa zachowuje stare właściwości obiektu, co nie zawsze jest pożądane.
- ![](/images/Edit_Cancel.svg) Nowe właściwości lub zmienione właściwości nie są obsługiwane, więc obiekt zostanie załadowany, ale może nie pokazywać prawidłowego zachowania nowej klasy.
- ![](/images/Edit_Cancel.svg) Stary moduł może być przechowywany w nieskończoność, aby zmigrować wszystkie stare obiekty utworzone w przeszłości.
...
W tym przypadku zakładamy, że zdefiniowaliśmy również nowego [dostawcę widoku](/Viewprovider/pl "Viewprovider/pl") w module viewp/new_view.py. Jeśli nie chcemy migrować tej klasy, możemy pominąć wszystko po sprawdzeniu `App.GuiUp`.
...
Sprawdzając obiekt z [konsoli Python](/Python_console "Python console") zobaczymy, że starsze właściwości zostały zachowane, a dodatkowo nowe właściwości zostały dodane wraz z nową klasą Proxy.
...
- ![](/images/Edit_OK.svg) Ta metoda pozwala nam sprawdzić, czy klasa, którą migrujemy, jest właściwą klasą, zamiast po prostu przekierowywać do nowszej klasy.
- ![](/images/Edit_OK.svg) Podobnie jak w metodzie 1, stare właściwości są zachowywane, o ile nowa klasa ich nie nadpisuje.
- ![](/images/Edit_OK.svg) W przeciwieństwie do metody 1, nowe właściwości są zawsze dodawane, jednak jeśli mają tę samą nazwę, ich nazwy zostaną zmienione.

* [24px](/index.php?title=Plik:Edit_OK.svg&action=edit&redlink=1 "Plik:Edit OK.svg (page does not exist)") Migracja nie jest natychmiastowa, nadal możemy manipulować informacjami lub drukować komunikaty podczas ładowania obiektu.

**Wady**

- [24px](/index.php?title=Plik:Edit_Cancel.svg&action=edit&redlink=1 "Plik:Edit Cancel.svg (page does not exist)") Jest bardziej rozwlekła niż metoda 1, ponieważ musimy zaimplementować metodę `onDocumentRestored`, aby zmigrować obiekt.

* ![](/images/Edit_Cancel.svg) Zawsze dodaje nowe właściwości, więc może tworzyć zduplikowane właściwości w przypadku, gdy nowe właściwości mają taką samą nazwę jak stare. To musi być obsługiwane ręcznie.
...
Podobnie jak w metodzie 2, jeśli chcemy, możemy również dodać fragment kodu, który sprawdza, czy klasa Proxy jest właściwa. W tym przykładzie ponownie zakładamy, że używamy niestandardowego [dostawcy widoku](/Viewprovider/pl "Viewprovider/pl"), z co najmniej jedną niestandardową właściwością.
...
Możemy usunąć tylko te właściwości, które zostały dodane przez klasy [Python](/Python/pl "Python/pl") podczas tworzenia [obiektów generowanych skryptami](/Scripted_objects/pl "Scripted objects/pl"). Inne atrybuty należą do bazowego obiektu C++ i nie mogą być usunięte.
...
Zakładając, że zmieniliśmy już stary moduł w ten sposób, jeśli otworzymy dokument ze starym obiektem, zobaczymy komunikaty wspominające o użyciu nowych klas. Sprawdzając obiekt z [konsoli Python](/Python_console/pl "Python console/pl") widzimy, że starsze właściwości zostały usunięte i istnieją tylko nowe.
...
- ![](/images/Edit_OK.svg) Podobnie jak metoda 2, ta metoda pozwala nam sprawdzić, czy klasa, którą migrujemy, jest właściwą klasą.
- ![](/images/Edit_OK.svg) Mamy pełną kontrolę nad tym, co zrobić ze starymi właściwościami. Zazwyczaj zostaną one usunięte, aby nie było kolizji nazw z nowo dodanymi właściwościami. W ten sposób unikamy zduplikowanych właściwości.
- ![](/images/Edit_OK.svg) Zapisując starsze wartości, możemy dowolnie manipulować informacjami w kroku przywracania i przypisać odpowiednie wartości do nowych właściwości.

**Wady**

- ![](/images/Edit_Cancel.svg) Ta metoda jest bardzo rozwlekła w porównaniu do poprzednich, ponieważ musimy zaimplementować metodę `onDocumentRestored` i obsłużyć każdą z właściwości indywidualnie (zapisać wartość, usunąć właściwość, ponownie przypisać wartość). Jest to problematyczne, jeśli obiekt, który chcemy migrować, ma wiele właściwości lub ich wartości muszą być przekształcane w bardzo szczególny sposób.
...
To samo można zrobić dla klasy [dostawcy widoku](/Viewprovider/pl "Viewprovider/pl").
...
Metoda 3 nie potrzebuje tego dodatku do nowej klasy, ponieważ starsze właściwości są wyraźnie usuwane, więc nie będzie żadnych konfliktów podczas instalowania nowych właściwości. Niemniej jednak, nadal dobrą praktyką jest, aby każda klasa dodawała swoje wymagane właściwości tylko wtedy, gdy jeszcze nie istnieją. Jest to pomocne zarówno w przypadku tworzenia nowych [obiektów generowanych skryptami](/Scripted_objects/pl "Scripted objects/pl"), jak i ich migracji.

### Zalety i wady

**Zalety**

- [24px](/index.php?title=Plik:Edit_OK.svg&action=edit&redlink=1 "Plik:Edit OK.svg (page does not exist)") Obiekt zachowa wszystkie poprzednie właściwości, ale dodatkowo zyska nowe właściwości bez powtórzeń.

**Wady**

- ![](/images/Edit_Cancel.svg) Podobnie jak metoda 2, nadal nie radzi sobie ze zmienionymi nazwami właściwości. Stare właściwości powinny zostać usunięte ręcznie.
...
Jeśli od początku utworzymy właściwość, która przechowuje numer wersji naszego obiektu, możemy użyć tego numeru w przyszłości, aby wykonać określoną migrację z tej wersji do dowolnej innej. Ustawiamy właściwość jako tylko do odczytu, więc nie możemy jej nadpisać w [edytorze właściwości](/Property_editor/pl "Property editor/pl"), chociaż jest ona nadal dostępna z [konsoli Python](/Python_console/pl "Python console/pl").
...
Nie zapisujemy wartości `Version`, ponieważ podczas migracji ustawimy nowy numer `Version`. Jak pokazano w przykładzie, możemy zaimplementować różne funkcje dla każdej odpowiedniej wersji obiektu, który zamierzamy zmigrować. Pomijamy migrację właściwości [dostawcy widoku](/Viewprovider/pl "Viewprovider/pl"), ale przebiega ona według tego samego schematu.

### Zalety i wady

**Zalety**

- ![](/images/Edit_OK.svg) Mamy pełną kontrolę nad tym, co zrobić ze starymi właściwościami i jak przeprowadzić migrację.
- ![](/images/Edit_OK.svg) Możemy zaimplementować konkretną metodę do migracji konkretnej wersji starego obiektu.

**Wady**

- ![](/images/Edit_Cancel.svg) Ta metoda jest bardzo gadatliwa, ponieważ musimy mieć jasny pomysł na to, jak obsługiwać każdą z właściwości każdej "wersji", którą chcemy zmigrować. Jeśli nasz obiekt ma wiele różnych wersji utworzonych na przestrzeni lat, być może będziemy musieli przygotować długą listę metod, aby zmigrować je do najnowszego obiektu.

## Uzupełnienie B2. Używanie wewnętrznych atrybutów klasy zamiast właściwości

Zamiast używać [właściwości](/Property/pl "Property/pl") obiektu do przechowywania informacji o wersji, możemy użyć atrybutu klasy. W ten sposób "ukrywamy" informacje o wersji, ponieważ właściwości są zwykle publiczne i widoczne w [edytorze właściwości](/Property_editor/pl "Property editor/pl"), podczas gdy atrybutami klasy można manipulować tylko z [konsoli Python](/Python_console/pl "Python console/pl"). Atrybuty klas mogą być zapisywane i przywracane, jak wyjaśniono w [Obiektchy generowanych skryptami](/Scripted_objects_saving_attributes/pl "Scripted objects saving attributes/pl").
...
Jak widzimy w przykładzie, stara właściwość `Area` jest usuwana i migrowana do nowej właściwości `GeneralArea` jak zwykle. Nie musimy usuwać `Length` ani `Version`, ponieważ w nowej klasie są one nadal używane z tą samą nazwą i nie zostaną ponownie utworzone ( uzupełnienie A). Ponieważ nie chcemy modyfikować `Length`, ta właściwość nie jest w ogóle dotykana; jest migrowana do nowej klasy po cichu. Aktualizujemy jednak `Version` do nowej wartości. Pomijamy migrację właściwości [dostawcy widoku](/Viewprovider/pl "Viewprovider/pl"), ale przebiega ona według tego samego schematu.

Powinno to działać jak metoda 3, co oznacza, że stare właściwości są usuwane i tylko nowe właściwości pozostają w nowym obiekcie. Jedyną różnicą jest to, że pomijamy usuwanie i ponowne tworzenie właściwości, które nazywają się tak samo. Proces ten powinien działać tak długo, jak długo stara [właściwość](/Property/pl "Property/pl") i nowa [właściwość](/Property/pl "Property/pl") mają ten sam typ _(na przykład `App::PropertyLength` lub `App::PropertyArea`)_, więc stara właściwość może przekazać swoją wartość bezpośrednio. Jeśli jednak nowa właściwość ma inny typ niż stara właściwość, wówczas stara właściwość powinna zostać usunięta, w przeciwnym razie stara właściwość całkowicie nadpisze nową właściwość, co prawdopodobnie nie jest tym, czego chcemy, ponieważ nowa klasa będzie oczekiwać nowego typu, a nie starego.

### Zalety i wady

**Zalety**

- ![](/images/Edit_OK.svg) Podobnie jak metoda 3, ta metoda pozwala nam na pełną kontrolę migracji starych informacji.
- ![](/images/Edit_OK.svg) Unikamy pisania kodu, który usuwa i odtwarza właściwości, które nazywają się tak samo.

**Wady**

- ![](/images/Edit_Cancel.svg) Podobnie jak metoda 3, ta metoda jest nadal bardzo gadatliwa, ponieważ musimy ostrożnie obchodzić się z właściwościami.
- ![](/images/Edit_Cancel.svg) Jeśli nowa [właściwość](/Property/pl "Property/pl") i stara [właściwość](/Property/pl "Property/pl") mają tę samą nazwę, nowa właściwość zostanie nadpisana, co może być niepożądanym zachowaniem, zwłaszcza jeśli obie właściwości mają różne typy. W takim przypadku konieczne jest usunięcie starej właściwości i ręczna migracja jej wartości.
...
- Metoda 3, 3+A lub 3+B. Złożone scenariusze migracji. Pełna kontrola nad właściwościami, usuwanie starych i dodawanie nowych właściwości. Identyfikator umożliwiający poznanie wersji obiektu jest przydatny do wybrania odpowiedniej funkcji do przeprowadzenia migracji _(dodatek B lub B2)_.

Najlepiej unikać następujących metod:

- Metoda 2. Właściwości zostaną zduplikowane, jeśli nowa klasa nie sprawdzi istniejących właściwości _(Dodatek A)_.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Scripted_objects_migration/pl&oldid=1491146>"
