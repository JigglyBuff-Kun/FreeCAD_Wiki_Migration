...
* zastąp *cp437* swoim *console\_encoding*. Aby dowiedzieć się, jakie jest kodowanie twojej konsoli, w systemach Windows i Linux uruchom interpreter Pythona, wpisz import sys, a następnie print sys.stdout.encoding. Python wyświetli twoje console\_encoding na ekranie.

Następnie musisz zapisać poniższy kod Pythona jako plik o nazwie freecadwiki\_family.py w podkatalogu /pywikibot/families *(razem z innymi plikami family\_xxx.py)*.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=WikiRobots/pl&oldid=1431298>"