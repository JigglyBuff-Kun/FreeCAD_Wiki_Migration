...
IfcOpenShell est une bibliothèque de logiciels open source (LGPL 3) qui aide les développeurs à travailler avec les classes de base de l'industrie [[1]](http://www.buildingsmart-tech.org/specifications/ifc-overview) ([Format de fichier IFC](/Arch_IFC/fr "Arch IFC/fr")). Le format de fichier IFC peut être utilisé pour décrire les données de bâtiment et de construction. Le format est couramment utilisé pour la [Building Information Modeling](https://fr.wikipedia.org/wiki/Building_information_modeling) (BIM), par exemple, l'analyse des charges mécaniques et les études d'efficacité thermique et énergétique. IfcOpenShell est principalement une collection de bibliothèques C ++, cependant, comme elle a des liaisons [Python](/Python/fr "Python/fr"), elle peut être intégrée à des programmes comme FreeCAD et Blender.

IfcOpenShell utilise [OpenCASCADE](/OpenCASCADE/fr "OpenCASCADE/fr") en interne pour convertir la géométrie implicite des fichiers IFC en une géométrie explicite que d'autres packages de CAO peuvent comprendre, par exemple, STEP, [OBJ](/Arch_OBJ/fr "Arch OBJ/fr") et [DAE](/Arch_DAE/fr "Arch DAE/fr").

Depuis la version 0.19, FreeCAD est capable d'importer des fichiers IFC tant que le module `ifcopenshell` [Python](/Python/fr "Python/fr") est disponible dans le système. De même, les ateliers [Arch](/Arch_Workbench/fr "Arch Workbench/fr") et [BIM](/BIM_Workbench/fr "BIM Workbench/fr") peuvent exporter un modèle de bâtiment au format IFC afin qu'il puisse être utilisé dans d'autres applications.

Pour vérifier que IfcOpenShell est installé sur votre système, essayez de l'importer depuis la [console Python](/Python_console/fr "Python console/fr"). La bibliothèque est correctement installée si aucun message d'erreur n'est renvoyé.
...
Pour les systèmes Windows et MacOS, les distributions FreeCAD associées au gestionnaire de packages [Conda](/Conda "Conda") incluent généralement IfcOpenShell et donc aucune autre installation n'est nécessaire. Prenez la distribution appropriée sur la page [Téléchargements](/Download/fr "Download/fr").

[AppImage](/AppImage/fr "AppImage/fr") pour Linux est également basé sur [Conda](/Conda "Conda") et inclut également IfcOpenShell.
...
:   * Le chemin de recherche peut être trouvé en inspectant la variable `sys.path` dans la [console Python](/Python_console/fr "Python console/fr").
...
Si le répertoire est correctement déplacé, vérifiez que le module `ifcopenshell` est accessible depuis la [console Python](/Python_console/fr "Python console/fr").
...
La compilation d'IfcOpenShell n'est recommandée que pour les utilisateurs avancés. Le processus est similaire à [compiler FreeCAD sous Linux](/Compile_on_Linux/fr "Compile on Linux/fr"), donc si vous l'avez déjà fait, alors vous pouvez déjà avoir les prérequis nécessaires comme les fichiers de développement [OpenCASCADE](/OpenCASCADE/fr "OpenCASCADE/fr"). Le processus utilise l'outil de configuration CMake pour produire un `Makefile` personnalisé à utiliser avec l'outil Make.
...
2. Rassemblez toutes les dépendances pour la compilation, y compris un compilateur C ++, CMake et Make, et les fichiers de développement pour Boost, libxml2, [OpenCASCADE](/OpenCASCADE/fr "OpenCASCADE/fr"), SWIG, Python et OpenCOLLADA (facultatif). La plupart de ces composants sont strictement facultatifs, cependant, pour une utilisation avec FreeCAD, ils doivent tous être installés. OpenCOLLADA est facultatif car il ne fournit que le support [DAE](/Arch_DAE/fr "Arch DAE/fr") du binaire `IfcConvert`.
3. Exécutez `cmake` pour générer un `Makefile`, puis démarrez la compilation en exécutant `make`.
4. Installez le module `ifcopenshell` [Python](/Python/fr "Python/fr") dans le répertoire `site-packages/` approprié afin qu'il soit trouvé par FreeCAD.
...
Installez les fichiers de développement de [OpenCASCADE](/OpenCASCADE/fr "OpenCASCADE/fr").
...
Pour une utilisation avec FreeCAD, vous avez besoin du wrapper [Python](/Python/fr "Python/fr") qui utilise SWIG pour générer les interfaces appropriées à partir des classes C ++.
...
L'exemple de fichier [IFC](/Arch_IFC/fr "Arch IFC/fr") doit apparaître dans le répertoire build et peut être utilisé comme entrée dans l'exécutable `IfcConvert` également récemment compilé. Cet utilitaire prend en entrée un fichier IFC et produit en sortie un format différent comprenant [OBJ](/Arch_OBJ/fr "Arch OBJ/fr"), [DAE](/Arch_DAE/fr "Arch DAE/fr") si le support OpenCOLLADA était activé, STEP, IGS, XML, [[Draft\_SVG/fr|SVG] ] ou un autre [IFC](/Arch_IFC/fr "Arch IFC/fr").
...
Si aucun fichier de sortie n'est spécifié, il créera par défaut un fichier [OBJ](/Arch_OBJ/fr "Arch OBJ/fr") et sa table de matériaux d'accompagnement (MTL).
...
Maintenant, le module `ifcopenshell` devrait être disponible pour être importé depuis une [console Python](/Python_console/fr "Python console/fr").
...
Au moment de la rédaction de cet article (2020), seule la version [PythonOCC](/PythonOCC/fr "PythonOCC/fr") compilée pour l'édition communautaire (OCE) [OpenCASCADE](/OpenCASCADE/fr "OpenCASCADE/fr") était prise en charge.
...
* [Compilation IfcOpenShell pour MacOS](/Import/Export_IFC_-_compiling_IfcOpenShell/fr "Import/Export IFC - compiling IfcOpenShell/fr"); un guide qui décrit le processus général. Cela peut ne plus être nécessaire car IfcOpenShell est maintenant distribué avec FreeCAD grâce à [Conda](/Conda/fr "Conda/fr").
* Quelles pages sont liées à [cette page](/Special:WhatLinksHere/IfcOpenShell "Special:WhatLinksHere/IfcOpenShell").

Retrieved from "<http://wiki.freecad.org/index.php?title=IfcOpenShell/fr&oldid=1436258>"