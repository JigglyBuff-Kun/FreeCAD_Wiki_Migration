...
Cet exemple utilise l'[Atelier Part](/Part_Workbench/fr "Part Workbench/fr") pour créer un [octaèdre](http://fr.wikipedia.org/wiki/Octaèdre), puis crée sa représentation **[coin](http://www.coin3d.org/) avec pivy**
...
Voir [Méthodes FeaturePython](/FeaturePython_methods/fr "FeaturePython methods/fr") pour la référence complète.
...
Voir [Propriétés personnalisées de FeaturePython](/FeaturePython_Custom_Properties/fr "FeaturePython Custom Properties/fr") pour une vue d'ensemble.
...
Les attributs définis à l'aide de `setEditorMode` peuvent être supprimés par l'utilisateur. Voir l'[éditeur de propriétés](/Property_editor/fr#Menu_contextuel "Property editor/fr"). Notez que les propriétés en lecture seule peuvent toujours être modifiées à partir de Python.

Vous pouvez également définir ces attributs, et d'autres encore, directement avec la fonction `addProperty`. Les attributs définis avec cette fonction ne peuvent pas être modifiés par l'utilisateur. Une possibilité intéressante est de marquer une propriété comme une propriété de sortie. De cette façon, FreeCAD ne marquera pas l'objet comme touché lorsqu'il le modifie (il n'est donc pas nécessaire de recalculer).
...
Les attributs qui peuvent être définis avec `addProperty` sont énumérés ci-dessous. Plusieurs attributs peuvent être définis en ajoutant des valeurs.
...
Pour `Prop_ReadOnly` et `Prop_Hidden`, la fonction `addProperty` a également des arguments booléens :
...
* `group` : sous-section de la propriété (utilisée dans l'[éditeur de propriétés](/Property_editor/fr "Property editor/fr")).
* `doc` : infobulle (idem).
* `attr` : attribut, voir ci-dessus.
* `read_only` : voir ci-dessus.
* `hidden` : voir ci-dessus.
* `enum_vals` : valeurs d'énumération (liste de chaînes), uniquement pertinentes si le type est `"App::PropertyEnumeration"`.
...
* [Viewproviders](/Viewprovider/fr "Viewprovider/fr")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Scripted_objects/fr&oldid=1499192>"