---
title: Création d'une fenêtre de dialogue
---
## Introduction

Dans cette page, nous allons montrer comment construire une interface graphique simple avec [Qt Designer](http://qt-project.org/doc/qt-4.8/designer-manual.html), l'outil officiel de Qt pour concevoir des interfaces. La fenêtre de dialogue sera convertie en code [Python](/Python/fr "Python/fr") puis elle sera utilisée dans FreeCAD. Nous supposerons que l'utilisateur sait comment éditer et exécuter [Python](/Python/fr "Python/fr") en général.

Dans cet exemple, l'interface entière est définie en [Python](/Python/fr "Python/fr"). Bien que cela soit possible pour les petites interfaces, pour les interfaces plus importantes, la recommandation est de charger les fichiers .ui créés directement dans le programme.
...
* Fenêtres de dialogue [modales ou non-modale](http://fr.wikipedia.org/wiki/Fenêtre_modale) :
  ...
  + Une fenêtre de dialogue **non modale** ne vous empêche pas de travailler sur la fenêtre principale, vous pouvez travailler sur les deux fenêtres.
...
Nous allons ensuite utiliser cette fenêtre de dialogue dans FreeCAD pour produire une belle surface plane rectangulaire.  
...
Maintenant, nous allons sauvegarder notre widget quelque part. Il sera sauvegardé dans un fichier **.Ui**, que nous allons facilement convertir en script Python avec **pyuic**.  
...
Et notre fenêtre de dialogue apparaîtra ! Notez que notre interpréteur Python fonctionne toujours, nous avons une fenêtre de dialogue non modale.  
...
Maintenant que nous pouvons afficher et masquer notre fenêtre de dialogue, nous avons juste besoin d'ajouter la dernière partie, pour en faire quelque chose !  
...
Ces signaux peuvent être connectés à des créneaux, qui peuvent être des fonctionnalités spéciales d'autres widgets (par exemple une fenêtre de dialogue a un bouton "Fermer" sur lequel vous pouvez connecter le signal à partir d'un autre bouton "Fermer"), ou, peuvent être des fonctions personnalisées.  
...
Il s'agit, comme vous le voyez, de relier le signal du bouton enfoncé de l'objet a créer (bouton **"Create!"**), à un emplacement nommé **createPlane**, dont nous venons de définir.  
Ça y est ! Maintenant, la touche finale, nous pouvons ajouter une petite fonction, pour créer la fenêtre de dialogue, elle sera plus facile a appeler.  
...
(Rappel sur Python : la méthode **\_\_init\_\_** est une classe qui s'exécute automatiquement chaque fois qu'un nouvel objet est créé ! ). Puis, à partir de FreeCAD, nous avons seulement besoin de faire :
...
Voilà, c'est tout ...  
Maintenant, vous pouvez essayer toutes sortes de choses, comme par exemple l'insertion de votre widget dans l'interface FreeCAD (voir la page [Code snippets](/Code_snippets/fr "Code snippets/fr")), ou, faire des outils personnalisés beaucoup plus avancés, en utilisant d'autres éléments dans votre widget.
...
* [Création d'une boîte de dialogues avec différents widgets](/Dialog_creation_with_various_widgets "Dialog creation with various widgets") avec `QPushButton`, `QLineEdit`, `QCheckBox`, `QRadioButton` et bien d'autres.
* [Création d'une boîte de dialogue lecture et écriture de fichiers](/Dialog_creation_reading_and_writing_files/fr "Dialog creation reading and writing files/fr") with `QFileDialog`.
* [Création d'une boîte de dialogue réglage des couleurs](/Dialog_creation_setting_colors/fr "Dialog creation setting colors/fr") avec `QColorDialog`.
* [Création d'une boîte de dialogue pour image et GIF animé](/Dialog_creation_image_and_animated_GIF/fr "Dialog creation image and animated GIF/fr") avec `QLabel` et `QMovie`.
* [Extraits d'utilisation de PySide](/PySide_usage_snippets/fr "PySide usage snippets/fr").
* [Macro Qt Example](/Qt_Example/fr "Qt Example/fr")

## Liens pertinents

* [Outils de création d'interfaces](/Manual:Creating_interface_tools/fr "Manual:Creating interface tools/fr")

Retrieved from "<http://wiki.freecad.org/index.php?title=Dialog_creation/fr&oldid=1565459>"