---
title: Script de données topologiques
---
## Introduction

Ici, nous vous expliquerons comment contrôler l'[atelier Part](/Part_Workbench/fr "Part Workbench/fr") directement à partir de l'interpréteur FreeCAD Python, ou à partir de n'importe quel script externe. Les principes de base des scripts de données topologiques sont décrits dans le [Module Part expliquant les concepts](/Part_Workbench#Explaining_the_concepts/fr "Part Workbench"). Assurez-vous de parcourir la section [Script](/Scripting/fr "Scripting/fr") et les pages [bases pour script dans Freecad](/FreeCAD_Scripting_Basics/fr "FreeCAD Scripting Basics/fr") si vous avez besoin de plus d'informations sur le fonctionnement des scripts Python dans FreeCAD. Si vous êtes nouveau sur Python, c'est une bonne idée de lire d'abord l'[Introduction à Python](/Introduction_to_Python/fr "Introduction to Python/fr").

### Voir aussi

* [Part É�crire un script](/Part_scripting/fr "Part scripting/fr")
* [OpenCASCADE](/OpenCASCADE/fr "OpenCASCADE/fr")

## Diagramme de classe

Voici un aperçu du [Langage de Modélisation Unifié (UML)](https://fr.wikipedia.org/wiki/UML_(informatique)) de la classe la plus importante du module Part :
![Classes Python du module Part](/images/Part_Classes.jpg)
...
Voir la page de l'[API Part](/Part_API/fr "Part API/fr") ou cette [documentation autogénérée de l'API Python de Part](https://freecad-python-stubs.readthedocs.io/en/latest/autoapi/Part/) pour une liste complète des méthodes disponibles du module Part.
...
Remarque : `makePlane()` accepte uniquement `App.Vector()` pour start\_pnt et dir\_normal et pas les tuples.
...
Lorsque notre matrice est paramétrée, nous pouvons l'appliquer à notre forme. FreeCAD fournit nous fournit 2 méthodes : `transformShape()` et `transformGeometry()`. La différence est que, avec la première, vous ne verez pas de différence (voir [Mettre à l'échelle une forme](#Mettre_à_l'échelle_une_forme) ci-dessous). Donc, nous pouvons opérer notre transformation comme ceci :
...
Il existe plusieurs façons de sauvegarder votre travail. Vous pouvez bien sûr enregistrer votre document FreeCAD, mais vous pouvez également enregistrer des objets [Part](/Part_Workbench/fr "Part Workbench/fr") directement dans des formats CAO courants, tels que BREP, IGS, STEP et STL.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Topological_data_scripting/fr&oldid=1311686>"