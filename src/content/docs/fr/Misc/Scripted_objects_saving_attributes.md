---
title: Sauvegarde des attributs des objets scripts
---
## Introduction
...
Un objet peut être créé à l'aide de cette classe et il peut être enregistré dans my\_document.FCstd. Si aucun [viewprovider](/Viewprovider/fr "Viewprovider/fr") particulier n'est attribué au nouvel objet, sa classe proxy est simplement définie sur une valeur différente de `None`, dans ce cas, sur `1`.
...
Normalement, les [objets créés par script](/Scripted_objects/fr "Scripted objects/fr") devraient utiliser des [propriétés](/Property/fr "Property/fr") pour stocker des informations, car celles-ci sont automatiquement restaurées à l'ouverture du document.
...
Par exemple, la plupart des objets de l'[atelier Draft](/Draft_Workbench/fr "Draft Workbench/fr") configurent un attribut `Type` qui peut être utilisé pour déterminer le type d'objet utilisé.
...
Tous les objets ont une propriété `TypeId`, mais pour les [objets créés par script](/Scripted_objects/fr "Scripted objects/fr"), cette propriété n'est pas utile car elle fait toujours référence à la classe C++ parente, par exemple, [`Part::Part2DObjectPython`](/Part_Part2DObject/fr "Part Part2DObject/fr") ou [`Part::FeaturePython`](/Part_Feature/fr "Part Feature/fr"). Par conséquent, le fait d'avoir cet attribut supplémentaire `Proxy.Type` dans la classe est utile pour traiter chaque objet d'une manière particulière.
...
Si la structure de la classe change, c'est-à-dire si ses propriétés ou méthodes changent, sont renommées ou supprimées, nous pourrions tester l'attribut version afin de faire migrer l'ancien objet vers un nouvel ensemble de propriétés ou vers une nouvelle classe. Ceci peut être fait en implémentant la méthode `onDocumentRestored`, comme expliqué dans [Migration d'objets créés par script](/Scripted_objects_migration/fr "Scripted objects migration/fr").
...
* [Migration d'objets créés par script](/Scripted_objects_migration/fr "Scripted objects migration/fr")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Scripted_objects_saving_attributes/fr&oldid=1359192>"