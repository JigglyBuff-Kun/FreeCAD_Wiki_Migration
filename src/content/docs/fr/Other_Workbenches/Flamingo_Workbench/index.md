...
:   * **Frame tools** : a pour but d'organiser des poutres, des fermes et autres éléments similaires dans FreeCAD en utilisant les objets Structure du module Arch. .../flamingo/tutorials/tutorialFrame.pdf
    ...
    * **Utils** fournit quelques fonctionnalités pour interroger les objets du modèle et leur distance, pour déplacer/pivoter le plan de travail et un petit hack de la boîte de dialogue de création de [Draft Polyligne](/Draft_Wire/fr "Draft Wire/fr"), qui permet de changer la position du plan de travail à la volée.
...
Outil permettant de créer un fil DWire continu sur le chemin défini par les arêtes sélectionnées dans la fenêtre, même si celles-ci ne se touchent pas, se croisent au milieu ou appartiennent à des objets différents. La seule contrainte est qu'il existe une intersection entre deux arêtes consécutives, dans l'ordre dans lequel elles ont été sélectionnées.
De plus, le DWire donne les propriétés d'affichage d'une ligne centrale, c'est-à-dire orange et pointillé.
...
:::caution
Dodo: a changé la portée de cette fonction.Dans dodo, cela ouvre une boîte de dialogue vous permettant de créer 10 formes pour une section de poutre avec des dimensions personnalisées :creux et carré pleincreux et cercle completT, I, C, L, ZomegaIl est également possible pour changer la position du centre ou d'éditer une section existante.
:::
...
Notes : si un bord de l'objet est sélectionné, il est utilisé comme pivot de rotation.
...
Outil pour affleurer les faces parallèles de deux objets.
...
Pour définir la position et l'orientation, les sélections suivantes sont possibles:
deux tuyaux parallèles (éventuellement colinéaires)
...
Créez un "espace réservé" d'une vanne à partir d'une table .csv comme ci-dessus. Outre la dimension de décalage, il est important car il définit également le coefficient Kv qui sera utilisé pour calculer les pertes de pression avec l'outil correspondant dans le menu "Utils".
Notez que le symbole de l'espace réservé change en fonction du type de vanne, si son nom est identifié par un mot clé parmi "boule", "papillon" ou "globe".

:   6) Ajouter une bride

Ouvre une boîte de dialogue pour insérer des brides.
Pour définir la position et l'orientation, les sélections suivantes sont possibles:
...
[Voir tutoriel Part 4 (1/2)](#Liens)

:   11) Insérer une conduite

[Voir tutoriel Part 4 (2/2)](#Liens)
...
Les ateliers FreeCAD sont faciles à programmer en [Python](/Python/fr "Python/fr"), de ce fait de nombreuses personnes développent des ateliers supplémentaires en dehors des développeurs principaux de FreeCAD.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Flamingo_Workbench/fr&oldid=1508164>"