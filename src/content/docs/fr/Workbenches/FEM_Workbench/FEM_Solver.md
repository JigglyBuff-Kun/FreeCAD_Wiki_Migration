...
Il s'agit du premier solveur intégré à fonctionner avec l'atelier FEM. CalculiX est principalement conçu pour les analyses statiques, thermomécaniques et modales. Plus d'informations sur ce solveur se trouvent dans [CalculiX pour l'analyse par la méthode des éléments finis](/FEM_CalculiX/fr "FEM CalculiX/fr").

### Elmer

Le solveur multiphysique Elmer a été intégré à FreeCAD en tant que projet du [Google Summer of Code 2017](/Google_Summer_of_Code_2017 "Google Summer of Code 2017") : [site web principal](https://www.csc.fi/web/elmer), [portail communautaire](http://www.elmerfem.org./), [dépôt du code](https://github.com/ElmerCSC/elmerfem), [intégration d'Elmer (GSoC) - journal d'activité (sujet du forum)](https://forum.freecadweb.org/viewtopic.php?f=18&t=22576).
...
Ces solveurs ne sont pas intégrés dans [atelier FEM](/FEM_Workbench/fr "FEM Workbench/fr") ce qui signifie qu'ils ont besoin d'une interface distincte pour configurer un projet de simulation. Ceci est réalisé par [macros](/Macros/fr "Macros/fr") ou [Ateliers externes](/External_workbenches/fr "External workbenches/fr").
...
DesignSPHysics peut être installé via le [Gestionnaire des extensions](/Std_AddonMgr/fr "Std AddonMgr/fr").
...
L'[atelier EM](/EM_Workbench/fr "EM Workbench/fr") est un atelier externe qui a été créé pour servir d'interface à ces solveurs électromagnétiques. FastHenry, pour l'analyse d'impédance magnéto-quasistatique en 3D, est entièrement supporté, tandis que FasterCap est accessible par le biais de quelques macros Python.
...
L'atelier EM peut être installé via le [Gestionnaire des extensions](/Std_AddonMgr/fr "Std AddonMgr/fr").
...
fcFEM est présenté sous la forme d'une bibliothèque Python et d'une macro. Vous pouvez le télécharger à partir du [dépôt de GitHub](https://github.com/HarryvL/fcFEM). Il sera éventuellement disponible à partir du [gestionnaire des extensions](/Std_AddonMgr/fr "Std AddonMgr/fr") ou sera distribué avec FreeCAD lui-même.
...
Les deux ateliers peuvent être installés via le [Gestionnaire des extensions](/Std_AddonMgr/fr "Std AddonMgr/fr") et disposent d'un espace de discussion dans le [sous-forum CfdOF/CFD](https://forum.freecadweb.org/viewforum.php?f=37).
...
[FenicsSolver](https://github.com/qingfengxia/FenicsSolver) est une plate-forme de simulation permettant de traiter des problèmes multi-corps, multi-physique (couplés) et multi-échelles. Il espère intégrer le solveur FEniCS à la fois dans [atelier FEM](/FEM_Workbench/fr "FEM Workbench/fr") et dans l'[atelier extérieur](/External_workbenches/fr "External workbenches/fr") Cfd afin que le système résultant fonctionne comme une alternative gratuite à Comsol ou à Moose. FenicsSolver est développé par le même auteur de Cfd.
...
* [Module d'extension FEM](/Extend_FEM_Module/fr "Extend FEM Module/fr")
* [Tutoriel pour ajouter des équations MEF](/Add_FEM_Equation_Tutorial/fr "Add FEM Equation Tutorial/fr")
* [Tutoriel pour ajouter des contraintes MEF](/Add_FEM_Constraint_Tutorial/fr "Add FEM Constraint Tutorial/fr")
...
[Frame3DD](http://frame3dd.sourceforge.net/) est un logiciel pour l'analyse structurelle statique et dynamique de cadres et de fermes en 2D et 3D, [principal dépôt](https://github.com/pslack/frame3dd). Un lecteur préliminaire pour les fichiers d'entrée a été annoncé dans les forums :
[Test read data from Frame3DD file](https://forum.freecadweb.org/viewtopic.php?f=24&t=19428). Sujet général dans le forum FEM :
...
Il a été suggéré d'intégrer cette bibliothèque dans FreeCAD dans le cadre du [Google Summer of Code project](/Google_Summer_of_Code "Google Summer of Code") : [GSOC 2019 Configuration Management Project (sujet du forum)](https://forum.freecadweb.org/viewtopic.php?f=8&t=35493).
...
Retrieved from "<http://wiki.freecad.org/index.php?title=FEM_Solver/fr&oldid=1351488>"