...
: Tampilan**View** : properties related to the "visual" display of an object.
...
- Tampilan**Bounding Box** : Indicates if a box showing the overall extent of the object is to be displayed. Value False, or True (Default, False).
- Tampilan**Control Point** : Indicates if the feature control points are to be displayed. Value False, or True (Default, False).
- Tampilan**Deviation** : Sets the accuracy of the polygonal representation of the model in the 3d view (tessellation). Lower values = better quality. The value is in percent of object's size (deviation in mm = (w+h+d)/3\*valueInPercent/100, where w,h,d are the bounding box dimensions).
- Tampilan**Display Mode** :Display mode of the feature, **Flat lines, Shaded, Wireframe, Points\*- ![](/images/Vue_DisplayModePartDesign_fr_00.png). (Default, **Flat lines\*\*).
- Tampilan**Lighting** : Lighting **One side, Two side\*- ![](/images/Vue_Lighting_fr_00.png). (Default, **Two side\*\*).
- Tampilan**Line Color** : Gives the color of the line (edges) (Default, **25, 25, 25**).
- Tampilan**Line Width** : Gives the thickness of the line (edges) (Default, **2**).
- Tampilan**Point Color** : Gives the color of the points (ends of the feature) (Default, **25, 25, 25**).
- Tampilan**Point Size** : Gives the size of the points (Default, **2**).
- Tampilan**Selectable** : Allows selection of the feature. Value False, ou True (Default, True).
- Tampilan**Shape Color** : Give the color shape (default, **204, 204, 204**).
- Tampilan**Transparency** : Sets the degree of transparency in the feature of **0** to **100** (Default, **0**).
- Tampilan**Visibility** : Determines the visibility of the feature (like the bar SPACE). Value False, or True (Default, True).
...
: \* **CopyOnChange**: if active, the property is copied when changed in a Link. The Link's Data**Link Copy on Change** property must be set to `Tracking` or `Enabled` for this to work. This is related to [Variant Links](https://forum.freecad.org/viewtopic.php?p=738833#p738833).
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Property_editor/id&oldid=1498279>"
