...
**Placement**是用户在FreeCAD中指定目标对象在空间中位置与姿态（朝向）的手段。指定方位属性的方式多种多样，例如通过 [脚本](/Python_scripting_tutorial#Vectors_and_Placements "Python scripting tutorial")、属性面板或**Placement**对话框（位于**Edit**菜单中）。
...
以脚本的方式利用y/p/r参数、矩阵与[定位API](/Placement_API "Placement API")来调整方位属性。
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Placement/zh-cn&oldid=1560857>"