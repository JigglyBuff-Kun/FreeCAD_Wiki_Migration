...
在你经历痛苦的调试过程之前请使用 [测试框架](/Testing "Testing") 来检查标准测试是否正常工作。如果它们未完全运行, 则可能这是不完整的安装。
...
* 安装的 gdb 软件包
* FreeCAD 的调试版本 (此时仅可通过 [从源代码构建](/Compile_on_Linux#For_a_Debug_build "Compile on Linux"))
* 导致崩溃的 FreeCAD 模式
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Debugging/zh-cn&oldid=1328486>"