...
FreeCAD的发展历史可以从2001年1月讲起。那时[Jürgen Riegel](/User:Jriegel "User:Jriegel")开始为Cas.CADE项目工作。这个项目是一个商用软件开发框架。它包括一个[几何建模内核](/Glossary/zh-cn#Geometric_modeling_kernel "Glossary/zh-cn")（或者叫CAD内核）。这个内核在2000年发布于一个开源协议之下，并更名为[开放的小瀑布](/Glossary#Open_CASCADE "Glossary")(缩写CAS.CADE的原意：为计算机辅助设计和工程服务的计算机辅助软件)。这就使得开发一个开源的3D CAD软件成为可能，因为非得自己从头开始编织CAD内核将会耗费巨大的工作量。
...
>
> *—[Jürgen Riegel](/index.php?title=User:Jriegel/zh-cn&action=edit&redlink=1 "User:Jriegel/zh-cn (page does not exist)"),*[谁站在FreeCAD背后?](http://forum.freecadweb.org/viewtopic.php?f=8&t=295)
> "
...
>
> —[Jürgen Riegel](/User:Jriegel "User:Jriegel"), *[发布FreeCAD项目](http://www.opencascade.org/org/forum/thread_6572/?forum=11)*
...
[Yorik van Havre](/User:Yorik "User:Yorik")2008年加入了这个项目，开始工作于[绘图模块](/Draft_Workbench/zh-cn "Draft Workbench/zh-cn")。在那之前，用户没法通过[用户图形界面](/Glossary/zh-cn#GUI "Glossary/zh-cn")创建两维几何体。这个模块完全采用Python开发，而不是C++，后者是FreeCAD采用的核心编程语言。这就证明了Python集成的成功，它可以用来扩展FreeCAD的能力或定制FreeCAD。Yorik除了制图模块，还致力于扩展FreeCAD的文档，并成了FreeCAD实际意义上的艺术总监，因为他为FreeCAD的用户图形界面创作出很多图标，[定义了它的风格](/index.php?title=Artwork/zh-cn&action=edit&redlink=1 "Artwork/zh-cn (page does not exist)")。

FreeCAD的0.7版在2009年4月发布。它第一次包含了绘图模块。零件模块提供了一个简单的[CSG建设性实体几何元素](/Glossary/zh-cn#Constructive_Solid_Geometry "Glossary/zh-cn")工作流程，先创建基础形状，然后通过零件菜单，对他们做布尔运算。它也有一些扩展，可以实现2D轮廓和倒角操作。
...
2010年7月发布的0.10版引入了[草图模块](/Sketcher_Workbench/zh-cn "Sketcher Workbench/zh-cn")。它创建2D几何体的基础是草图解算器，一个基于约束的解算器。第一版的功能仅限于创建长方形和线。
...
Retrieved from "<http://wiki.freecad.org/index.php?title=History/zh-cn&oldid=1508934>"