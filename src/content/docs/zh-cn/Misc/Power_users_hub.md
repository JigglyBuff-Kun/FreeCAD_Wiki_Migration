...
这些页面还远没有成熟。如果你没找到你要找的信息，或者你在别的地方发现了有用的信息，而我们没有链接在这里，那为什么不[自己添加进来](/Help_FreeCAD "Help FreeCAD")？同时，请在[论坛](http://forum.freecadweb.org/)留下你的意见。

## 定制FreeCAD

* [界面定制](/Interface_Customization "Interface Customization"): 从头开始：工具栏和快捷方式
* [用宏工作](/Macros "Macros"): 轻松录制多遍任务或Python代码
* [宏菜单](/Macros_recipes "Macros recipes")
* [定制工具栏](/Customize_Toolbars "Customize Toolbars")
* [安装更多工作台](/Installing_more_workbenches "Installing more workbenches")
...
* [介绍Python](/Introduction_to_Python "Introduction to Python") - 在页面底端还有其他Python教程。
* [FreeCAD脚本教程](/Python_scripting_tutorial "Python scripting tutorial") - 概略看一下在FreeCAD中用Python写脚本。
* [FreeCAD脚本基础](/FreeCAD_Scripting_Basics "FreeCAD Scripting Basics") - 好吧，基本的东西……
* [Gui命令](/Gui_Command "Gui Command") - 向GUI添加定制的命令。
* 在FreeCAD里使用混合[单位](/Units "Units")。

### 模块

在FreeCAD中，功能性的东西被划分到各个模块，处理特定的数据类型和应用。FreeCAD有内建模块和扩展模块（插件）。插件模块一旦安装，它们就和内建模块一样方便触达。下面讲的是默认模块，FreeCAD的每一个安装包都包含了。

* [内建模块](/Builtin_modules "Builtin modules")是FreeCAD的主体模块。里面的工具，可以调整FreeCAD的一般配置、文档和内容。
* [创建工作台](/Workbench_creation "Workbench creation")展示给你如何创建自己的工作台。

#### Working with Meshes

### 网格类工作

* [网格脚本](/Mesh_Scripting "Mesh Scripting")：怎样与[网格模块](/Mesh_Workbench "Mesh Workbench")交互。

#### Working with Parts

### 零件类工作

* [零件模块](/Part_Workbench "Part Workbench")：[开源CASCADE技术](http://en.wikipedia.org/wiki/Open_CASCADE)的工具和体系如何用在FreeCAD中。
* [拓扑数据脚本](/Topological_data_scripting "Topological data scripting")：怎样与零件模块交互。
* [PythonOCC](/PythonOCC "PythonOCC")：怎样释放开源CASCADE的全部能量。
* [网格到零件](/Mesh_to_Part "Mesh to Part")：转换对象类型。

#### Accessing the Coin scenegraph

### 触达Coin的场景图

* [Coin/Inventor的场景图](/Scenegraph "Scenegraph")：FreeCAD的场景表现如何运作。
* [Pivy](/Pivy "Pivy")：如何触达和变更场景图。

### Controlling the Qt interface

### 控制Qt接口

* [PySide](/PySide "PySide")：如何触达接口，并修改它的内容。
* 在另一个Qt应用里，通过PyQt，来[使用FreeCAD的GUI](/Embedding_FreeCADGui "Embedding FreeCADGui")。

### 参数对象类工作

* [脚本化的对象](/Scripted_objects "Scripted objects")：在FreeCAD里如何制作100%的Python脚本化的对象。
* [制图模块](/Drawing_Workbench "Drawing Workbench")：让3D到2D的过程实现自动化。
...
* [代码片段](/Code_snippets "Code snippets")：搜集了一些FreeCAD的Python代码段，作为你的脚本的配料……
* [画线函数](/Line_drawing_function "Line drawing function")：如何建造一个简单工具，来画线。
* [创建对话](/Dialog_creation "Dialog creation")：如何用Qt设计器来建造对话框，把它们用在FreeCAD里。
* [嵌入式FreeCAD](/Embedding_FreeCAD "Embedding FreeCAD")：在其他应用里，如何载入FreeCAD，作为一个Python模块。
* [绘图模块](/Draft_Workbench "Draft Workbench")为FreeCAD加入了2D绘图函数。它整个都是用Python写的，所以如果你想写自己的模块，它就是个好例子。
* [FreeCAD向量数学程序库](/FreeCAD_vector_math_library "FreeCAD vector math library")：几个很顺手的函数，来操控FreeCAD的向量。这个函数库也包含在绘图模块里面了。

## API函数

FreeCAD完整的API文档放在http://www.freecadweb.org/api/中。它既有C++也有Python的API，还没有完全格式化好。当你想找只含Python的代码，就会发现那里有点乱。更便于浏览的版本在[这里](/Category:API "Category:API")。注意，它可能并不完整，因为它得用人工更新。要找更精确信息的话，直接在FreeCAD的Python控制台里浏览相应模块吧。

Related: [Exposing C++ to Python](/Exposing_C%2B%2B_to_Python "Exposing C++ to Python")

## 高级修改

* [启动和配置](/Start_up_and_Configuration "Start up and Configuration")：启动和命令行选项。
* [在Windows上安装](/Installing_on_Windows "Installing on Windows")：使用Windows安装器。
* [在Windows上编译FreeCAD](/Compile_on_Windows "Compile on Windows")，[在Unix上编译FreeCAD](/Compile_on_Linux "Compile on Linux")
* [品牌化](/Branding "Branding")：简单变更FreeCAD的源代码，改变FreeCAD的一些方面。
* [额外的Python模块](/Extra_python_modules "Extra python modules")：用这些强大的模块，来扩展FreeCAD的Python解释器。
...
在[社区入口](/FreeCAD_Community_Portal "FreeCAD Community Portal")，你会找到其它基于FreeCAD的项目，它们由FreeCAD用户社区运行。如果你正开始一个新的FreeCAD项目，一定要登记上去。如果你愿意[帮助FreeCAD](/Help_FreeCAD "Help FreeCAD")，我们也准备了那个页面，列出了你可以做的事情。

Retrieved from "<http://wiki.freecad.org/index.php?title=Power_users_hub/zh-cn&oldid=1463669>"