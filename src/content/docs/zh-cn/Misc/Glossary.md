...
:   建筑[工作台（workbench）](#Workbench)的缩写，主要用于构建各种建筑物与结构的模型。它与 [草图工作台](#Draft)密切相关。

应用程序(App)
:   The FreeCAD App layer is AKA Graphical interface ([GUI](#GUI)) layer.

[Assembly](/Assembly "Assembly")
:   1.  相对于彼此的位置关系所定义的一组[零件](#Part)。
:   2.  一种[工作台](#Workbench)，目标是便于将所创零件装配在一起。当前正处于开发过程中，还未作为FreeCAD的一部分。
...
内核（Kernel）
:   参见[几何建模内核（Geometric modeling kernel）](#Geometric_modeling_kernel)。
...
Raytracing
:   [光线追踪（Ray tracing），简作光追](http://en.wikipedia.org/wiki/Ray_tracing_%28graphics%29)。

Revolve
:   A tool in the [Part](/Part_Workbench "Part Workbench") [workbench](#Workbench). See [Part Revolve](/Part_Revolve "Part Revolve").

机器人（Robot）
:   [工业机器人（Industrial robot）](http://en.wikipedia.org/wiki/Industrial_robot)
...
并集（Union）
:   一种在所选形状上执行特定[布尔运算](#Boolean_Operation)的[零件](/Part_Workbench "Part Workbench")[工作台](#Workbench)工具。

不稳定的（Unstable）
:   对FreeCAD软件最新版本的昵称。此版本中将包括开发者最近实现的许多变更内容。它通常不会出现错误或产生错误结果，但是却未经过完整的测试，因而有一定的风险。

[Upgrade](/Draft_Upgrade "Draft Upgrade")
:   一种[草图](/Draft_Workbench "Draft Workbench") [工作台](#Workbench)工具。
...
顶点（Vertex）
:   空间中的单独[点（point）](#Point)，或[图形（shape）](#shape)中[边（edges）](#Edge)间的交点。Open Cascade Technology将其定义为：对应于几何图形中点的零维[图形（shape）](#shape)。[[参见OCCT Profile: Defining the topology](https://dev.opencascade.org/doc/overview/html/occt__tutorial.html#OCCT_TUTORIAL_SUB2_3%7C)]
...
WB
:   [工作台（workbench）](#Workbench)的缩写。

连线（Wire）
:   1.  多个[顶点（vertices）](#Vertex)间的一系列连接[边（edges）](#Edge)。此术语常用于Open Cascade Technology [[[1]](https://dev.opencascade.org/doc/overview/html/occt__tutorial.html#OCCT_TUTORIAL_SUB2_3)]之中，因此也用于FreeCAD。
:   2.  用于创建参数化连线的草图工作台（[Draft](/Draft_Workbench "Draft Workbench") [workbench](#Workbench)）命令。

工作台（Workbench）
:   又称模块（module），每个 [工作台（workbench）](/Workbenches "Workbenches")囊括了处理某个特定任务的工具集。
...
Z
:   常指3D坐标系中的Z[坐标轴（axis）](#Axis)。

Retrieved from "<http://wiki.freecad.org/index.php?title=Glossary/zh-cn&oldid=1461730>"