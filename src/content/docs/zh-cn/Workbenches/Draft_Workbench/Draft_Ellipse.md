...
椭圆工具会根据用户输入的两个点，在当前的[工作平面](/Draft_SelectPlane "Draft SelectPlane")内创建一个椭圆。该椭圆将恰好适配于这两个矩形角点所定义的矩形盒中。另外，此工具将根据[Draft Tray](/Draft_Tray "Draft Tray")中的[Draft Linestyle](/Draft_Linestyle "Draft Linestyle")设置来绘制椭圆。

此工具也可用于创建椭圆弧，此时还要指定弧的起始角度与结束角度。而要创建圆与圆弧则可采用[底图圆形](/Draft_Circle "Draft Circle")工具与[底图弧形](/Draft_Arc "Draft Arc")工具。另外，您还可以通过[底图B样条](/Draft_BSpline "Draft BSpline")与[底图贝塞尔曲线](/Draft_BezCurve "Draft BezCurve")工具来创建近似的椭圆弧与圆弧。
...
1. 按下![](/images/Draft_Ellipse.png) [Draft Ellipse](/Draft_Ellipse "Draft Ellipse")按钮，或先按E键再按L键。
2. 在3D视图中点击第一个点，或输入一个坐标并按下![](/images/Draft_AddPoint.svg) add point按钮。
3. 在3D视图中点击第二个点，或输入一个坐标并按下![](/images/Draft_AddPoint.svg) add point按钮。
...
* Press F or click the **Filled**1.0 and below or **Make face**[introduced in 1.1](/Release_notes_1.1 "Release notes 1.1") checkbox to toggle make face mode. If make face mode is on, the created ellipse will have 数据**Make Face** set to `true` and have a filled face.
...
* 数据**Area** (`Area`): (read-only) specifies the area of the face of the ellipse. The value will be `0.0` if 数据**Make Face** if `false` or the face cannot be created.
* 数据**First Angle** (`Angle`): specifies the angle of the first point of the ellipse, normally `0°`.
* 数据**Last Angle** (`Angle`): specifies the angle of the last point of the ellipse, normally `0°`.
* 数据**Major Radius** (`Length`): specifies the major radius of the ellipse.
* 数据**Make Face** (`Bool`): specifies if the ellipse makes a face or not. If it is `true` a face is created, otherwise only the perimeter is considered part of the object. This property only works if the shape is a full ellipse.
* 数据**Minor Radius** (`Length`): specifies the minor radius of the ellipse.

### View

Draft

* 视图**Pattern** (`Enumeration`): specifies the [Draft Pattern](/Draft_Pattern "Draft Pattern") with which to fill the face of the ellipse. This property only works if 数据**Make Face** is `true` and if 视图**Display Mode** is `Flat Lines`.
* 视图**Pattern Size** (`Float`): specifies the size of the [Draft Pattern](/Draft_Pattern "Draft Pattern").
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Draft_Ellipse/zh-cn&oldid=1556229>"