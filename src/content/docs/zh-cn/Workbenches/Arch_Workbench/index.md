...
由于建筑工作台要利用 2D 对象来构建其建筑对象，因此，它导入了[底图工作台（Draft Workbench）](/Draft_Workbench "Draft Workbench")中的所有工具。除此以外，建筑工作台亦可使用如[零件工具台（Part）](/Part_Workbench "Part Workbench")与[零件设计工作台（PartDesign）](/PartDesign_Workbench "PartDesign Workbench")所创建的对象。

FreeCAD 中的 BIM 功能正逐步拆分至此建筑工作台中，它持有基本的建筑体工具与[BIM Workbench](/BIM_Workbench "BIM Workbench")，后者可以利用[Addon Manager](/Std_AddonMgr "Std AddonMgr")进行安装。本工作台在建筑工具（Arch tools）之上添加了一个新的接口层，目标是令 FreeCAD 中的 BIM 工作流程更为直观且更易于使用。
...
- ![](/images/Preferences-arch.svg) [建筑工作台首选项（Preferences）](/Arch_Preferences "Arch Preferences"): 针对墙体、构件、钢筋、窗口、台阶、面板、管道、网格与坐标轴默认外观的设置首选项。

### 文件格式

- [IFC](/Arch_IFC "Arch IFC") : 建筑业国际工业标准（Industry foundation Classes）
- [DAE](/Arch_DAE "Arch DAE") : Collada 网格格式
- [OBJ](/Arch_OBJ "Arch OBJ") : Obj 网格格式 (仅限于导出)
- [JSON](/Arch_JSON "Arch JSON") : JavaScript 对象表示法格式 (仅限于导出)
- [3DS](/Arch_3DS "Arch 3DS") : 3DS 格式 (仅限于导入)

## API

在[Python](/Python "Python")脚本与[宏（macros）](/Macros "Macros")中调用[Arch Python API](http://www.freecadweb.org/api/Arch.html)函数即可使用本建筑模块。
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Workbench/zh-cn&oldid=1433541>"
