...
* 对一个[Draft Shape String](/Draft_ShapeString "Draft ShapeString")进行挤型操作，将生成一个复合实体（生成的字符串是由多个字母实体复合而成）
...
2. 点击工具栏中的![](/images/Part_Extrude.png) **Extrude** 图标，或前往Part → Extrude菜单
3. 设置挤型的方向与长度，以及其他可选参数（参见后面的[参数](#Parameters)部分获得更多详情）。
4. 单击OK。
...
* **Placement**: 标准的[定位](/Placement "Placement")参数集
...
## 与[PartDesign Pad](/PartDesign_Pad "PartDesign Pad")的比较
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Part_Extrude/zh-cn&oldid=1334419>"