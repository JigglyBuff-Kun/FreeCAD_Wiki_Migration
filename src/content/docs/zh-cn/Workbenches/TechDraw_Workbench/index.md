...
![](/images/Workbench_TechDraw.svg) [TechDraw 工作台](/TechDraw_Workbench "TechDraw Workbench")被用来从来自其他工作台（比如[Part](/Part_Workbench "Part Workbench"), [PartDesign](/PartDesign_Workbench "PartDesign Workbench"), 或 [Arch](/Arch_Workbench "Arch Workbench")）创建的 或 从其他应用程序导入的 3D 模型生成基本的技术图纸。每张图纸是一页，可以包含可绘制对象(比如 Part::Features, PartDesign::Bodies, App::Part groups 和文档对象组)的各种视图。得到的图纸可被用于诸如文档、制造说明、合同、许可等。

尺寸标注、剖面、填充区、注解和[SVG](/SVG "SVG")符号都可以加到图纸中，而这些又可以进一步导出到不同的格式，如 [DXF](/DXF "DXF"), [SVG](/SVG "SVG"), [PDF](/PDF "PDF").

如果你的主要目标是产生复杂的 2D 图纸和[DXF](/DXF "DXF")文件，并且不需要 3D 建模，FreeCAD 可能不是正确的选择。你应该考虑一个用于绘制工程草图的专门的软件，如 [LibreCAD](https://en.wikipedia.org/wiki/LibreCAD) 或 [QCad](https://en.wikipedia.org/wiki/QCad).
...
- ![](/images/TechDraw_PageDefault.svg) [插入默认页](/TechDraw_PageDefault "TechDraw PageDefault"): 使用默认[模板](/TechDraw_Templates "TechDraw Templates")添加一个新页面 .
...
Retrieved from "<http://wiki.freecad.org/index.php?title=TechDraw_Workbench/zh-cn&oldid=1492923>"
