---
title: Módulo Plot
---

![](/images/Workbench_Plot.svg)

Plot workbench icon

## Introdução

FreeCAD is able to perform plots using the [matplotlib](https://matplotlib.org/) [Python](/Python "Python") library. A module is provided to this end, as an external add-on in version 0.19 and as a core component from version 0.20 on. Older versions of FreeCAD are not covered in this documentation.

The produced plots offer the standard [matplotlib](https://matplotlib.org/) tools to edit and save. On top of that, a ![](/images/Workbench_Plot.svg) [Plot Workbench](/Plot_Workbench "Plot Workbench") is provided as an external add-on offering more complete tools to edit the plot and save it. The add-on can be installed with the [Add-on manager](/Std_AddonMgr "Std AddonMgr").

## Module

The module can be invoked in a Python console or in a [macro](/Macros "Macros"). The first thing you must do is importing the module. In FreeCAD 0.19 you must first install the ![](/images/Workbench_Plot.svg) [Plot Workbench](/Plot_Workbench "Plot Workbench") using the [Add-on manager](/Std_AddonMgr "Std AddonMgr"), and then you can import Plot by typing:

```
from freecad.plot import Plot

```

Since FreeCAD 0.20 the plot module is already packaged with the program, so you don't need to install any add-on, but just type:

```
from FreeCAD.Plot import Plot

```

After that, you can plot a straight line from (0,0) to (1,2) just simply typing:

```
Plot.plot([0, 1], [0, 2])

```

You can find more complex examples in the [Plot Basic tutorial](/Plot_Basic_tutorial "Plot Basic tutorial") and the [Plot MultiAxes tutorial](/Plot_MultiAxes_tutorial "Plot MultiAxes tutorial").

## Ferramentas

If you decide to install the ![](/images/Workbench_Plot.svg) [Plot Workbench](/Plot_Workbench "Plot Workbench") using the [Add-on manager](/Std_AddonMgr "Std AddonMgr"), you will have the following tools available to manage the plots created with the module:

- ![](/images/Plot_Save.png) [Salvar gráfico](/index.php?title=Plot_Save/pt-br&action=edit&redlink=1 "Plot Save/pt-br (page does not exist)"): Salva o gráfico em vários formatos. Você pode selecionar o tamanho de saída, assim também como a resolução.
- ![](/images/Plot_Axes.png) [Eixos](/index.php?title=Plot_Axes/pt-br&action=edit&redlink=1 "Plot Axes/pt-br (page does not exist)"): Adiciona, remove ou edita eixos do gráfico.
- ![](/images/Plot_Series.png) [Séries](/index.php?title=Plot_Series/pt-br&action=edit&redlink=1 "Plot Series/pt-br (page does not exist)"): Edita o título e o estilo da série.
- ![](/images/Plot_Grid.png) [Grade](/index.php?title=Plot_Grid/pt-br&action=edit&redlink=1 "Plot Grid/pt-br (page does not exist)"): Mostra/Oculta a grade.
- ![](/images/Plot_Legend.png) [Legenda](/index.php?title=Plot_Legend/pt-br&action=edit&redlink=1 "Plot Legend/pt-br (page does not exist)"): Mostra/Oculta a legenda.
- ![](/images/Plot_Labels.png) [Rótulos](/index.php?title=Plot_Labels/pt-br&action=edit&redlink=1 "Plot Labels/pt-br (page does not exist)"): Edita rótulos.
- ![](/images/Plot_Positions.png) [Posições](/index.php?title=Plot_Positions/pt-br&action=edit&redlink=1 "Plot Positions/pt-br (page does not exist)"): Coloca as posições dos elementos.

## Scripting

## Scripting

Como o Plot Workbench é uma camada sobre `matplotlib`, você está livre para usar qualquer função desta biblioteca em instâncias de plotagem. Veja [[Scripting\_and\_macros/pt-br|Scripting and macros] para exemplos.

## Tutorial

## Tutorial

- [Plot Basic tutorial](/index.php?title=Plot_Basic_tutorial/pt-br&action=edit&redlink=1 "Plot Basic tutorial/pt-br (page does not exist)")
- [Plot MultiAxes tutorial](/index.php?title=Plot_MultiAxes_tutorial/pt-br&action=edit&redlink=1 "Plot MultiAxes tutorial/pt-br (page does not exist)")

Retrieved from "<http://wiki.freecad.org/index.php?title=Plot_Workbench/pt-br&oldid=1157578>"
