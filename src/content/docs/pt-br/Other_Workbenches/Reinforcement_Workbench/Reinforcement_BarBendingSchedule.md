---
title: Cronograma de dobra da barra de reforço
---
|  |
| --- |
...
* Dados**Reinforcement Group By**: Reinforcement Group By specifies how reinforcement objects should be grouped in Bar Bending Schedule i.e. "Host" or "Mark".
* Dados**Rebar Length Type**: Rebar Length Type specifies the type of rebar length used for BOM calculations i.e. "RealLength" or "LengthWithSharpEdges".
* Dados**Column Headers**: A dictionary with column\_data as key and tuple (column\_display\_header, column\_sequence) as value.
* Dados**Column Units**: A dictionary with keys: "Diameter", "RebarLength", "RebarsTotalLength" and their corresponding units as value.
* Dados**Font Family**: Font Family of text in Bar Bending Schedule SVG.
* Dados**Font Size**: Font size in mm.
* Dados**Column Width**: Width of each column in Bar Bending Schedule SVG.
* Dados**Row Height**: Height of each row in Bar Bending Schedule SVG.
* Dados**SVG Output File**: The output file to write generated rebar shape cut list SVG.

**Rebar Shape column data:** The data related to the Rebar Shape column in Bar Bending Schedule

* Dados**Column Header**: The column header for the rebar shape column.
* Dados**Stirrup Extended Edge Offset**: The offset of extended end edges of the stirrup, so that end edges of the stirrup with a 90-degree bent angle do not overlap with stirrup edges.
* Dados**Rebars Stroke Width**: The stroke-width of rebars in the rebar shape column.
* Dados**Rebars Color Style**: The color style of rebars.

**Rebar Shape Column Dimension Data:** The data related to rebar shape dimensions in the Rebar Shape column

* Dados**Include Dimensions**: If True, then each rebar edge dimensions and bent angle dimensions will be included in the rebar shape cut list.
* Dados**Include Units in Dimension Label**: If it is True, then rebar edge length units will be shown in dimension label.
* Dados**Rebar Edge Dimension Units**: The units to be used for rebar edge length dimensions.
* Dados**Rebar Edge Dimension Precision**: The number of decimals that should be shown for rebar edge length as a dimension label.
* Dados**Dimension Font Family**: The font-family of dimension text.
* Dados**Dimension Font Size**: The font-size of dimension text.
* Dados**Bent Angle Dimension Exclude List**: The list of bent angles to not include their dimensions.
* Dados**Helical Rebar Dimension Label Format**: The format of the helical rebar dimension label. e.g. "%L,r=%R,pitch=%P" where %L -> Length of helical rebar, %R -> Helix radius of helical rebar, %P -> Helix pitch of helical rebar.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Reinforcement_BarBendingSchedule/pt-br&oldid=1372729>"