---
title: Bancada de trabalho A2plus
---
![](/images/A2p_workbench.svg)

Icone bancada de trabalho A2plus
...
The first added part gets a fixed position by default. (You can change this later via the part property Dados**fixed Position**.)
...
Constraints can be disabled by changing its [visibility](/Std_ToggleVisibility "Std ToggleVisibility"). This is done by selecting the constraint in the tree view and pressing Space. This toggles the property Dados**Suppressed**. A suppressed constraint is not taken into account when the assembly is solved.
...
Constraints can be temporarily suppressed by selecting them in the model tree and changing the tree element property Dados**Suppressed**.
...
:   The solver begins with a part that has the property Dados**fixed Position** set to *true* and a part constrained to it. All other parts are not calculated. If a solution could be found, the next constrained part is added to the calculation and so on.

Use "magnetic" solver, solving all parts at once
:   The solver tries to move all parts at once in direction to a part that has the property Dados**fixed Position** set to *true*. Note that this will in most cases take more time for the calculation of a solution.

Force fixed position
:   This sets the property Dados**fixed Position** to *true* for all parts in the assembly. Then no calculation is actually performed since all parts will always be fixed to the positions where they were created.
...
:   While importing parts to the assembly an algorithm generates topological names for each subelement of the imported shape. The topological names are written into the Dados**mux Info**. When an imported part needs to be updated, these topological names are used to update the subelements of the constraints. So assemblies get more robust against volatile subelement numbers of FreeCAD.  
...
**Note:** Assure that at least one part of the constraint has the property Dados**fixed Position** set to *false*.

### Setting the Tip Property

If you miss some features of your part after the import to an A2plus assembly, check the property Dados**[Tip](/PartDesign_MoveTip "PartDesign MoveTip")**.

A2plus imports bodies of parts with all their features up to the tip feature. This is sensible because setting the tip to a certain feature means that all features behind the tip should not appear in the final part. So if you miss a part feature in A2plus, open the part via the toolbar button ![](/images/A2p_EditPart.svg), then select a body and look at its property Dados**Tip**. If the tip is not at the feature where you want it, right-click on the feature where the tip should be and choose **![](/images/PartDesign_MoveTip.svg) Set tip**. Finally save the part and reload the assembly using the toolbar button ![](/images/A2p_ImportPart_Update.svg).
...
When you set a constraint between two parts and no part has the property Dados**fixed Position** set to *true* or is connected by a constraint to a part with Dados**fixed Position** set to *true*, the constraint cannot be solved. The same happens if both parts of the constraint have Dados**fixed Position** set to *true*.
...
Therefore check that at least one of your parts in the assembly has Dados**fixed Position** set to *true*. Then assure that you only set constraints to a part which is somehow connected to the fixed part. To visualise these dependencies, see section [Assembly Structure](#Assembly_Structure).
...
Retrieved from "<http://wiki.freecad.org/index.php?title=A2plus_Workbench/pt-br&oldid=1415018>"