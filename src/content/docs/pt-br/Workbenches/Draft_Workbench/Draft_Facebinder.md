...
* Dados**Area** (`Area`): (read-only) specifies the total area of the linked faces of the facebinder.
* Dados**Extrusion** (`Distance`): specifies the extrusion thickness of the facebinder. [introduced in 1.1](/Release_notes_1.1 "Release notes 1.1"): If the facebinder has connected faces and Dados**Sew** is `true`, an attempt is made to close the corners of the extrusion. If this is not possible faces are extruded separately.
* Dados**Faces** (`LinkSubList`): specifies the linked faces of the facebinder.
* Dados**Offset** (`Distance`): specifies an offset distance to apply between the facebinder and the original faces, prior to extrusion.
* Dados**Remove Splitter** (`Bool`): Specifies whether to remove splitter lines that divide co-planar faces of the facebinder.
* Dados**Sew** (`Bool`): Specifies whether to perform a topological sewing operation on the facebinder.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Draft_Facebinder/pt-br&oldid=1551504>"