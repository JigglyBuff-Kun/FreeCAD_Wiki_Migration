...
* Dados**Dimline** (`VectorDistance`): specifies the point through which the dimension line passes.
* Dados**Linked Geometry** (`LinkSubList`): specifies the object and its subelement(s) the dimension is linked to.
* Dados**Normal** (`Vector`): specifies the normal of the plane of the text.
* Dados (hidden)**Support** (`Link`): specifies the measured object.

Linear/radial dimension

* Dados**Direction** (`Vector`): specifies the direction of the measurement.
* Dados**Distance** (`Length`): (read-only) specifies the value of the measurement.
* Dados**End** (`VectorDistance`): specifies the end point of the measurement.
* Dados**Start** (`VectorDistance`): specifies the start point of the measurement.

Radial dimension

* Dados**Diameter** (`Bool`): specifies if a radial dimension is displayed as a diameter dimension. Not used for linear dimensions.

### Data angular dimension

Angular dimension

* Dados**Angle** (`Angle`): (read-only) specifies the value of the measurement.
* Dados**Center** (`VectorDistance`): specifies the center of the measurement.
* Dados**First Angle** (`Angle`): specifies the start angle of the measurement.
* Dados**Last Angle** (`Angle`): specifies the end angle of the measurement.

Dimension

* Dados**Dimline** (`VectorDistance`): specifies the point through which the dimension arc passes.
* Dados (hidden)**Linked Geometry** (`LinkSubList`): not used.
* Dados (hidden)**Normal** (`Vector`): specifies the normal of the plane of the dimension.
* Dados (hidden)**Support** (`Link`): not used.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Draft_Dimension/pt-br&oldid=1565035>"