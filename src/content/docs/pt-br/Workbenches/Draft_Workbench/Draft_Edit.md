...
* A single node to control the height of the wall is displayed above the Dados**Placement** of the wall.
* No context menus for this object.
...
* The color of the temporary nodes is the same as the color of the snap symbols. This color can be changed in the preferences: **Edit → Preferences... → Draft → Grid and snapping → Snap symbol color**. Note that this color is not used for the temporary nodes displayed for [Draft BezCurves](/Draft_BezCurve "Draft BezCurve"). These nodes use the Vista**Line Color** of the curve instead.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Draft_Edit/pt-br&oldid=1513487>"