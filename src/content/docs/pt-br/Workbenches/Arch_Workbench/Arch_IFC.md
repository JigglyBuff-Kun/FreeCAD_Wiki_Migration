...
The ![](/images/Workbench_BIM.svg) [BIM Workbench](/BIM_Workbench "BIM Workbench") supports [Industry Foundation Classes (IFC)](http://en.wikipedia.org/wiki/Industry_Foundation_Classes) files natively, and also provides an importer and exporter. The IFC format is a continuously growing format to interchange data among [BIM](http://en.wikipedia.org/wiki/Building_Information_Modeling) applications, used in architecture and engineering.

Read more about handling IFC files in FreeCAD on the [Native IFC](/Native_IFC "Native IFC") page.

#### IfcOpenShell

All the IFC functionality depends on the [IfcOpenShell](/IfcOpenShell "IfcOpenShell") library, which is bundled in some distributions of FreeCAD. An easy way to check if IfcOpenShell is available is to enter the following in the [Python console](/Python_console "Python console"):
...
If no error message appears, IfcOpenShell is installed, and you may proceed with [opening](/Std_Open "Std Open") or [importing](/Std_Import "Std Import") IFC files. Otherwise, you will need to install IfcOpenShell yourself; read the [IfcOpenShell](/IfcOpenShell "IfcOpenShell") page to learn more about this process.

*Note:* The ![](/images/BIM_Setup.svg) [BIM Setup](/BIM_Setup "BIM Setup") tool will look for IfcOpenShell too, and issue a notification if it is not installed.

## Abrindo e Importando
...
IfcOpenShell supports all IFC2x3 and IFC4 entities (IFC4-add1 and IFC4-add2 are being implemented in v0.6 and might be available by the time you read this) but not all of them can be converted to [BIM](/BIM_Workbench "BIM Workbench") objects, those that can't will be imported as simple [Part](/Part_Workbench "Part Workbench") shapes. The IFC importer starts by importing all IFC entities derived from [IfcProduct](http://standards.buildingsmart.org/IFC/RELEASE/IFC2x3/TC1/HTML/ifckernel/lexical/ifcproduct.htm), that is, basically, all the objects that compose a building, such as walls or windows or pipes. All other entities needed by one of these objects, such as profiles of extrusion, or components of boolean operations, will be imported as required.
...
**Note**: The BIM Workbench features an [IFC explorer](/BIM_IfcExplorer "BIM IfcExplorer") tool that allows you to open an IFC file in fast, text-only mode, and import only the parts you wish.
...
O importador legado pode ser usado em Python:
...
Exporting to IFC files will export all the selected objects and their descendants. All Arch/BIM objects are supported, as well as other objects created in other workbenches. The only not fully supported objects, at the moment, are ![](/images/PartDesign_Body.svg) [PartDesign Bodies](/PartDesign_Body "PartDesign Body"), ![](/images/Std_Part.svg) [Std Parts](/Std_Part "Std Part"), and new structures such as ![](/images/Link.svg) [App Links](/Std_LinkMake "Std LinkMake") and ![](/images/LinkGroup.svg) LinkGroups, so you will need a bit of testing if using them. [Arch References](/Arch_Reference "Arch Reference") will currently export as `IfcBuildingElementProxies`.
...
* [IfcOpenShell](/IfcOpenShell "IfcOpenShell"), more information on installing this library.

Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_IFC/pt-br&oldid=1545527>"