...
* You can add a terrain object by editing the Site's Dados**Terrain** property. The terrain can be an open shell or ([introduced in 0.21](/Release_notes_0.21 "Release notes 0.21")) a solid.
...
* Dados**Addition Volume** (`Volume`): The volume of earth to be added to this terrain.
* Dados**Additions** (`LinkList`): Other shapes that are appended to this object.
* Dados**Address** (`String`): The street and house number of this site.
* Dados**City** (`String`): The city of this site.
* Dados**Country** (`String`): The country of this site.
* Dados**Declination** (`Angle`): The angle between the true North and the North direction in this document, that is, the Y axis. This means that by default North points to the Y axis, and East to the X axis; the angle increments counterclockwise. This property was previously known as Dados**North Deviation**.
* Dados**EPW File** (`FileIncluded`): Allow to attach an EPW file from the [Ladybug EPW data website](https://www.ladybug.tools/epwmap/) to this site. This is needed to display wind rose diagrams.
* Dados**Elevation** (`Length`): The elevation of level 0 of this site.
* Dados**Extrusion Vector** (`Vector`): An extrusion vector to use when performing boolean operations.
* Dados**Latitude** (`Float`): The latitude of this site.
* Dados**Longitude** (`Float`): The longitude of this site.
* Dados**Origin Offset** (`Vector`): An optional offset between the model (0,0,0) origin and the point indicated by the geo-coordinates.
* Dados**Perimeter** (`Length`): The perimeter length of the projected area.
* Dados**Postal Code** (`String`): The postal or zip code of this site.
* Dados**Projected Area** (`Area`): The area of the projection of this object onto the XY plane
* Dados**Region** (`String`): The region, province or county of this site.
* Dados**Remove Splitter** (`Bool`): Remove splitters from the resulting shape.
* Dados**Subtraction Volume** (`Volume`): The volume of earth to be removed from this terrain.
* Dados**Subtractions** (`LinkList`): Other shapes that are subtracted from this object.
* Dados**Terrain** (`Link`): The base terrain of this site
* Dados**Time Zone** (`Integer`): Represents the time zone of the site's location. Since this value is an integer only standard time zones are allowed. If the Vista**Solar Diagram** property is set to `true`, and the right module is installed, this value will be used to generate more accurate [solar and wind diagrams](#Solar_and_wind_diagrams). The time zone should be an offset between -12 and +14, where 0 is at Greenwich, UK, (GMT+0). Positive values are to the East of Greenwich and negative values are to the West.
* Dados**Url** (`String`): An url that shows this site in a mapping website.
...
Start by creating an object that represents your terrain. For example, it is easy to import mesh data, that can be turned into a Part Shape from menu **Part → Create Shape from Mesh**. Then, create a Site object, and set its Dados**Terrain** property to the Part we just created:
...
If [Ladybug](https://www.ladybug.tools/ladybug.html) is installed on your system, [Arch Sites](/Arch_Site "Arch Site") can display a sun path diagram and/or a wind rose. For this, Dados**Longitude**, Dados**Latitude** and Dados**Declination** (previously Dados**North Deviation**) must be correctly set, and Vista**Solar Diagram** or Vista**Wind Rose** set to `true`.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Site/pt-br&oldid=1561143>"