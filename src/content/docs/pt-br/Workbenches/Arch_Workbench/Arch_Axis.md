...
* Dados**Length**: The length of the axes
* Dados**Limit**: If greater than zero, each axis will be represented as two lines of the given length instead of one continuous line [introduced in 0.20](/Release_notes_0.20 "Release notes 0.20")
* Vista**Bubble Size**: The size of the axis bubbles
* Vista**Numeration style**: How the axes are numbered: 1,2,3, A,B,C, etc...
* Vista**Bubble Position**: Where the bubble is placed on the axis: At start point, endpoint, both or none.
* Vista**Font Name**: A font to draw the bubble number and/or labels
* Vista**Font Size**: The size of the label text only (bubble text is controlled by the bubble size)
* Vista**Show Labels**: Turns the display of the label texts on/off
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Axis/pt-br&oldid=1432758>"