...
* When used as the Dados**Axis** property of other Arch objects, the grid will drive the positioning of these objects. The Dados**Points Output** property defines how the other objects are placed on the grid: At vertices, edge midpoints or face centers.
* By setting the Dados**Auto Height** or Dados**Auto Width** properties to a non-zero value, the total number of rows/columns and their individual heights/widths is ignored. Instead, the maximum number of columns or rows of the given auto width/height get automatically created.

## Propriedades

* Dados**Rows**: The number of rows
* Dados**Columns**: The number of columns
* Dados**Row Size**: The sizes for rows
* Dados**Column Size**: The sizes of columns
* Dados**Points Output**: The type of 3D points produced by this grid object
* Dados**Width**: The total width of this grid
* Dados**Height**: The total height of this grid
* Dados**Auto Width**: Creates automatic column divisions (set to 0 to disable)
* Dados**Auto Height**: Creates automatic row divisions (set to 0 to disable)
* Dados**Reorient**: When in edge midpoint mode, if this grid must reorient its children along edge normals or not
* Dados**Hidden Faces**: The indices of faces to hide
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Grid/pt-br&oldid=1435505>"