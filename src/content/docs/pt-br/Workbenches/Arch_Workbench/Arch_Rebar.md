...
Although normally a rebar is used inside an Arch Structure, since FreeCAD 0.19 the rebar can be created outside of any host object. To host a rebar inside an object, you just need to set its Dados**Host**.
...
* Dados**Amount**: The amount of bars.
* Dados**Diameter**: The diameter of the bars.
* Dados**Direction**: The direction (and length) along which the bars must spread. If the value is (0,0,0), the direction is calculated automatically from the host structural object.
* Dados**Offset Start**: The offset distance between the border of the structural object and the first bar.
* Dados**Offset End**: The offset distance between the border of the structural object and the last bar.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Rebar/pt-br&oldid=1539724>"