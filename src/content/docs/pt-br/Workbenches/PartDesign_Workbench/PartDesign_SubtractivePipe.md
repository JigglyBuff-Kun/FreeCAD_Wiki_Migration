...
* Dados**Spine Tangent**: true or false (default). True extends the path to include tangent edges.
* Dados**Auxiliary Spine Tangent**: true or false (default). True extends the auxiliary path to include tangent edges.
* Dados**Auxiliary Curvelinear**: true or false (default). True calculates normal between equidistant points on both spines.
* Dados**Mode**: profile mode. See [Options](#Options).
* Dados**Binormal**: binormal vector for corresponding orientation mode.
* Dados**Transition**: transition mode. Options are *Transformed*, *Right Corner* or *Round Corner*.
* Dados**Transformation**: *Constant* uses a single cross-section. *Multisection* uses two or more cross-sections. *Linear*, *S-shape* and *Interpolation* are currently not functional.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=PartDesign_SubtractivePipe/pt-br&oldid=1364426>"