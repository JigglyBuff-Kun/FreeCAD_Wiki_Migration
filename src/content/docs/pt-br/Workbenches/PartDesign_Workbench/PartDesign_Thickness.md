...
* Dados**Value** (`Length`): Thickness value. Default: `1 mm`.
* Dados**Mode** (`Enumeration`): Mode. `Skin` (default), `Pipe` or `Recto verso`. Only `Skin` is implemented.
* Dados**Join** (`Enumeration`): Join type. `Arc` (default) or `Intersection`.
* Dados**Reversed** (`Bool`): Apply the thickness towards the solids interior. Default: `false`.
* Dados**Intersection** (`Bool`): Enable intersection-handling. Default: `false`.

Retrieved from "<http://wiki.freecad.org/index.php?title=PartDesign_Thickness/pt-br&oldid=1485220>"