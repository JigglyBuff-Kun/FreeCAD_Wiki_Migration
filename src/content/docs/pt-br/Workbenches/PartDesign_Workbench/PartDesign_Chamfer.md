...
* Dados**Base** (`LinkSub`): Link to the selected edges and faces of the parent feature. Can be a link to only the parent feature if Dados**Use All Edges** is `true`.
* Dados**Support Transform** (`Bool`): If `true` the chamfered shape of the additive/subtractive parent feature will be used when the chamfer object is included in a [pattern](/PartDesign_Workbench#Transformation_tools "PartDesign Workbench"), else only the shape of the chamfer itself will be used. The default is `false`.
...
* Dados**Chamfer Type** (`Enumeration`): The chamfer type: `Equal distance` (default), `Two distances` or `Distance and Angle`.
* Dados**Size** (`QuantityConstraint`): The first chamfer distance. The default is `1 mm`.
* Dados**Size2** (`QuantityConstraint`): The second chamfer distance. Only used if Dados**Chamfer Type** is `Two distances`. The default is `1 mm`.
* Dados**Angle** (`Angle`): The chamfer angle. Only used if Dados**Chamfer Type** is `Distance and Angle`. The default is `45 °`.
* Dados**Flip Direction** (`Bool`): If `true` the direction of the chamfer is flipped. Not used if Dados**Chamfer Type** is `Equal distance`. The default is `false`.
* Dados**Use All Edges** (`Bool`): If `true` all edges of the feature are chamfered, and the edges specified by Dados**Base** are ignored. The default is `false`.

Part Design

* Dados**Refine** (`Bool`): If `true` redundant edges are removed from the result of the operation. The default value is determined by the **Automatically refine model after sketch-based operation** preference. See [PartDesign Preferences](/PartDesign_Preferences#General "PartDesign Preferences").

## Known issues

See [PartDesign Fillet](/PartDesign_Fillet#Known_issues "PartDesign Fillet").

Retrieved from "<http://wiki.freecad.org/index.php?title=PartDesign_Chamfer/pt-br&oldid=1515557>"