...
3. Optionally change the dimensions and Dados**Placement** of the torus by doing one of the following:
...
* Dados**Radius1** (`Length`): The radius of the circular path of the torus. The default is `10mm`.
* Dados**Radius2** (`Length`): The radius of the circular profile of the torus. The default is `2mm`.
* Dados**Angle1** (`Angle`): The start angle of the circular profile. Valid range: `-180° <= value <= 180°`. The default is `-180°`.
* Dados**Angle2** (`Angle`): The end angle the circular profile. Valid range: `-180° <= value <= 180°`. The default is `180°`. If the total angle of the circular profile is smaller than `360°` the profile will have a pie-shape.
* Dados**Angle3** (`Angle`): The angle of the circular path of the torus. Valid range: `0° < value <= 360°`. The default is `360°`. If it is smaller than `360°` the resulting solid will be a segment of a torus.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Part_Torus/pt-br&oldid=1466575>"