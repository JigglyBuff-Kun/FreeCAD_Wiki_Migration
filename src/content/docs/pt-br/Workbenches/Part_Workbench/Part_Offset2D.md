...
* Dados**Source**: Link to original shape

* Dados**Value**: The distance to enlarge the wire/face by. If negative, the wire/face is shrunk instead.

* Dados**Mode** ("Pipe" or "Skin"): sets how non-closed wires are processed. If "Pipe", the wire is outlined as if it was an extremely thin closed contour. If "Skin", an open wire is created.

:   ![](/images/Part_Offset2D_Mode.png)

* Dados**Join** ("Arc", "Tangent", "Intersection"): sets the behavior around kinks. If "Arc", offset segments are connected with an arc of circle, centered at the vertex. "Tangent" is unsupported on OCC7.0.0. "Intersection": offset segments are extended till they intersect.

:   ![](/images/Part_Offset2D_Join.png)

* Dados**Intersection** ("false", "true"): sets if multiple wires are treated collectively or independently. If "false", wires are offset independently, intersections between resulting wires are ignored. If "true", the wires are offset in collective manner.
...
* Dados**Fill** ("false", "true"): if "true", the space between original wire/face and the offset is filled with a face.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Part_Offset2D/pt-br&oldid=1145933>"