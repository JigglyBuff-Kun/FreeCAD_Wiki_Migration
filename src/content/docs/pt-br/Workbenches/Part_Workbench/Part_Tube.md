...
The ![](/images/Part_Tube.svg) **Part Tube** command creates a parametric tube solid. In the coordinate system defined by its Dados**Placement** property, the bottom face of the tube lies on the XY plane with its center at the origin.
...
7. Optionally change the Dados**Placement** of the tube in the [Property editor](/Property_editor "Property editor"), or with the ![](/images/Std_TransformManip.svg) [Std TransformManip](/Std_TransformManip "Std TransformManip") command.
...
* Dados**Height** (`Length`): The height of the tube. The default is `10mm`.
* Dados**Inner Radius** (`Length`): The inner radius of the tube. Must be smaller than Dados**Outer Radius**. Can be `0`. The default is `2mm`.
* Dados**Outer Radius** (`Length`): The outer radius of the tube. Must be larger than Dados**Inner Radius**. The default is `5mm`.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Part_Tube/pt-br&oldid=1466593>"