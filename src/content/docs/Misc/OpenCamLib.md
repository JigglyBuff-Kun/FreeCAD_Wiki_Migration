...
OpenCamLib (OCL) is an open source library aiming to provide computer aided manufacturing (CAM) algorithms. FreeCAD uses OCL in the experimental ![](/src/assets/images/Path_Surface.svg) [3D Surface](/Path_Surface "Path Surface") path operations and other experimental features.
...
2. Navigate to your OCL build _or_ binary folder
3. Copy the **ocl.pyd** library file
4. Proceed with one of the following four(4) options:
   - Navigate to your **FreeCAD\lib** folder, and paste the **ocl.pyd** file there. (_This is the preferred option._)
   ...
   - Navigate to your **%USERPROFILE%\AppData\Roaming\FreeCAD** folder. Create a new folder, **Mod**. Enter the **Mod** folder. Create a new **OCL** folder. Enter the **OCL** folder and paste the **ocl.pyd** file. (_This is the least preferred option._)
5. Restart FreeCAD
...
2. Then run the following code snippet inside FreeCAD's [Python console](/Python_console "Python console")
...
To test the build enter the following in the [Python console](/Python_console "Python console"):
...
Retrieved from "<http://wiki.freecad.org/index.php?title=OpenCamLib/en&oldid=1245964>"
