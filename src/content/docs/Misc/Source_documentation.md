...
The FreeCAD source code is commented to allow automatic programming documentation generation using [Doxygen](/Doxygen "Doxygen"), a popular source code documentation system. Doxygen can document both the C++ and Python parts of FreeCAD, resulting in HTML pages with hyperlinks to each documented function and class.
...
![](/src/assets/images/FreeCAD_documentation_compilation_workflow.svg)
...
As mentioned in [compiling (speeding up)](</Compiling_(Speeding_up)> "Compiling (Speeding up)"), the `-j` option sets the number of CPU cores used for compilation. The resulting documentation files will appear in the directory
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Source_documentation/en&oldid=1072414>"
