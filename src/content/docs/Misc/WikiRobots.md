...
* replace *cp437* with your *console\_encoding*. To find out what is your console encoding, for Windows and Linux, launch the Python interpreter, enter import sys followed by print sys.stdout.encoding. Python will write your console\_encoding on the screen.

Then you must save the following Python code as a file with the name freecadwiki\_family.py under the sub-directory /pywikibot/families (together with the other family\_xxx.py files).
...
Retrieved from "<http://wiki.freecad.org/index.php?title=WikiRobots/en&oldid=1431289>"