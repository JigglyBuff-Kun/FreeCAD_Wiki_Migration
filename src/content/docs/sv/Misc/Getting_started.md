...
Först av allt (om det inte redan är gjort), ladda ned och installera FreeCAD. Se [Nedladdningssidan](/Download/sv "Download/sv") för information om nuvarande versioner och updateringar. Det finns färdiga installationspaket för Windows (.msi), Ubuntu & Debian (.deb) openSUSE (.rpm) och Mac OSX.
...
## [Skript](/Scripting/sv "Scripting/sv")

Och slutligen, en av de kraftfullaste funktionerna i FreeCAD är [skript](/Scripting/sv "Scripting/sv") miljön. Från den integrerade pythonkonsolen (eller från något externt pythonskript), så får du åtkomst till nästan vilken del som helst av FreeCAD, skapa eller ändra geometri, ändra representationen av dessa objekt i 3D scenen eller förändra FreeCAD's gränssnitt. Python skript kan även användas i [makron](/Macros/sv "Macros/sv"), vilket erbjuder en lätt metod att skapa anpassade kommandon.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Getting_started/sv&oldid=1480483>"