...
| - |
| Se även |
| [Arch Floor](/Arch_Floor/sv "Arch Floor/sv"), [Arch Building](/Arch_Building/sv "Arch Building/sv") |
|  |
...
* Vy**Compass** (`Bool`): Default is `False`. Shows or hides the compass.
* Vy**Compass Position** (`Vector`): The position of the compass relative to the site placement.
* Vy**Compass Rotation** (`Angle`): The rotation of the compass relative to the site.
* Vy**Update Declination** (`Bool`): Default is `False`. Update the declination value based on the compass rotation.

Site

* Vy**Orientation** (`Enumeration`): Default is `Project North`. When set to `True North` the whole geometry will be rotated to match the true north of this site.
* Vy**Solar Diagram** (`Bool`): Default is `False`. Shows or hides the sun path diagram. See [Solar and wind diagrams](#Solar_and_wind_diagrams).
* Vy**Solar Diagram Color** (`Color`): The color of the sun path diagram.
* Vy**Solar Diagram Position** (`Vector`): The position of the sun path diagram.
* Vy**Solar Diagram Scale** (`Float`): The scale of the sun path diagram.
* Vy**Wind Rose** (`Bool`): Default is `False`. Shows or hides the wind rose diagram (requires the **EPW File** data property filled, and the Ladybug Python module installed. See [Solar and wind diagrams](#Solar_and_wind_diagrams)).
...
If [Ladybug](https://www.ladybug.tools/ladybug.html) is installed on your system, [Arch Sites](/Arch_Site "Arch Site") can display a sun path diagram and/or a wind rose. For this, Data**Longitude**, Data**Latitude** and Data**Declination** (previously Data**North Deviation**) must be correctly set, and Vy**Solar Diagram** or Vy**Wind Rose** set to `true`.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Site/sv&oldid=1561144>"