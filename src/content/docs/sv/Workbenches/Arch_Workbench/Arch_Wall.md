...
| - |
| Se även |
| [Arch Structure](/Arch_Structure/sv "Arch Structure/sv") |
|  |
...
* Walls can be made to display blocks, instead of one single solid, by turning their Data**Make Blocks** property on. The size and offset of blocks can be configured with different properties, and the amount of blocks is automatically calculated.
...
* Data**Block Height**: The height of each block.
* Data**Block Length**: The length of each block.
* Data**Count Broken**: The number of broken blocks (read-only).
* Data**Count Entire**: The number of entire blocks (read-only).
* Data**Joint**: The size of the joints, the empty space, between blocks.
* Data**Make Blocks**: Enables block generation.
* Data**Offset First**: The horizontal offset of the first and every uneven line of blocks.
* Data**Offset Second**: The horizontal offset of the second and every even line of blocks.
...
* Data**Face**: The index of the face from the base object to use. If the value is not set or 0, the whole object is used.
* Data**Height**: The height of the wall. Ignored if the wall is based on a solid. If set to zero, and the wall is inside a [floor](/Arch_Floor "Arch Floor") object with its height defined, the wall will automatically take the value of the floor height.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Wall/sv&oldid=1548509>"