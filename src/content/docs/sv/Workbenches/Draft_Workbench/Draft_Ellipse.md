...
* Vy**Pattern** (`Enumeration`): specifies the [Draft Pattern](/Draft_Pattern "Draft Pattern") with which to fill the face of the ellipse. This property only works if Data**Make Face** is `true` and if Vy**Display Mode** is `Flat Lines`.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Draft_Ellipse/sv&oldid=1556235>"