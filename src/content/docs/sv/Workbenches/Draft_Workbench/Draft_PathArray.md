...
| - |
| Se även |
| [Draft Array](/Draft_Array/sv "Draft Array/sv") |
|  |
...
* Vy**Draw Style** (`Enumeration`)
* Vy**Line Width** (`FloatConstraint`)
* Vy**Override Material** (`Bool`)
* Vy**Point Size** (`FloatConstraint`)
* Vy**Selectable** (`Bool`): this is an inherited property that appears in the Selection group for other arrays.
* Vy**Shape Material** (`Material`)

Base

The properties in this group, with the exception of the inherited property, are only available for Link arrays. See [Std LinkMake](/Std_LinkMake#Properties "Std LinkMake") for more information.

* Vy (Hidden)**Child View Provider** (`PersistentObject`)
* Vy (Hidden)**Material List** (`MaterialList`)
* Vy (Hidden)**Override Color List** (`ColorList`)
* Vy (Hidden)**Override Material List** (`BoolList`)
* Vy (Hidden)**Proxy** (`PythonObject`): this is an inherited property.
...
* Vy**Bounding Box** (`Bool`): this property is not inherited by Link arrays.
* Vy**Display Mode** (`Enumeration`): for Link arrays it can be `Link` or `ChildView`. For other arrays it can be: `Flat Lines`, `Shaded`, `Wireframe` or `Points`.
* Vy**Show In Tree** (`Bool`)
* Vy**Visibility** (`Bool`)
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Draft_PathArray/sv&oldid=1557836>"