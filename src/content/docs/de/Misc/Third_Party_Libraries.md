...
Die Abhängigkeiten müssen im System installiert werden, bevor Du mit der Kompilierung fortfahren kannst; siehe [Kompilieren unter Linux](/Compile_on_Linux/de "Compile on Linux/de"), [Kompilieren unter Windows](/Compile_on_Windows/de "Compile on Windows/de"), und [Kompilieren unter MacOS](/Compile_on_MacOS/de "Compile on MacOS/de") für weitere Informationen.

Wenn du unter Windows kompilierst, ziehe es in Betracht, die [LibPack](#LibPack/de) zu verwenden, anstatt zu versuchen, die Bibliotheken einzeln zu installieren.
...
* zum Schreiben von [Makros](/Macros/de "Macros/de") und zur Makroaufnahme,
* zur Implementierung von Anwendungslogik für Standardpakete,
* um Hilfswerkzeuge wie den [Addon-Manager](/Std_AddonMgr/de "Std AddonMgr/de") zu implementieren,
* um ganze Arbeitsbereiche wie [Draft](/Draft_Workbench/de "Draft Workbench/de") und [BIM](/BIM_Workbench/de "BIM Workbench/de") zu implementieren,
...
Coin3D ist über eine Vielzahl von Plattformen anwendbar: UNIX, Linux, BSD, macOS und Microsoft Windows Betriebssysteme. Um mehr über diese Bibliothek zu lesen, besuche [Coin3D homepage](https://github.com/coin3d/coin).
...
[Pivy](/Pivy/de "Pivy/de") ist eine Bibliothek, die die Coin3d-Bibliothek zur Verwendung in [Python](/Python/de "Python/de") umhüllt. Sie wird nicht benötigt, um FreeCAD zu bauen oder zu starten, aber sie wird als Laufzeit-Abhängigkeit vom Arbeitsbereich [Draft](/Draft_Workbench/de "Draft Workbench/de") und von anderen Arbeitsbereichen, die sie intern verwenden, wie [BIM](/BIM_Workbench/de "BIM Workbench/de"), benötigt.
...
Ply ist der Python-Lex-Yacc Parser. Er wird als Laufzeitabhängigkeit von der [OpenSCAD Arbeitsbereich](/OpenSCAD_Workbench/de "OpenSCAD Workbench/de") verwendet. Wenn du diesen Arbeitsbereich nicht verwendest, benötigst du dieses Paket möglicherweise nicht.
...
Eine Kopie der Bibliothek ist im Quellcode des Arbeitsbereichs [CAM](/CAM_Workbench "CAM Workbench") enthalten, so dass sie zusammen mit diesem kompiliert wird.
...
Wenn du unter Linux arbeitest, brauchst du das LibPack nicht, da du die Abhängigkeiten aus den Repositorien deiner Distribution beziehen kannst, wie auf der [Kompilieren unter Linux](/Compile_on_Linux/de "Compile on Linux/de")-Seite erwähnt.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Third_Party_Libraries/de&oldid=1462913>"