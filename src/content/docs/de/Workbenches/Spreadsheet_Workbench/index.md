...
- Einheiten: Anzeigeeinheiten für diese Zelle. Den Abschnitt [Einheiten](#Einheiten) unten beachten.
- Alias: Einen [Alias](/Spreadsheet_SetAlias "Spreadsheet SetAlias") für diese Zelle festlegen. Dieser Alias kann in Zellformeln und auch in allgemeinen [Ausdrücken](/Expressions/de "Expressions/de") verwendet werden; siehe Abschnitt [Tabellendaten in Ausdrücken](#Tabellendaten_in_Ausdrücken) für weitere Informationen.
...
(Siehe aber [Aktuelle Einschränkungen](#Aktuelle_Einschränkungen) weiter unten).
Eine Zelle kann über ihre Adresse (GROSSBUCHSTABEN für die Spalte + Zeilennummer, z.B. B4) referenziert werden oder über ihren [Alias](/Spreadsheet_SetAlias/de "Spreadsheet SetAlias/de").
...
- Die Anzahl der angezeigten Dezimalstellen kann von denen in den [Voreinstellungen](/Preferences_Editor/de#Einheiten "Preferences Editor/de") abweichen.

Referenzen zu Objekten im Modell werden unter [Referenzen auf CAD-Daten](#References_to_CAD-data/de) unten erklärt.
Die Verwendung von Tabellenkalkulationszellenwerten zur Definition von Modelleigenschaften wird unter [Tabellenkalkulationsdaten in Ausdrücken](#Spreadsheet_data_in_Expressions/de) weiter unten erläutert.
Weitere Besonderheiten zur Bildung von Ausdrücken findest du unter [Ausdrücke](/Expressions/de "Expressions/de").
...
3. Lege das Tabellenblatt in eine separate [FreeCAD Datei](/File_Format_FCStd/de "File Format FCStd/de"):
...
FreeCAD Tabellenblätter können im [[1]](https://de.wikipedia.org/wiki/CSV_(Dateiformat)) Format importiert und exportiert werden, welches auch von den meisten anderen Tabellenkalkulationsanwendungen wie Microsoft Excel oder LibreOffice Calc gelesen und geschrieben werden kann. Siehe [Tabellenblatt Import](/Spreadsheet_Import/de "Spreadsheet Import/de") und [Tabellenblatt Export](/Spreadsheet_Export/de "Spreadsheet Export/de") für weitere Informationen.

### XLSX-Format

Tabellenblätter im Excel-Format XLSX können mit dem Befehl [Std Import](/Std_Import/de "Std Import/de") oder dem Befehl [Std Öffnen](/Std_Open/de "Std Open/de") importiert werden. Die folgenden Funktionen werden unterstützt:
...
Um die erforderlichen Seiteneinstellungen für den Druck von FreeCAD-Tabellen einzurichten, werden diese in eine [TechDraw Tabellenansicht](/TechDraw_SpreadsheetView/de "TechDraw SpreadsheetView/de") eingefügt.
...
1. Rechts-Klick in eine gebundene Zelle (es ist nicht notwendig den gesamten Zellbereich auszuwählen) und wähle **Bind...** aus dem Kontext-Menü.
2. Der **Bind Spreadsheet Cells** Dialog öffnet sich.
3. Ändere eine oder mehrere Optionen. Beachte dass **Bind cells**, der gebundene Zell-Bereich, nicht geändert werden kann.
4. Drücke OK.

### Bindung entfernen

1. Rechts-Klick auf eine gebundene Zelle (es muss nicht der gesamte Zell-Bereich ausgewählt werden) und wähle **Bind...** aus dem Kontext-Menü.
2. Der **Bind Spreadsheet Cells** Dialog öffnet sich.
3. Drücke Unbind.

### Hinweise

- Die Option **Abhängigkeit der Bindung ausblenden** kann verwendet werden, um Probleme mit zyklischen Abhängigkeiten zwischen Tabellenblättern zu vermeiden. Die Auswahl dieser Option ist notwendig, wenn z.B. Zellen in _Arbeitsblatt A_ an _Arbeitsblatt B_ gebunden sind, während Zellen in _Arbeitsblatt B_ wiederum an einige andere Zellen in _Arbeitsblatt A_ gebunden sind. Diese Option sollte mit Vorsicht verwendet werden:
...
Sie können Tabellenkalkulationen verwenden, um Konfigurationstabellen mit vordefinierten Parametersätzen für Ihr Modell zu erstellen und dann dynamisch zu ändern, welche Konfiguration verwendet werden soll. Siehe das [Konfigurationstabellen](/index.php?title=Konfigurationstabellen&action=edit&redlink=1 "Konfigurationstabellen (page does not exist)")-Tutorial. Lesen Sie [this Forum post](https://forum.freecadweb.org/viewtopic.php?f=17&t=42183), wenn Sie mehr über die Funktionsweise dieser Funktion erfahren möchten.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Spreadsheet_Workbench/de&oldid=1535011>"
