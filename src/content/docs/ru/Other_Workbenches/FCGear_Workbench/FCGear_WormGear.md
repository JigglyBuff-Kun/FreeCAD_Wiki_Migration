...
* Данные**diameter** (`Length`): Default is `5 mm`. Pitch diameter.
* Данные**height** (`Length`): Default is `5 mm`. Value of the worm length.
...
* Данные**reverse\_pitch** (`Bool`): Default is `false`, `true` changes the rotating direction from right to left.
* Данные**teeth** (`Integer`): Default is `3`. Number of teeth (see [Notes](#Notes)).

computed

* Данные**beta** (`Angle`): (read-only) Lead angle (see also the information in [Notes](#Notes) and [Useful formulas](#Useful_formulas)).
...
Retrieved from "<http://wiki.freecad.org/index.php?title=FCGear_WormGear/ru&oldid=1195237>"