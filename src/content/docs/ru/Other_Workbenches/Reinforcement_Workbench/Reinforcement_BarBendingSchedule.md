---
title: Армирование. Список форм для гибки арматуры
---
|  |
| --- |
...
* Данные**Reinforcement Group By**: Reinforcement Group By specifies how reinforcement objects should be grouped in Bar Bending Schedule i.e. "Host" or "Mark".
* Данные**Rebar Length Type**: Rebar Length Type specifies the type of rebar length used for BOM calculations i.e. "RealLength" or "LengthWithSharpEdges".
* Данные**Column Headers**: A dictionary with column\_data as key and tuple (column\_display\_header, column\_sequence) as value.
* Данные**Column Units**: A dictionary with keys: "Diameter", "RebarLength", "RebarsTotalLength" and their corresponding units as value.
* Данные**Font Family**: Font Family of text in Bar Bending Schedule SVG.
* Данные**Font Size**: Font size in mm.
* Данные**Column Width**: Width of each column in Bar Bending Schedule SVG.
* Данные**Row Height**: Height of each row in Bar Bending Schedule SVG.
* Данные**SVG Output File**: The output file to write generated rebar shape cut list SVG.

**Rebar Shape column data:** The data related to the Rebar Shape column in Bar Bending Schedule

* Данные**Column Header**: The column header for the rebar shape column.
* Данные**Stirrup Extended Edge Offset**: The offset of extended end edges of the stirrup, so that end edges of the stirrup with a 90-degree bent angle do not overlap with stirrup edges.
* Данные**Rebars Stroke Width**: The stroke-width of rebars in the rebar shape column.
* Данные**Rebars Color Style**: The color style of rebars.

**Rebar Shape Column Dimension Data:** The data related to rebar shape dimensions in the Rebar Shape column

* Данные**Include Dimensions**: If True, then each rebar edge dimensions and bent angle dimensions will be included in the rebar shape cut list.
* Данные**Include Units in Dimension Label**: If it is True, then rebar edge length units will be shown in dimension label.
* Данные**Rebar Edge Dimension Units**: The units to be used for rebar edge length dimensions.
* Данные**Rebar Edge Dimension Precision**: The number of decimals that should be shown for rebar edge length as a dimension label.
* Данные**Dimension Font Family**: The font-family of dimension text.
* Данные**Dimension Font Size**: The font-size of dimension text.
* Данные**Bent Angle Dimension Exclude List**: The list of bent angles to not include their dimensions.
* Данные**Helical Rebar Dimension Label Format**: The format of the helical rebar dimension label. e.g. "%L,r=%R,pitch=%P" where %L -> Length of helical rebar, %R -> Helix radius of helical rebar, %P -> Helix pitch of helical rebar.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Reinforcement_BarBendingSchedule/ru&oldid=1372705>"