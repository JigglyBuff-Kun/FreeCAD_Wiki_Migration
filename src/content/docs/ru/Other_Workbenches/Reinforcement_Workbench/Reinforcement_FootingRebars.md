---
title: Arch Армировать фундамент
---
|  |
| --- |
| Армировать фундамент |
| Расположение в меню |
| Arch → Rebar tools → Footing Reinforcement 3D/BIM → Reinforcement tools → Footing Reinforcement |
| Верстаки |
...
| - |
| См. также |
| [Reinforcement](/Reinforcement_Workbench/ru "Reinforcement Workbench/ru"), [Арматура по эскизу](/Arch_Rebar/ru "Arch Rebar/ru") |
|  |

## Описание

Инструмент [Армирование фундамента](/Arch_Rebar_Footing_Reinforcement/ru "Arch Rebar Footing Reinforcement/ru") позволяет пользователю армировать фундамент таких объектов как [Структуры](/Arch_Structure/ru "Arch Structure/ru").
...
* Данные**Mesh Cover Along**: It represent alignment of rebar mesh along top and/or bottom face of structure. It can have three values "Top", "Bottom" and "Both".
* Данные**Rebar Type**: Type of rebar for parallel rebars for footing reinforcement. It can have three values 'StraightRebar', 'LShapeRebar' and 'UShapeRebar'.
...
* Данные**Rear Cover**: Rear cover for footing reinforcement of parallel rebars.
...
* Данные**Rebar Type**: Type of rebar for cross rebars for footing reinforcement. It can have three values 'StraightRebar', 'LShapeRebar' and 'UShapeRebar'.
* Данные**Front Cover**: The distance between cross rebar and cross\_face (face perpendicular to selected face).
...
* Данные**Rear Cover**: Rear cover for footing reinforcement of cross rebars.
...
* Данные**Front Cover**: Distance between selected face and columns.
* Данные**Left Cover**: Distance between left face and columns.
* Данные**Right Cover**: Distance between right face and columns.
* Данные**Rear Cover**: Distance between rear face and columns.
* Данные**Column Width**: Width of column.
* Данные**Column Length**: Length of column.
* Данные**X direction column amount**: It contains count of columns in x direction. If X direction amount radio button is enabled.
* Данные**X direction column spacing**: It contains spacing between columns in x direction. If X direction spacing radio button is enabled.
* Данные**Y direction column amount**: It contains count of columns in y direction. If Y direction amount radio button is enabled.
* Данные**Y direction column spacing**: It contains spacing between columns in y direction. If Y direction spacing radio button is enabled.
* Данные**Add Secondary Rebars**: If checked add secoundary x and y direction rebars in columns.

**Properties for Ties in Columns of footing Reinforcement:**

* Данные**Top Cover**: Top cover for ties outside footing from Main Rebars end.
* Данные**Bottom Cover** : Bottom cover of ties from Bottom of Main Rebars in footing near mesh.
* Данные**Diameter**: Diameter of ties.
* Данные**Bent Angle**: Bent angle for ties.
* Данные**Extension Factor**: Extension factor for ties extended edge.
* Данные**Tie Number**: It contains count of rebars or spacing between ties, if Number radio button is enabled.
* Данные**Tie Spacing**: It contains the spacing between ties, if Spacing radio button is enabled.

**Properties for Main Rebars in Columns of footing Reinforcement:**

* Данные**Rabar Type**: Rebar type for main rebars of column. It takes two different inputs for 'StraightRebar', 'LShapeRebar'.
* Данные**Hook Orientation**: Hook orientation of main rebars in columns if main rabar type is LShapeRebar. It takes eight different orientations input for L-shaped hooks i.e. 'Top Inside', 'Top Outside', 'Bottom Inside', 'Bottom Outside', 'Top Left', 'Top Right', 'Bottom Left', 'Bottom Right'.
* Данные**Hook Extend Along**: Direction of main rebar (LShapeRebar) hook. it has two option "x-axis" and "y-axis".
* Данные**Hook Extension**: It specifies length of hook of main rebar (LShapeRebar).
* Данные**Rebar Rounding**: A rounding value to be applied to the corners of the bars, expressed in times the Main Rebar Diameter.
* Данные**Top Offset**: Top offset of main rebars in column outside footing Top face.
* Данные**Diameter**: Diameter of main rebars in columns.

**Properties for X Direction Rebars in Columns of footing Reinforcement:**

Rebars along x-direction except main rebars

* Данные**Rebar Type**: Type of x-direction rebars in a column.It has two values, 'StraightRebar' and 'LShapeRebar'.
* Данные**Hook Orientation**: Orientation of L-Shaped rebar hooks.It takes eight different orientations input for L-shaped hooks i.e. 'Top Inside', 'Top Outside', 'Bottom Inside', 'Bottom Outside', 'Top Left', 'Top Right', 'Bottom Left', 'Bottom Right'.
* Данные**Hook Extension**: Length of hook of L-Shape rebars.
* Данные**Rounding**: A rounding value to be applied to the corners of the L-Shape rebars, expressed in times the diameter.
* Данные**Top Offset**: The distance between rebar from the top face of the structure.
* Данные**Number#Diameter**: Number#Diameter set of the x-direction rebars.

**Properties for Y Direction Rebars in Columns of footing Reinforcement:**

Rebars along y-direction except main rebars

* Данные**Rebar Type**: Type of y-direction rebars. It has two values, 'StraightRebar' and 'LShapeRebar'.
* Данные**Hook Orientation**: Orientation of LShaped hooks. It takes eight different orientations input for L-shaped hooks i.e. 'Top Inside', 'Top Outside', 'Bottom Inside', 'Bottom Outside', 'Top Left', 'Top Right', 'Bottom Left', 'Bottom Right'.
* Данные**Hook Extension**: Length of hook of L-Shape rebars.
* Данные**Rounding**: A rounding value to be applied to the corners of the L-Shape rebars, expressed in times the diameter.
* Данные**Top Offset**: The distance between rebar from the top face of the structure.
* Данные**Number#Diameter**: Number#Diameter set of the y-direction rebars.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Reinforcement_FootingRebars/ru&oldid=1441031>"