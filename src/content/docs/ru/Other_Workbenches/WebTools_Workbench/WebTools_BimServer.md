...
| Arch BimServer‏‎‏‎                             |
| Расположение в меню                            |
| Архитектура → Утилиты → BIM server             |
| Верстаки                                       |
...
- All the file transfers with the BIMServer are done with IFC files. Therefore, you need to know how to work with [IFC files](/Arch_IFC "Arch IFC").
...
- Choose the **Root object** you wish to upload. It must be either an [Arch Site](/Arch_Site "Arch Site") or an [Arch Building](/Arch_Building "Arch Building"). Only objects belonging to that root object will be uploaded.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=WebTools_BimServer/ru&oldid=1066941>"
