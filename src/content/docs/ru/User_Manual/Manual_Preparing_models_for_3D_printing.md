---
title: Подготовка для объёмной печати
---
...
- Убедитесь, что Ваш трёхмерных объект представляет собой **твёрдое тело**. Трёхмерные модели должны быть телами так же как объекты реального мира. Мы видели в предыдущих главах, что FreeCAD сильно помогает в этом вопросе, и что [верстак PartDesign](/PartDesign_Workbench/ru "PartDesign Workbench/ru") предупредит Вас, если производимая Вами операция помешает вашей модели оставаться твердотельной. [Верстак Part](/Part_Workbench/ru "Part Workbench/ru") так же содержит инструмент ![](/images/Part_CheckGeometry.svg) [Проверка геометрии](/Part_CheckGeometry/ru "Part CheckGeometry/ru"), полезный для дальнейших проверок возможных дефектов.
...
- Переключимся на [верстак Mesh](/Mesh_Workbench/ru "Mesh Workbench/ru")
...
- Загружаем файл, содержащий наш элемент Lego, и переключаемся на [верстак Path](/Path_Workbench/ru "Path Workbench/ru").
...
- Выделяем верхнюю поверхность и нажимаем кнопку ![](/images/Path_Profile.svg) [Profile](/Path_Profile/ru "Path Profile/ru").
...
- Затем дублируем первую петлю несколько раз, чтобы инструмент выпилил целый блок. Выберем трассу FaceProfile, и нажмём ![](/images/Path_Array.svg) [Array](/Path_Array/ru "Path Array/ru") button.
...
- Выделите верхнюю поверхность, и нажмите кнопку ![](/images/Path_Pocket_Shape.svg) [Face Pocket](/Path_Pocket_Shape/ru "Path Pocket Shape/ru"). Установите параметр **Offset** в 1 мм, а **retraction height** в 20 мм. Это высота, на которой фреза пройдёт при переходе с одной петли к другой. Без него фреза может прорезать прямо через одну из наших точек:

- Теперь всё, что осталось сделать, это объединить эти операции. Это можно сделать через [Path Compound](/index.php?title=Path_Compound/ru&action=edit&redlink=1 "Path Compound/ru (page does not exist)") или [Path Job](/Path_Job/ru "Path Job/ru"). Поскольку нам больше ничего не нужно и мы уже готовы к экспорту, мы используем Job. Нажмите кнопку ![](/images/Path_Job.svg) [Job](/Path_Job/ru "Path Job/ru").
...
- [Верстак Mesh](/Mesh_Workbench/ru "Mesh Workbench/ru")
...
- [Верстак Path](/Path_Workbench/ru "Path Workbench/ru")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Manual:Preparing_models_for_3D_printing/ru&oldid=1526269>"
