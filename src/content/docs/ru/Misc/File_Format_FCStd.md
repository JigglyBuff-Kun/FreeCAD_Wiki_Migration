...
Then in the [property editor](/Property_editor "Property editor") you can go to the added property and choose a file in the computer. Once the FCStd file is saved, the file assigned to the Данные**PropertyFileIncluded** property will be packed inside the `.FCStd`. When the document is restored, the same file will be restored with the Данные**PropertyFileIncluded** property.
...
* See [Std ProjectUtil](/Std_ProjectUtil "Std ProjectUtil").

Retrieved from "<http://wiki.freecad.org/index.php?title=File_Format_FCStd/ru&oldid=1409716>"