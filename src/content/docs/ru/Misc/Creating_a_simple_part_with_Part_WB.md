---
title: Создание простой детали в верстаке Part
---
|  |
| --- |
| Руководство |
...
|  |
| Смотрите также |
| [Создание простой детали в верстаке PartDesign](/Creating_a_simple_part_with_PartDesign/ru "Creating a simple part with PartDesign/ru"), [Создание простой детали в верстаках Draft и Part](/Creating_a_simple_part_with_Draft_and_Part_WB/ru "Creating a simple part with Draft and Part WB/ru") |
|  |

## Введение

Это руководство является введением в 3D моделирование с использованием верстака [Part](/Part_Workbench/ru "Part Workbench/ru") ![](/images/Switch_PartWorkbench.JPG) в системе FreeCAD. После его прочтения вы сможете создавать простые 3D модели с использованием таких примитивов как кубы, цилиндры, и т.д. с применением техники [Конструктивная сплошная геометрия](https://ru.wikipedia.org/wiki/%D0%9A%D0%BE%D0%BD%D1%81%D1%82%D1%80%D1%83%D0%BA%D1%82%D0%B8%D0%B2%D0%BD%D0%B0%D1%8F_%D1%81%D0%BF%D0%BB%D0%BE%D1%88%D0%BD%D0%B0%D1%8F_%D0%B3%D0%B5%D0%BE%D0%BC%D0%B5%D1%82%D1%80%D0%B8%D1%8F). Другим способом создать 3D модели можно с использованием двумерных фигур путем вращения и движения этих фигур в 3D пространстве. Подробнее об этой технике читайте, пожалуйста, в руководстве *[Создание простой детали в PartDesign](/Creating_a_simple_part_with_PartDesign/ru "Creating a simple part with PartDesign/ru")*. В этих двух руководствах специально рассматривается одна и та же модель для того, чтобы начинающие на собственном опыте изучили две разные техники их создания и то, как они реализованы в системе FreeCAD. Эти техники различаются концептуально, но при создании моделей их можно использовать одновременно. Есть некоторые ограничения при одновременном использовании двух техник моделирования, но они касаются встроенного языка программирования FreeCAD. Существует руководство [Создание простой детали в верстаках Draft и Part](/Creating_a_simple_part_with_Draft_and_Part_WB/ru "Creating a simple part with Draft and Part WB/ru"), которое является примером использования двух техник моделирования одновременно. Оно использует верстак **Draft** и 2D фигуры, с помощью которых затем в верстаке **Part** создаётся 3D модель такая же, как в этом руководстве.

Перед тем как мы начнем, пожалуйста, посмотрите, как осуществлять **[навигацию](/Mouse_navigation/ru "Mouse navigation/ru")** в 3D пространстве. При наведении указателя мыши на иконку с изображением мыши в нижнем правом углу окна FreeCAD, появляется подсказка как на картинке ниже.
...
Создайте новый документ и сразу же сохраните его под новым именем. Возьмите хорошую привычку регулярно сохранять документ, или сохраняйте его хотя бы перед большими изменениями. Затем включите верстак **[Part](/Part_Workbench/ru "Part Workbench/ru")** используя либо [переключатель верстаков](/Getting_started/ru#Exploring_the_interface "Getting started/ru") (помечен числом 10 на картинке по ссылке), либо через главное меню **Вид → Верстак**. FreeCAD запустится с панелями инструментов вверху, комбинированным видом слева и 3D видом справа.

### Создание основного твердого тела

Кликните мышкой на иконке ![](/images/Part_Box.svg) [Создать твердотельный куб](/Part_Box/ru "Part Box/ru") чтобы создать исходный куб. Он появится в окне [3D вид](/3D_view/ru "3D view/ru") и в виде нового объекта в [Дереве проекта](/Tree_view/ru "Tree view/ru") на боковой панели.

Нажмите ![](/images/Std_ViewIsometric.svg) [Установить изометрический вид](/Std_ViewIsometric/ru "Std ViewIsometric/ru") чтобы видеть куб в 3D пространстве.

![](/images/T101pwb01-03_cube1.png)

Выберите куб в [Дереве проекта](/Tree_view/ru "Tree view/ru"), в окне 3D вида он окрасится в зелёный цвет. Ниже в дереве проекта вы теперь сможете увидеть, что по умолчанию куб создан с размерами **Длина x Ширина x Высота** в *10 x 10 x 10 mm*. Измените эти размеры на **100 x 30 x 50**, это будут исходные размеры модели.
...
Кликните мышкой на иконке ![](/images/Std_ViewFitAll.svg) **[Уместить все содержимое на экране](/Std_ViewFitAll/ru "Std ViewFitAll/ru")** чтобы поместить модель в центр окна 3D вида.

![](/images/T101pwb01-05_cube2.png)

### Создание скругления

Для того, чтобы создать скругление ребра модели, на панели инструментов выберите иконку ![](/images/Part_Fillet.svg) **[Скругление выбранного края формы](/Part_Fillet/ru "Part Fillet/ru")**, которая откроет *панель задач* для скруглений в [Комбо панели](/Combo_view/ru "Combo view/ru"). Указателем мыши выберите верхний правый угол модели и установите значение *Радиус* в 20 мм, и нажмите кнопку OK.
...
Теперь создадим 30-ти градусную фаску, предварительно сделав видимым дочерний объект *Куб* объекта *Скругление*. В верстаке [Part](/Part_Workbench/ru "Part Workbench/ru") имеется специальный инструмент для создания фасок, но мы сейчас сделаем фаску с помощью другого примитива и булевой операции обрезания.

Создайте новый объект ![](/images/Part_Box.svg) **[Твердотельный куб](/Part_Box/ru "Part Box/ru")** и дайте ему размеры 60 x 30 x 30. В свойстве **Placement** установите угол в -30 градусов.
...
Для проверки наших вычислений и верности расположения фаски можно использовать **[Измерительный инструмент](/Part_Workbench/ru#Measure "Part Workbench/ru")** верстака **Part**. Для этого нажмите на иконку ![](/images/Part_Measure_Linear.svg) **[Рулетка](/Part_Measure_Linear/ru "Part Measure Linear/ru")**, откроется *панель задач*. Затем мышкой на модели выберите два конца ребра фаски.
...
Чтобы создать отверстие нажмите на иконку ![](/images/Part_Cylinder.svg) **[Создать цилиндр](/Part_Cylinder/ru "Part Cylinder/ru")**, установите *радиус* в 5 мм и *высоту* в 50 мм.

![](/images/T101pwb01-16_cyl1.png)

Далее нам нужно разместить отверстие согласно размерам на чертеже. Установите ![](/images/Std_ViewTop.svg) **[Вид сверху](/Std_ViewTop/ru "Std ViewTop/ru")**, нажатием правой кнопкой мыши на объекте **Цилиндр** в дереве проекта откройте меню и выберите в нем строку **Преобразовать**.

![](/images/T101pwb01-17_cyl1translate.png)

Установите *Приращение перемещения* в 5 мм. Используя красную и зеленую стрелки переместите цилиндр на 15мм по оси y и на 65мм по оси x. Для этого нужно на стрелке нажать на левую кнопку мыши и не отпуская ее вести цилиндр к нужной позиции, затем кнопку отпустить. Нажмите кнопку OK и закройте диалог *Приращения*. Чтобы сделать отверстие, нажмите кнопку Ctrl, левой кнопкой мыши на дереве проекта выберите объекты **Cut** и **Цилиндр**, затем нажмите на иконку ![](/images/Part_Cut.svg) **[Обрезать две фигуры](/Part_Cut/ru "Part Cut/ru")** в панели инструментов. Теперь на вашем дереве проекта будет один объект **Cut001**.
...
К сожалению, когда объект активен , т.е. выделен, не видно, каким будет его внешний вид после изменений. Чтобы увидеть изменения нужно сделать его не активным. Установите изометрический вид, так отверстие в модели будет видно лучше. Другим способом изменить внешний вид можно через меню **Вид → ![](/images/Std_SetAppearance.svg) Внешний вид...**.

![](/images/T101pwb02-02_appearance2.png)

## Другой способ размещения отверстия

В меню **Файл → Сохранить как...** сохраните проект под новым именем. Удалите объект **Cut001** и переместите цилиндр в исходную позицию.
Ваша модель должна выглядеть как на картинке ниже. Рассмотрим различные варианты создания отверстия в центре верхней грани модели. Заметьте, что цвет модели опять стал серым и все изменения, которые мы сделали, исчезли.

![](/images/T101pwb03-01_cyl.png)

Для размещения отверстия теперь мы будем использовать верстак ![](/images/Workbench_Draft.svg) **[Чертеж](/Draft_Workbench/ru "Draft Workbench/ru")**. Оно по-прежнему должно быть расположено в центре верхней грани модели, т.е. посередине диагональных линий верхней грани.

Включите верстак **Draft**, в 3D пространстве должна появится *сетка*. Она включается и выключается кнопкой ![](/images/Draft_ToggleGrid.svg) [Toggle Grid](/Draft_ToggleGrid/ru "Draft ToggleGrid/ru") на панели инструментов. Используя функции **[привязок](/Draft_Snap/ru "Draft Snap/ru")** верстака **Draft** включайте только те *типы привязок* которые нужны в данный момент. Сейчас нам достаточно оставить включенными *конечную точку, среднюю точку и центр круга*, т.е. настройки привязок должны выглядеть как на картинке ниже.
...
Выберите **Цилиндр** на дереве проекта (он станет зеленым в 3D пространстве) и нажмите иконку ![](/images/Draft_Move.svg) **[Move](/Draft_Move/ru "Draft Move/ru")** на панели инструментов. Откроется *панель задач* для передвигаемых объектов, убедитесь, что чекбокс *Копировать* выключен.
...
Вернитесь на верстак [Part](/Part_Workbench/ru "Part Workbench/ru") и с помощью иконки ![](/images/Part_Cone.svg) **[Создать твердотельный конус](/Part_Cone/ru "Part Cone/ru")** на панели инструментов создайте *конус*. Установите свойство *radius1* в 0 мм и *radius2* в 7 мм, это даст 2 мм *фаски* по радиусу. Установите *высоту* конуса в 7 мм, в результате угол вершины конуса будет 90 градусов, а угол фаски - 45 градусов. Стоит отметить, что можно было бы так же использовать операцию ![](/images/Part_Chamfer.svg) [Фаска](/Part_Chamfer/ru "Part Chamfer/ru").
...
На рисунке ниже совмещение конуса и отверстия делается командой *Преобразовать*, которая имеется в меню, доступном при нажатии на правую кнопку мыши. Приращение перемещения установлено
в 1 мм т.к. если бы приращение было установлено в 5 мм как было ранее, то это не позволило бы совместить конус и отверстие из-за того, что диаметр конуса у нас 7 мм. Для того, чтобы было легче установить конус в правильную позицию используется ![](/images/Std_DrawStyleWireFrame.svg) **[Каркасный режим](/Std_DrawStyle/ru#Wireframe "Std DrawStyle/ru")** прорисовки модели.

![](/images/T101pwb04-02_conetranslate.png)

Чтобы закончить модель, давайте будем использовать ![](/images/Part_Boolean.svg) **[Булеву операцию с двуми выбранными формами](/Part_Boolean/ru "Part Boolean/ru")** вместо того, чтобы сначала выделить объекты, а потом применить булеву операцию. Выберите ее на панели инструментов и в результате откроется окно как на левом рисунке ниже.
...
Сохраните модель под новым именем при помощи команды *Сохранить как...* в главном меню. FreeCAD может выполнять все типичные операции 3D моделирования, одна из них это ![](/images/Part_Thickness.svg) **[Смена толщины](/Part_Thickness/ru "Part Thickness/ru")**, которую можно использовать для создания *полой* детали.

Поверните модель так, чтобы была видна нижняя её поверхность.

![](/images/T101pwb05-01_frombottom.png)

Выберите *нижнюю поверхность* модели, затем на верстаке [Part](/Part_Workbench/ru "Part Workbench/ru")  выберите инструмент ![](/images/Part_Thickness.svg) **[Смена толщины](/Part_Thickness/ru "Part Thickness/ru")** и на экране появится фигура как на рисунке ниже.
...
Если изменить положение тела, из которого образована фаска, таким же образом как на картинке, то в результате появятся 3 не пересекающихся тела. Верстак [Part](/Part_Workbench/ru "Part Workbench/ru") позволит вам это сделать, а [PartDesign](/PartDesign_Workbench/ru "PartDesign Workbench/ru") нет. Вы получите сообщение об ошибке с несколькими телами, либо он просто не отобразит все тела.

### Проблема топологического именования

Давайте вернемся к завершенной модели и посмотрим, как именуются её поверхности.

На рисунке ниже включена панель **[Просмотр выделения](/Selection_view/ru "Selection view/ru")**, чтобы показать, что выделено, а что нет, а также изменен цвет, чтобы выделение было легче увидеть.
...
Это называется **[Проблема топологического именования](/Topological_naming_problem/ru "Topological naming problem/ru")**. Желательно изучить, как избежать поломки моделей.
...
Вместо применения разрезания для каждого примитива, мы можем сначала их логически объединить операцией ![](/images/Part_Fuse.svg) **[Объединение](/Part_Fuse/ru "Part Fuse/ru")**, а затем выполнить разрезание между фигурами **Fillet** и **Fusion**.
...
FreeCAD делают добровольцы в свободное время. Если вы хотите еще больше расширить возможности FreeCAD, подумайте о том, чтобы внести свой [вклад](/Help_FreeCAD/ru "Help FreeCAD/ru") в FreeCAD, например, помочь улучшить документацию.

Retrieved from "<http://wiki.freecad.org/index.php?title=Creating_a_simple_part_with_Part_WB/ru&oldid=1519500>"