...
FreeCAD может импортироваться как модуль [Python](/Python/ru "Python/ru") в другие программы или в автономную консоль python, вместе со всеми модулями и компонентами. Можно даже импортировать пользовательский интерфейс FreeCAD как модуль python, но с [некоторыми ограничениями](#Caveats).
...
Однако,Для не-Qt преложений, имеется несколько ограничений , вы должны знать об этом. Это решение , вероятно не сработает вместе с остальными инструментальными средствами.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Embedding_FreeCAD/ru&oldid=1479957>"