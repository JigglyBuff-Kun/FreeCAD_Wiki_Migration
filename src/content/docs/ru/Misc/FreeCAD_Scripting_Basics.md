---
title: Основы составления скриптов в FreeCAD
---
...
Модули `FreeCAD` и `FreeCADGui` отвечают за создание и управление объектами в документе FreeCAD. Они не занимаются созданием или изменением геометрии. Это потому, что эта геометрия может быть нескольких типов, и поэтому она управляется дополнительными модулями, каждый из которых отвечает за управление определенным типом геометрии. Например, [Модуль Part](/Part_Workbench/ru "Part Workbench/ru") использует ядро OpenCascade и поэтому может создавать и манипулировать геометрией типа [B-rep](http://en.wikipedia.org/wiki/Boundary_representation). В то же время [Модуль Mesh](/Mesh_Workbench/ru "Mesh Workbench/ru") может создавать и модифицировать объекты типа сетка. Таким образом, FreeCAD может обрабатывать самые разные типы объектов, которые могут сосуществовать в одном документе, и новые типы могут быть легко добавлены в будущем.
...
Теперь вы действительно можете начать играть с FreeCAD! Насчёт полного списка всех доступных модулей и доступных инструментов смотрите в разделе [Category:API](/Category:API "Category:API").

[наверх](#top)

Retrieved from "<http://wiki.freecad.org/index.php?title=FreeCAD_Scripting_Basics/ru&oldid=1058136>"