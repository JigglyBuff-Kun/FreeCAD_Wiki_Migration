...
Перед тем как пройти через боль от процесса отладки , используйте [фреймворк для тестирования](/Testing/ru "Testing/ru") , чтобы проверить будут ли работать стандартные тесты должным образом. Если они не будут пройдены полностью, то возможно дело, в сбоях при установке программы.
...
* отладочная сборка FreeCAD (в настоящее время доступна только [сборка из исходного кода](/Compile_on_Linux/ru#For_a_Debug_build "Compile on Linux/ru"))
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Debugging/ru&oldid=1328616>"