...
* «Официальные» пакеты при выпуске новых релизов доступны через [Snap packages](/Ubuntu_Snap/ru "Ubuntu Snap/ru"), [AppImages](/AppImage/ru "AppImage/ru"), [Flatpaks](/Flatpak "Flatpak") и [PPA](#Stable_PPA_version)
* Экспериментальные сборки доступны через [PPA](#Development_PPA_.28Daily.29) daily repository, [AppImages](/AppImage/ru "AppImage/ru"), [Ubuntu Snaps](/Ubuntu_Snap/ru "Ubuntu Snap/ru").
...
Версия пакета официального релиза как правило сильно отстает от релизов FreeCAD. [Package: freecad](http://rpms.remirepo.net/rpmphp/zoom.php?rpm=freecad) показывает версии, включенные в репозитории Fedora с течением времени и версии.
...
Инструкции также доступны на [компиляция FreeCAD самостоятельно](/Compile_on_Linux/ru "Compile on Linux/ru"), включая сценарий специально для Fedora. С небольшим изменением, чтобы проверить конкретную фиксацию из git, любая версия начиная примерно с FreeCAD 0.15 может быть построена на любом дистрибутиве начиная с Fedora 21.
...
Если и этого недостаточно, и вы не можете найти другой источник предварительно скомпилированного пакета для вашей ситуации, вам потребуется [скомпилировать FreeCAD самостоятельно](/Compile_on_Linux/ru "Compile on Linux/ru").

## Следующий Шаг

После того, как вы установили FreeCAD, пора [приступить к работе](/Getting_started "Getting started")!

Retrieved from "<http://wiki.freecad.org/index.php?title=Installing_on_Linux/ru&oldid=1295742>"