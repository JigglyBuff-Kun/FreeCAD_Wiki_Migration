...
Compilation of documentation proceeds the same as [for C++ sources](#Compiling_the_documentation). If both Python files, `pyexample_a.py` and `pyexample_b.py`, with distinct commenting style are in the same directory, both will be processed.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Doxygen/ru&oldid=1536750>"