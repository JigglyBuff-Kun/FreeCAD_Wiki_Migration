---
title: Примечания к выпуску 013
---
...
Предыдущие версии: [0.12](/Release_notes_0.12 "Release notes 0.12") - [0.11](/Release_notes_0.11 "Release notes 0.11")
...
* **Функция обрезки**: Новый инструмент [Обрезка изображения](/Drawing_Clip "Drawing Clip") позволяет разместить проекции объектов внутри невидимого прямоугольника который обрежет границы изображения.
...
* **Функция аннотации**: Новая функция [Аннотация](/Drawing_Annotation "Drawing Annotation") , простой инструмент для размещения блока текста на чертеже.
...
* **Преобразование Чертеж <-> Эскиз**: Теперь Чертежный модуль обладает новым инструментом для конверсии [Draft2Sketch](/Draft_Draft2Sketch "Draft Draft2Sketch") , он позволяет преобразовать выбранные Чертежные объекты (или любую плоскую форму) в Эскизы , а также это работает наоборот.
...
* **Скругленные углы**: Некоторые Чертежные объекты, как ([Ломаные](/Draft_Wire "Draft Wire"), [Прямоугольник](/Draft_Rectangle "Draft Rectangle") и [Многоугольники](/Draft_Polygon "Draft Polygon")) теперь обладают свойством **Радиус скругления** , скругляет их углы по указанному значению радиуса.

![](/images/013-draft-fillet.jpg)

* **Двумерная проекция**: Новый инструмент [Shape2DView](/Draft_Shape2DView "Draft Shape2DView") позволяет быстро поместить проекцию выбранного объекта в трехмерную область документа(не пуать с модулем Проекций). Вы можете указать направление вектора проекции.

![](/images/013-draft-shape2dview.jpg)

## Архитектурный модуль

* **Интеграция с Чертежным модулем**: Архитектурный и Чертежный модуль теперь тесно интегрированный друг с другом. Архитектурные инструменты используют [систему привязок Чертежного модуля](/Draft_Snap "Draft Snap") , а все чертежные инструменты представлены в Архитектурном модуле. На самом деле, если вы хотите вы можете теперь полностью скрыть Чертежный модуль (Настройки -> Draft ->Hide the Draft workbench)
* **Новый инструмент для создания стен**: [Инструмент создания стен](/Arch_Wall "Arch Wall") был значительно усовершенствован, теперь появился режим рисования напрямую, он активируется , при нажатии на кнопку Стены, если не выбрано ни одного объекта, это позволяет вам рисовать стены, как будто вы рисуете простые линии. В дополнение к этому, теперь стены автоматически присоединяются если вы привязываетесь к существующей стене.

![](/images/013-arch-wall.jpg)

!!FUZZY!!\* **Новый инструмент создания крыш**: Теперь в Архитектурном модуле доступен новый [инструмент создания крыш](/Arch_Roof "Arch Roof") , который позволяет вам быстро создать скатные крыши из выбранных граней.

* **Новый инструмент создания окон**: Теперь [Окна](/Arch_Window "Arch Window") создаются напрямую на поверхности плоской формы содержащий одну и более кломаных кривых(замкнутых), таких как прямоугольник или эскиз. Если эта форма была сразу нарисована на грани Стены, окно, автоматически вырезает отверстие в стене.
* **Новая система сечений**: Теперь очень просто создать виды, разрезы и сечения из вашей модели: Поместите [Секущую плоскость](/Arch_SectionPlane "Arch SectionPlane") , сориентируйте её, отредактируйте его чтобы включить только те объекты которые она должна видеть и .... готово!
* **Новая система отрисовки твердых тел**: В дополнение к основанном на OpenCasCADe каркасной 2D визуализации , которая сейчас используется [Модуле создания проекций](/Drawing_Workbench "Drawing Workbench"), Архитектурный модуль тперь обладает новым двумерным визуализатором, который позволяет отрисовывать окрашенные грани в SVG Чертеж, создавая прекрасные 2D виды.
...
* **Новая система осей**: A new [axes system](/Arch_Axis "Arch Axis") feature has been added, that allows to quickly layout complex axes systems. These axes can then be added to [Structure](/Arch_Structure "Arch Structure") objects, so they spread automatically on the grid nodes.

![](/images/013-arch-axes.jpg)

* **Создание архитектурного объекта из полигионального**: Теперь, вы можете напрямую из полигиональных моделей создавать [стены](/Arch_Wall "Arch Wall") и [структуры](/Arch_Structure "Arch Structure"), только если замкнуты а тела и ребра [многообразны](http://doc.spatial.com/index.php/Manifold_and_Non-manifold_Objects). Это позволяет быстро преобразовывать импортированную геометрию из других приложений, таких как [blender](http://www.blender.org) в объекты Архитектурного модуля.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Release_notes_0.13/ru&oldid=1461966>"