---
title: PartDesign Скругление (Галтель)
---
|  |
| --- |
| Скругление |
| Расположение в меню |
| PartDesign → Apply a dress-up feature → Скругление |
| Верстаки |
...
| - |
| См. также |
| [Фаска](/PartDesign_Chamfer/ru "PartDesign Chamfer/ru"), [Скругление](/Part_Fillet/ru "Part Fillet/ru") |
|  |
...
* Данные**Base** (`LinkSub`): Link to the selected edges and faces of the parent feature. Can be a link to only the parent feature if Данные**Use All Edges** is `true`.
* Данные**Support Transform** (`Bool`): If `true` the filleted shape of the additive/subtractive parent feature will be used when the fillet object is included in a [pattern](/PartDesign_Workbench#Transformation_tools "PartDesign Workbench"), else only the shape of the fillet itself will be used. The default is `false`.
...
* Данные**Radius** (`QuantityConstraint`): The fillet radius. The default is `1 mm`.
* Данные**Use All Edges** (`Bool`): If `true` all edges of the feature are filleted, and the edges specified by Данные**Base** are ignored. The default is `false`.

Part Design

* Данные**Refine** (`Bool`): If `true` redundant edges are removed from the result of the operation. The default value is determined by the **Automatically refine model after sketch-based operation** preference. See [PartDesign Preferences](/PartDesign_Preferences#General "PartDesign Preferences").
...
Edge numbers are not completely stable, therefore it is advisable that you finish the main design work of your solid body before applying features like fillets and chamfers, otherwise edges could change names and filleted edges would likely become invalid. When the Данные**Use All Edges** property is `true` there is some protection from this. Because in such cases all the edges of the base object are used and there is no dependence on individual edge names.

Read more in [topological naming problem](/Topological_naming_problem "Topological naming problem").

Retrieved from "<http://wiki.freecad.org/index.php?title=PartDesign_Fillet/ru&oldid=1515569>"