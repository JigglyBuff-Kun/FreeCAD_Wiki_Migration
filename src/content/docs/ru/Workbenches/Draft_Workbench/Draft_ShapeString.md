...
| 0.14 |
| См. также |
| [Draft Text](/Draft_Text/ru "Draft Text/ru"), [Part Extrude](/Part_Extrude/ru "Part Extrude/ru") |
|  |
...
9. Optionally change the Данные**Justification** of the ShapeString. See [Properties](#Properties).
...
On Windows access to the default font folder is restricted. This affects the font file selection for ShapeStrings. There are three cases in FreeCAD where a font file for ShapeStrings can be specified: in the ShapeString task panel, when changing the Данные**Font File** property of a ShapeString, and when specifying the default font file in the [Draft Preferences](/Draft_Preferences#Texts_and_dimensions "Draft Preferences").
...
* Данные**Font File** (`File`): Font file name.
* Данные**Fuse** (`Bool`): Fuse faces if faces overlap, usually not required (can be very slow). Ignored if Данные**Make Face** is `false`. [introduced in 1.0](/Release_notes_1.0 "Release notes 1.0")
* Данные**Justification** (`Enumeration`): Horizontal and vertical alignment. Options: `Top-Left`, `Top-Center`, `Top-Right`, `Middle-Left`, `Middle-Center`, `Middle-Right`, `Bottom-Left`, `Bottom-Center`, `Bottom-Right`. [introduced in 1.0](/Release_notes_1.0 "Release notes 1.0")
* Данные**Justification Reference** (`Enumeration`): Height reference used for justification. Options: `Cap Height`, `Shape Height`. The shape height depends on the characters in Данные**String**. [introduced in 1.0](/Release_notes_1.0 "Release notes 1.0")
* Данные**Keep Left Margin** (`Bool`): Keep left margin and leading white space when justification is left. [introduced in 1.0](/Release_notes_1.0 "Release notes 1.0")
* Данные**Make Face** (`Bool`): Fill letters with faces.
* Данные**Oblique Angle** (`Angle`): Oblique (slant) angle. Must be in the -80° to +80° range. [introduced in 1.0](/Release_notes_1.0 "Release notes 1.0")
* Данные**Scale To Size** (`Bool`): Scale to ensure cap height is equal to size. If set to `false`, depending on the font, cap height will not match Данные**Size** exactly. [introduced in 1.0](/Release_notes_1.0 "Release notes 1.0")
* Данные**Size** (`Length`): Height of text.
* Данные**String** (`String`): Text string. A ShapeString can only display a single text line.
* Данные**Tracking** (`Distance`): Inter-character spacing. [introduced in 1.0](/Release_notes_1.0 "Release notes 1.0"): The property type has been updated.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Draft_ShapeString/ru&oldid=1529239>"