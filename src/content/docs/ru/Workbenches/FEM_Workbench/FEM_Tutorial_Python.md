...
Этот пример предназначен для демонстрации того, как Python выполняет простой анализ методом конечных элементов (FEA) во FreeCAD [FEM Workbench](/FEM_Workbench "FEM Workbench"). Модель из [FEM\_CalculiX\_Cantilever\_3D](/FEM_CalculiX_Cantilever_3D "FEM CalculiX Cantilever 3D") будет взята для этого примера.
...
* Настройка модуля FEM. Проверьте [FEM\_CalculiX\_Cantilever\_3D](/FEM_CalculiX_Cantilever_3D "FEM CalculiX Cantilever 3D")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=FEM_Tutorial_Python/ru&oldid=1251684>"