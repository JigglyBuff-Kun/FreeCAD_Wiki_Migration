---
title: Сетки конечных элементов
---
...
В [верстаке FEM](/FEM_Workbench/ru "FEM Workbench/ru") существует несколько способов получения сетки для расчета МКЭ:

- Через графический интерфейс с помощью инструмента [GMSH](/FEM_MeshGmshFromShape/ru "FEM MeshGmshFromShape/ru").
- Через графический интерфейс с помощью инструмента [Netgen](/FEM_MeshNetgenFromShape/ru "FEM MeshNetgenFromShape/ru").
...
- Создание сетки "вручную" с помощью [Python](/Python/ru "Python/ru") скриптов.

Инструменты Gmsh и Netgen поддерживают создание сеток из тел, созданных в верстаках [Part](/Part_Workbench/ru "Part Workbench/ru") и [PartDesign](/PartDesign_Workbench/ru "PartDesign Workbench/ru"), а также из простых копии этих тел. В общем, любой верстак, который генерирует твердые объекты, например [верстак Arch](/Arch_Workbench/ru "Arch Workbench/ru"), также можно использовать для создания сеток. Обратите внимание, что сетка, используемая для АКЭ, не то же самое что сетка, созданная или импортированная из [верстака Mesh](/Mesh_Workbench/ru "Mesh Workbench/ru").
...
- ENigMA, [ветка на форуме](https://forum.freecadweb.org/viewtopic.php?f=18&t=33048), [исходный код](https://github.com/bjaraujo/ENigMA)
- libMesh, [основной вебсайт](http://libmesh.github.io/), [исходный код](https://github.com/libMesh/libmesh), [ветка на форуме](https://forum.freecadweb.org/viewtopic.php?f=18&t=33621); это активный проект, но работает только на C++
...
Импорт и экспорт элементов сетки
...
| Название | Название | создание элементов | просмотр элементов | импорт/экспорт | импорт/экспорт | импорт/экспорт  | импорт      | импорт/экспорт | импорт/экспорт                                                                 |
| seg 2    | B31      |                    |                    |                |                |                 |             |                |                                                                                |
| seg 3    | B32      |                    |                    |                |                |                 |             | НР             |                                                                                |
| tria 3   | S3       |                    |                    |                |                |                 |             |                |                                                                                |
| tria 6   | S6       |                    |                    |                |                |                 |             |                |                                                                                |
...
| tetra 4  | C3D4     |                    |                    |                |                |                 |             |                |                                                                                |
| tetra 10 | C3D10    |                    |                    |                |                |                 |             |                |                                                                                |
| hexa 8   | C3D8     |                    |                    |                |                |                 |             |                | () допускается форматом, но не доступно для чтения или записи с помощью FEniCS |
| hexa 20  | C3D20    |                    |                    |                |                |                 |             |                |                                                                                |
| penta 6  | C3D6     |                    |                    |                |                |                 |             | ?              |                                                                                |
| penta 15 | C3D15    |                    |                    |                |                |                 |             | ?              |                                                                                |
| pyra 5   |          |                    |                    |                |                |                 |             |                |                                                                                |
...
* ![](/images/Edit_Cancel.svg) означает, что спецификация формата не поддерживает этот тип элемента, поэтому FreeCAD не поддерживает его.
...
_Смотрите так же:_ [Основы составления скриптов в FreeCAD](/FreeCAD_Scripting_Basics/ru "FreeCAD Scripting Basics/ru").
...
Retrieved from "<http://wiki.freecad.org/index.php?title=FEM_Mesh/ru&oldid=1459822>"
