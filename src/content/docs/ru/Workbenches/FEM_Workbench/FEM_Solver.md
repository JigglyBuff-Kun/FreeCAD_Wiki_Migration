...
На этой странице собрана вся информация о решателях метода конечных элементов (МКЭ), используемых [верстаком FEM](/FEM_Workbench/ru "FEM Workbench/ru"). Интерфейс между решателем и FreeCAD в предварительной и последующей обработке выполнен через текстовые файлы. Теоретически это означает, что любой решатель, который может быть сконфигурирован и управляться через текстовые файлы, способен работать с FreeCAD, чтобы эта коммуникация действовала, необходимо запрограммировать соответствующий синтаксический анализатор и писатель входных и выходных файлов.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=FEM_Solver/ru&oldid=1351070>"