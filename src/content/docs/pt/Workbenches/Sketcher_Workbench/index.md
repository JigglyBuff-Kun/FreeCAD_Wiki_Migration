...
In sketches that have **Auto constraints** checked (default) several constraints are applied automatically. The icon of a proposed automatic constraint is shown next to the cursor when it is placed correctly. Left-clicking will then apply that constraint. This is a per-sketch setting that can be changed in the [Sketcher Dialog](/Sketcher_Dialog#Constraints "Sketcher Dialog") or by changing the Ver**Autoconstraints** [property](/Property_editor "Property editor") of the sketch.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Sketcher_Workbench/pt&oldid=1537982>"
