...
O sistema de preferências de FreeCAD está localizado no menu Editar → Preferências.

A funcionalidade FreeCAD esta dividida em diferentes módulos, sendo cada módulo responsável pelo funcionamento de um específico  [bancada](/Workbenches "Workbenches"). FreeCAD também usa um conceito chamado de carregamento atrasado, o que significa que os componentes são carregados somente quando eles são necessários. Você deve ter notado que quando você selecciona uma bancada na barra de ferramentas FreeCAD, que bancada e todos os seus componentes são carregados no momento. Isso inclui suas configurações de preferências.
...
Sem qualquer módulo carregado, você terá acesso a duas secções de configuração, responsáveis das configurações gerais do aplicativo e das configurações de visualização.
...
## As configurações gerais das preferências

Quando começar a FreeCAD sem bancada carregada, vai ter uma janela de preferências mínima. Quando carregar módulos adicionais, aparecerão novas secções na janela de preferências, o que lhe permite configurar os detalhes de cada bancada.
...
| **Two-side rendering** | If checked, the default value for the Ver**Lighting** property is `Two side` instead of `One side`. `Two side` means the color of the interior side of faces is the same as the color of the exterior side. `One side` means their color is either the [backlight color](#3D_View), if enabled, or black. |
| **Show bounding-box for highlighted or selected meshes** | If checked, the default value for the Ver**Selection Style** property is `BoundBox` instead of `Shape`. `BoundBox` means a highlighted bounding box is displayed if meshes are highlighted or selected. `Shape` means the shape itself is then highlighted. |
...
O módulo rascunho tem a sua [preferences](/index.php?title=Draft_Preferences/pt&action=edit&redlink=1 "Draft Preferences/pt (page does not exist)") ecrã.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Preferences_Editor/pt&oldid=1542506>"