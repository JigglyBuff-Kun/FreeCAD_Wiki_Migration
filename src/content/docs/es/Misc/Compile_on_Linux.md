...
---
:::caution
Hay un contenedor experimental FreeCAD Docker que se está probando para el desarrollo de FreeCAD. Lea más sobre ello enCompilar en Docker
:::
...
A continuación, encontrará explicaciones detalladas de todo el proceso, algunos [build scripts](#Automatic_build_scripts), y las particularidades que puede encontrar. Si encuentras algo incorrecto o desactualizado en el texto de abajo (las distribuciones de Linux cambian a menudo), o si usas una distribución que no está en la lista, discute el tema en el [forum](https://forum.freecadweb.org/index.php), y ayúdanos a corregirlo.
...
Para más información sobre el uso de Git y la contribución de código al proyecto, véase [Gestión del código fuente](/index.php?title=Source_code_management/rs&action=edit&redlink=1 "Source code management/rs (page does not exist)").

## Obtener las dependencias

Para compilar FreeCAD tienes que instalar las dependencias necesarias mencionadas en [Bibliotecas de terceros](/Third_Party_Libraries/es "Third Party Libraries/es"); los paquetes que contienen estas dependencias están listados abajo para diferentes distribuciones de Linux. Tenga en cuenta que los nombres y la disponibilidad de las bibliotecas dependerán de su distribución particular; si su distribución es antigua, algunos paquetes pueden no estar disponibles o tener un nombre diferente. En este caso, busque en la sección [Distribuciones antiguas y no convencionales](#Older_and_non-conventional_distributions) más abajo.

Una vez que tengas todas las dependencias instaladas, procede a [compilar FreeCAD](#Compilar_FreeCAD).
...
* `libcoin-dev`, para Debian Buster, Ubuntu 19.04 y más recientes, así como para Ubuntu 18.04/18.10 con los PPAs [freecad-stable/freecad-daily](/Installing_on_Linux#Official_Ubuntu_repository "Installing on Linux") añadidos a sus fuentes de software.
...
Para compilar FreeCAD para Debian Buster, Ubuntu 19.04 y posteriores, así como Ubuntu 18.04/18.10 con el [freecad-stable/freecad-daily PPAs](/Installing_on_Linux/es#Repositorio_Oficial_Ubuntu "Installing on Linux/es") añadido a sus fuentes de software, instale las siguientes dependencias.
...
Para Debian Buster y Ubuntu 18.10 y posteriores, así como para Ubuntu 18.04 con el [freecad-stable/freecad-daily PPAs](/Installing_on_Linux#Repositorio_Oficial_Ubuntu "Installing on Linux") añadido a sus fuentes de software, instale los paquetes oficiales.
...
* `libspnav-dev`, para soporte de [Dispositivos de entrada 3D](/3D_input_devices/es "3D input devices/es"), como el "Space Navigator" o el "Space Pilot" de 3Dconnexion.
...
Requiere Pyside2 disponible en Debian buster y el [freecad-stable/freecad-daily PPAs](/Installing_on_Linux/es#Repositorio_Oficial_Ubuntu "Installing on Linux/es").
...
Ver [piano\_jonas guía no oficial "Compilar en openSUSE"](https://forum.freecadweb.org/viewtopic.php?f=4&t=49726).
...
[Pivy](/Pivy/es "Pivy/es") (envoltorios de Python para Coin3d) no es necesario para construir FreeCAD o para iniciarlo, pero es necesario como una dependencia en tiempo de ejecución por el [Ambiente de Trabajo Borrador](/Draft_Workbench/es "Draft Workbench/es"). Si no vas a utilizar este ambiente de trabajo, no necesitarás Pivy. Sin embargo, ten en cuenta que el Ambiente de Trabajo Borrador es utilizado internamente por otros ambientes de trabajo, como [Arquitectura](/Arch_Workbench/es "Arch Workbench/es") y [BIM](/BIM_Workbench/es "BIM Workbench/es"), por lo que Pivy necesita ser instalado para utilizar estos ambientes de trabajo también.

En noviembre de 2015 la versión obsoleta de Pivy incluida con el código fuente de FreeCAD ya no compilará en muchos sistemas. Esto no es un gran problema ya que normalmente deberías obtener Pivy desde el gestor de paquetes de tu distribución; si no puedes encontrar Pivy, puede que tengas que compilarlo tú mismo, ver [Instrucciones de compilación de Pivy](/Extra_python_modules/es#Pivy "Extra python modules/es").
...
Por ejemplo, para evitar construir el [Ambiente de trabajo MEF](/FEM_Workbench/es "FEM Workbench/es"):
...
Crea una compilación `Debug` para solucionar fallos en FreeCAD. Ten en cuenta que con esta compilación el [Croquizador](/Sketcher_Workbench/es "Sketcher Workbench/es") se vuelve muy lento con croquis complejos.
...
7. Si las variables no cargan el proyecto correctamente, puede que tenga que ir a **Projects → Manage Kits → Kits → Default (o Kit importado o similar) → CMake Configuration**.
A continuación, pulse Cambiar, y añada la configuración adecuada como se ha descrito anteriormente. Puede que tenga que añadir más variables sobre las rutas de Python, si no se encuentra el Python del sistema.
...
Anteriormente, una versión de Pivy estaba incluida en el código fuente de FreeCAD (interno). Si querías usar la copia de Pivy de tu sistema (externo), necesitabas usar <código>-DFREECAD\_USE\_EXTERNAL\_PIVY=1</código>.
...
Si tienes Doxygen instalado puedes construir la documentación del código fuente. Consulte [documentación de origen](/Source_documentation/es "Source documentation/es") para obtener instrucciones.
...
Enviado por el usuario [[PrzemoF](http://forum.freecadweb.org/memberlist.php?mode=viewprofile&u=3666)] en el foro.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Compile_on_Linux/es&oldid=1571080>"