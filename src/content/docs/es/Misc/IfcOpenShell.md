...
IfcOpenShell es una biblioteca de software de código abierto (LGPL 3) que ayuda a los desarrolladores a trabajar con el formato de archivo [industry foundation classes](http://www.buildingsmart-tech.org/specifications/ifc-overview) ([IFC](/Arch_IFC/es "Arch IFC/es")). El formato de archivo IFC puede utilizarse para describir datos de construcción y edificación. El formato se utiliza habitualmente para [building information modelling](https://en.wikipedia.org/wiki/Building_information_modeling) (BIM), por ejemplo, para el análisis de cargas mecánicas y para los estudios de eficiencia térmica y energética. IfcOpenShell es principalmente una colección de librerías C++, sin embargo, como tiene enlaces [Python](/Python/es "Python/es"), puede integrarse con programas como FreeCAD y Blender.

IfcOpenShell utiliza [OpenCASCADE](/OpenCASCADE/es "OpenCASCADE/es") internamente para convertir la geometría implícita de los archivos IFC en geometría explícita que otros paquetes CAD pueden entender, por ejemplo, STEP, [OBJ](/Arch_OBJ/es "Arch OBJ/es"), y [DAE](/Arch_DAE/es "Arch DAE/es").

A partir de la v0.19, FreeCAD es capaz de importar archivos IFC siempre que el módulo `ifcopenshell` [Python](/Python/es "Python/es") esté disponible en el sistema. Asimismo, los [Arquitectura](/Arch_Workbench/es "Arch Workbench/es") y [BIM ambientes de trabajo](/BIM_Workbench/es "BIM Workbench/es") pueden exportar un modelo de edificio al formato IFC para que pueda ser utilizado en otras aplicaciones.

Para verificar que IfcOpenShell está instalado en su sistema, intente importarlo desde la [Consola de Python](/Python_console/es "Python console/es"); la biblioteca está correctamente instalada si no se devuelve ningún mensaje de error.
...
Para los sistemas Windows y MacOS, las distribuciones de FreeCAD que se han creado con el gestor de paquetes [Conda](/Conda/es "Conda/es") suelen incluir ya IfcOpenShell, por lo que no es necesaria ninguna instalación adicional. Obtén la distribución apropiada desde la página [Download](/Download/es "Download/es").

La [AppImage](/AppImage/es "AppImage/es") para Linux también se basa en [Conda](/Conda/es "Conda/es"), y también incluye IfcOpenShell.
...
:   * La ruta de búsqueda se puede encontrar inspeccionando la variable `sys.path` en la [consola de Python](/Python_console/es "Python console/es").
...
Si el directorio se ha movido correctamente, comprueba que el módulo `ifcopenshell` es accesible desde la [Consola de Python](/Python_console/es "Python console/es").
...
* [Compilación de IfcOpenShell para MacOS](/Import/Export_IFC_-_compiling_IfcOpenShell/es "Import/Export IFC - compiling IfcOpenShell/es"); una guía antigua que describe el proceso general. Puede que ya no sea necesaria ya que IfcOpenShell se distribuye ahora junto con FreeCAD gracias a [Conda](/Conda/es "Conda/es").
* Qué páginas enlazan con [esta página](/Special:WhatLinksHere/IfcOpenShell/es "Special:WhatLinksHere/IfcOpenShell/es").

Retrieved from "<http://wiki.freecad.org/index.php?title=IfcOpenShell/es&oldid=1433909>"