...
Este artículo trata de llevar un robot industrial estándar de 6 ejes a FreeCAD para su simulación en el ![](/images/Workbench_Robot.svg) [Ambiente de trabajo Robot](/Robot_Workbench/es "Robot Workbench/es")

La siguiente descripción sigue el sistema de [parámetros Denavit-Hartenberg](http://en.wikipedia.org/wiki/Denavit-Hartenberg_Parameters), como también se describe en el libro de John J. Craigs "Introducción a la robótica".
...
Desmarcar como comentario el sensor de tiempo TimeSensor y las 6 rutas. Estas 6 líneas te dan una pista de la situación del eje real del robot! Primero busca por "AXIS\_04\_OBJ" que te lleva a esta parte:
...
Tienes que insertar justo encima de la definición de este nodo la línea **"DEF FREECAD\_AXIS1 Transform { rotation 0 1 0 0 children ["** la cual es el anclaje para que FreeCAD pueda mover el eje.

Ahora haz lo mismo para **FREECAD\_AXIS2**,**FREECAD\_AXIS3**,**FREECAD\_AXIS4**,**FREECAD\_AXIS5** y **FREECAD\_AXIS6**. No olvides cerrar los corchetes al final del archivo y listo!
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Robot_6-Axis/es&oldid=1251475>"