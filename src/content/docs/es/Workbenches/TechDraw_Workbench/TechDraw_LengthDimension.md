---
title: TechDraw Dimensión longitudinal
---
|  |
| --- |
| TechDraw Dimensión Longitud |
...
| - |
| Ver también |
| [TechDraw Dimensión Horizontal](/TechDraw_HorizontalDimension/es "TechDraw HorizontalDimension/es"), [TechDraw Dimensión Vertical](/TechDraw_HorizontalDimension/es "TechDraw HorizontalDimension/es") |
|  |
...
* Datos**References 2D** (`LinkSubList`): The 2D drawing View object(s) on which the measurement is based. Used if Datos**Measure Type** is `Projected`.
* Datos**References 3D** (`LinkSubList`): The 3D object(s) on which the measurement is based. Used if Datos**Measure Type** is `True`.
* Datos**Type** (`Enumeration`): Length, radius, diameter, etc. Not normally manipulated by the end user.
* Datos**Measure Type** (`Enumeration`): How the measurement is performed.

:   :   `True` - based on 3D geometry.
    :   `Projected` - based on 2D drawing View geometry.

* Datos**Theoretical Exact** (`Bool`): Specifies a theoretically exact (or basic) dimension.
...
* Datos**Equal Tolerance** (`Bool`): If over- and undertolerance are equal. Then the negated value of the overtolerance is used as undertolerance.

:   :   `true` - the negated value of the Datos**Over Tolerance** is used as Datos**Under Tolerance**. The display will be ![](/images/TechDraw_equal-tolerance.png)
    :   `false` - the Datos**Under Tolerance** is taken into account. The display will be ![](/images/TechDraw_Non-equal-tolerance.png)

* Datos**Over Tolerance** (`QuantityConstraint`): The amount by which the dimension may be larger.
* Datos**Under Tolerance** (`QuantityConstraint`): The amount by which the dimension may be smaller.
* Datos**Inverted** (`Bool`): Marks whether the dimension represents a common or an inverted value.
...
* Datos**X** (`Distance`): Horizontal position of the dimension text relative to the View.
* Datos**Y** (`Distance`): Vertical position of the dimension text relative to the View.
* Datos (Hidden)**Lock Position** (`Bool`): Locks the position of the dimension text when `true`.
* Datos (Hidden)**Rotation** (`Angle`): Read only.
* Datos (Hidden)**Scale Type** (`Enumeration`): Read only.
* Datos (Hidden)**Scale** (`FloatConstant`): Read only.
* Datos (Hidden)**Caption** (`String`): Not used.

Format

* Datos**Format Spec** (`String`): How the dimension value will be formatted. See [Formatting](#Formatting).
* Datos**Format Spec Over Tolerance** (`String`): Like Datos**Format Spec**, but for overtolerances.
* Datos**Format Spec Under Tolerance** (`String`): Like Datos**Format Spec**, but for undertolerances.
* Datos**Arbitrary** (`Bool`): Whether the dimension is replaced by the content of the **Format Spec** field.
...
* Datos**Arbitrary Tolerances** (`Bool`): Like Datos**Arbitrary**, but for the tolerance.

Override

* Datos**Angle Override** (`Bool`): Whether the direction of dimension and extension lines is overridden.
...
* Datos**Line Angle** (`Angle`): Angle of dimension line with view X axis (in degrees).
* Datos**Extension Angle** (`Angle`): Angle of extension line(s) with view X axis (in degrees).

References

* Datos (Hidden)**Saved Geometry** (`TopoShapeList`): Reference geometry. [introduced in 0.21](/Release_notes_0.21 "Release notes 0.21")

### View

Base

* Vista**Keep Label** (`Bool`): Not used.
* Vista**Stack Order** (`Integer`): Over or underlap relative to other drawing objects. [introduced in 0.21](/Release_notes_0.21 "Release notes 0.21")

Dimension Format

* Vista**Arrowsize** (`Length`): The size of the dimension arrows. [introduced in 0.21](/Release_notes_0.21 "Release notes 0.21")
* Vista**Color** (`Color`): Color for lines and text.
* Vista**Flip Arrowheads** (`Bool`): By default the value *inside* the dimension line/arc means the arrows pointing *outwards*. If placed *outside* the dimension line/arc, the arrows point *inwards* the dimension line/arc.
...
* Vista**Font** (`Font`): The name of the font to use for the dimension text.
* Vista**Fontsize** (`Length`): Dimension text size.
* Vista**Gap Factor ASME** (`Float`): Adjusts the gap between the dimension points and the start of the extension lines. The gap is this value times the line width. [introduced in 0.21](/Release_notes_0.21 "Release notes 0.21")
* Vista**Gap Factor ISO** (`Float`): Adjusts the gap between the dimension points and the start of the extension lines. The gap is this value times the line width. [introduced in 0.21](/Release_notes_0.21 "Release notes 0.21")
* Vista**Line Spacing Factor** (`Float`): Adjusts the gap between the dimension text and the dimension line. The gap is this value times the line width. [introduced in 0.21](/Release_notes_0.21 "Release notes 0.21")
* Vista**Line Width** (`Length`): Dimension line weight.
* Vista**Rendering Extent** (`Enumeration`): Rather universal property specifying how much space the dimension drawing may take up:
...
* Vista**Standard And Style** (`Enumeration`): Specifies the standard (and its style) according to which the dimension is drawn:
...
Retrieved from "<http://wiki.freecad.org/index.php?title=TechDraw_LengthDimension/es&oldid=1486696>"