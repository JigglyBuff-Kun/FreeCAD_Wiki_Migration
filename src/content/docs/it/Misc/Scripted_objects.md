---
title: Oggetti creati da script
---
## Introduzione

Oltre ai tipi di oggetti standard, come le Annotazioni, gli oggetti Mesh e gli oggetti Parte, FreeCAD offre anche la straordinaria possibilità di costruire al 100% oggetti parametrici in script di Python, chiamati [Python Features](/App_FeaturePython/it "App FeaturePython/it") (*Caratteristiche* Python). Questi oggetti si comportano esattamente come un qualsiasi altro oggetto di FreeCAD, e sono salvati e ripristinati automaticamente con salva/apri il file.
...
`obj.addProperty(...)` restituisce `obj`, in modo che il valore della proprietà possa essere impostato sulla stessa riga:
...
In questo esempio si utilizza il Modulo [Part](/Part_Workbench/it "Part Workbench/it") per creare un ottaedro, quindi si crea la sua rappresentazione Coin con Pivy.
...
Vedi [Metodi FeaturePython](/FeaturePython_methods/it "FeaturePython methods/it") per i riferimenti completi.
...
Vedere [Proprietà personalizzate FeaturePython](/FeaturePython_Custom_Properties/it "FeaturePython Custom Properties/it") per una panoramica.

Quando si aggiungono proprietà ai propri oggetti personalizzati, fare attenzione a quanto segue:

* Non utilizzare i caratteri `<` o `>` nelle descrizioni delle proprietà (questo potrebbe danneggiare le porzioni xml nel file .FCStd).
...
Gli attributi impostati utilizzando `setEditorMode` possono essere rimossi dall'utente. Vedere [Editor delle proprietà](/Property_editor/it#Context_menu "Property editor/it"). Tenere presente che le proprietà di sola lettura possono ancora essere modificate da Python.

E' possibile anche impostare questi e altri attributi direttamente con la funzione `addProperty`. Gli attributi impostati con tale funzione non possono essere modificati dall'utente. Una possibilità interessante è contrassegnare una proprietà come proprietà di output. In questo modo FreeCAD non contrassegnerà l'oggetto come touched quando lo si modifica (quindi non è necessario ricalcolarlo).
...
Gli attributi che possono essere impostati con `addProperty` sono elencati di seguito. È possibile impostare più attributi aggiungendo valori.
...
Per `Prop_ReadOnly` e `Prop_Hidden` anche la funzione `addProperty` ha argomenti booleani:
...
[disponibile dalla versione 1.0](/Release_notes_1.0/it "Release notes 1.0/it"): La firma completa della funzione è:
...
* `group`: Sottosezione della proprietà (utilizzata nell'[Editor della proprietà](/Property_editor/it "Property editor/it")).
* `doc`: Tooltip (idem).
* `attr`: Attributo, vedere sopra.
* `read_only`: Vedi sopra.
* `hidden`: Vedi sopra.
* `enum_vals`: Valori di enumerazione (elenco di stringhe), rilevante solo se il tipo è `"App::PropertyEnumeration"`.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Scripted_objects/it&oldid=1500318>"