---
title: Note di rilascio della versione 0.13
---
...
Versioni precedenti: ([0.11 en](/Release_notes_0.11 "Release notes 0.11")) - [0.12](/Release_notes_0.12/it "Release notes 0.12/it")
...
* **Operazione Ritaglio**: Un nuovo oggetto di [Ritaglio](/Drawing_Clip/it "Drawing Clip/it") consente di posizionare le viste di oggetti all'interno di rettangoli ritagliati nelle pagine di disegno.
...
* **Operazione Annotazione**: Un nuovo oggetto [Annotazione](/Drawing_Annotation/it "Drawing Annotation/it"), una semplice funzione per posizionare rapidamente i blocchi di testo in una pagina di disegno.
* **Viste ortografiche**: Un nuovo strumento [Viste ortogonali](/Drawing_Orthoviews/it "Drawing Orthoviews/it") facilita la creazione di viste multiple tutte allineate tra di loro, secondo la proiezione del primo o del terzo quadrante.
...
* **Importatore DXF**: L'importatore di file .DXF ora supporta i punti (tradotti come [punti di Draft](/Draft_Point/it "Draft Point/it")) e le linee (tradotte come [spezzate di Draft](/Draft_Wire/it "Draft Wire/it"))
* **Nuovo sistema di ancoraggio**: Il [sistema di aggancio](/Draft_Snap/it "Draft Snap/it") del modulo Draft è stato riscritto quasi completamente. Ora è molto più facile estenderlo e utilizzarlo in altri script o moduli, ora ha nuove icone e nuovi cursori, e dispone di una barra degli strumenti che consente di attivare o disattivare le singole posizioni di blocco o l'intero sistema di snap.
...
* **Conversione Draft <-> Schizzo**: L'ambiente Draft ora offre il nuovo strumento di conversione [Draft2Sketch](/Draft_Draft2Sketch/it "Draft Draft2Sketch/it"), che converte gli oggetti Draft selezionati (o qualsiasi forma piana) in Schizzi, e viceversa.
...
* **Angoli arrotondati**: Diversi oggetti di Draft ([Contorni](/Draft_Wire/it "Draft Wire/it"), [Rettangoli](/Draft_Rectangle/it "Draft Rectangle/it") e [Poligoni](/Draft_Polygon/it "Draft Polygon/it")) ora hanno la proprietà **Raggio di raccordo** (Fillet Radius), che arrotonda i loro angoli con il valore del raggio di raccordo indicato.

![](/images/013-draft-fillet.jpg)

* **Vista 2D dell'oggetto**: Il nuovo strumento [Shape2DView](/Draft_Shape2DView/it "Draft Shape2DView/it") permette di posizionare rapidamente nel documento una vista 2D di un oggetto selezionato. È possibile specificare il vettore della proiezione.

![](/images/013-draft-shape2dview.jpg)

## Modulo Architettura

* **Integrazione con Draft**: I moduli **Architettura** e **Draft** ora sono strettamente integrati. Gli strumenti di Arch utilizzano il sistema di [ancoraggio](/Draft_Snap/it "Draft Snap/it") di Draft, e tutti gli strumenti di Draft sono presenti nell'ambiente Architettura. Infatti, se si desidera, è possibile disattivare completamente il modulo Draft (Modifica → Preferenze → Draft → Nascondi l'ambiente Draft).
* **Nuovo strumento Muro**: Lo strumento [Muro](/Arch_Wall/it "Arch Wall/it") è stato notevolmente migliorato. Ora dispone di una modalità di disegno diretta, che si attiva quando si preme il pulsante Muro senza alcun oggetto selezionato, questo consente di disegnare i muri come si disegnano semplici linee. Inoltre, ora, quando si aggancia un muro esistente, la connessione tra le pareti è automatica.

![](/images/013-arch-wall.jpg)

* **Nuovo strumento Tetto**: Ora nel modulo Architettura è disponibile un nuovo strumento [Tetto](/Arch_Roof/it "Arch Roof/it") che consente di creare rapidamente tetti inclinati partendo da una faccia selezionata.
* **Nuovo strumento Finestra**: Le [Finestre](/Arch_Window/it "Arch Window/it") ora vengono create direttamente sopra una forma piana che contiene uno o più contorni, come, ad esempio, su un rettangolo o uno schizzo. Se tale forma è stata disegnata direttamente su una faccia di una parete, la finestra taglia la parete e produce automaticamente un'apertura nel muro.
* **Nuovo metodo di sezione**: Ora è molto semplice creare piani 2D, sezioni e prospetti del modello. Basta inserire un oggetto [Piano di sezione](/Arch_SectionPlane/it "Arch SectionPlane/it"), orientarlo nel modo desiderato, modificarlo per includere gli oggetti che si devono vedere, ed è fatto!
* **Nuovo renderizzatore solido**: Oltre al render di contorni 2D basato su OpenCasCADe, utilizzato attualmente dal modulo [Drawing](/Drawing_Workbench/it "Drawing Workbench/it"), il modulo **Arch** offre ora un nuovo renderer 2D, che è in grado di rendere le facce piene su un foglio di disegno in formato .SVG, producendo delle viste 2D più gradevoli.
...
* **Nuovo sistema di assi**: E' stato aggiunto un nuovo [Sistema di assi](/Arch_Axis/it "Arch Axis/it") che permette la disposizione in modo rapido di sistemi di assi complessi. Questi assi possono essere aggiunti agli oggetti [Struttura](/Arch_Structure/it "Arch Structure/it") e, in questo modo, si propagano automaticamente sui nodi della griglia.

![](/images/013-arch-axes.jpg)

* **Oggetti Architettura da oggetti Meshes**: Ora [Muri](/Arch_Wall/it "Arch Wall/it") e [Strutture](/Arch_Structure/it "Arch Structure/it") possono essere creati direttamente da oggetti maglia, purché questi siano ​​chiusi, solidi e tutti i bordi siano [manifold](http://doc.spatial.com/index.php/Manifold_and_Non-manifold_Objects). Questo consente di trasformare molto rapidamente le geometrie importate da altre applicazioni, quali [Blender](http://www.blender.org), in oggetti Arch validi.
...
* Nuovo modulo navale ([tutorial](/FreeCAD-Ship_s60_tutorial/it "FreeCAD-Ship s60 tutorial/it"))
...
* Per ulteriori informazioni consultare la pagina [OpenSCAD](/OpenSCAD_Workbench/it "OpenSCAD Workbench/it") nel wiki di FreeCAD.

Retrieved from "<http://wiki.freecad.org/index.php?title=Release_notes_0.13/it&oldid=1462735>"