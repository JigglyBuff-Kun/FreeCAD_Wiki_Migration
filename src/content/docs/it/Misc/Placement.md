...
**Posizionamento** è il modo in cui FreeCAD specifica la posizione e l'orientamento di un oggetto nello spazio. Il posizionamento può essere specificato in più forme e manipolato tramite [scripting](/Python_scripting_tutorial#Vectors_and_placements "Python scripting tutorial"), l'[editor di proprietà](/Property_editor/it "Property editor/it") o selezionando **Modifica → Posizionamento...** per aprire [Pannello azioni di Posizionamento](/Std_Placement/it "Std Placement/it").
...
Posizionamento nell'editor delle proprietà

![](/images/PlacePyConv10.png)

Script Placement con y/p/r, Matrix e la sua [API](/Placement_API "Placement API").

![](/images/PlacementDialogv10.png)

Pannello azioni di Posizionamento
...
**Axis = (ax,ay,az)** : è un vettore unitario che descrive un asse di rotazione (vedere [Nota](#Note) relativa all'asse di rotazione).
Esempi:
...
Pannello azioni di Posizionamento: Angoli di Eulero ![](/images/ComboBox.svg) selezionato

**Posizionamento = [Posizione, Imbardata-Beccheggio-Rollio]**

La seconda forma di **Placement** fissa la posizione di un oggetto nello spazio con **Position** (come nella prima forma), ma descrive il suo orientamento con gli angoli di imbardata, beccheggio e rollio ([Yaw, Pitch, Roll](http://en.wikipedia.org/wiki/Yaw,_pitch,_and_roll)).
Questi angoli sono anche denominati [angoli di Eulero](http://it.wikipedia.org/wiki/Angoli_di_Eulero) o angoli **Tait-Bryan**.
...
PS: poiché la versione 0.17 introduce un nuovo oggetto Part, questo oggetto ha un suo Posizionamento e l'oggetto Posizionamento creato nell'oggetto Part viene incrementato con il Posizionamento di Part. [disponibile dalla versione 0.17](/Release_notes_0.17/it "Release notes 0.17/it")
...
Impostazione dell'intero Posizionamento in un'unica espressione

**NOTA:** è anche possibile *creare* oggetti Placement nelle espressioni. Consultare la pagina [Expressioni](/Expressions/it#Posizionamento "Expressions/it") per i dettagli.
...
* Questo tutorial: [Aeroplano](/index.php?title=Aereo/it&action=edit&redlink=1 "Aereo/it (page does not exist)") copre ampiamente i meccanismi di modifica del Posizionamento di un oggetto.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Placement/it&oldid=1562494>"