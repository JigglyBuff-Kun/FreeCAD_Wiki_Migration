---
title: Creare un Ambiente di lavoro
---
## Introduzione

Questa pagina da indicazioni su come aggiungere un nuovo ambiente di lavoro all'interfaccia di FreeCAD. Gli [Ambienti di Lavoro](/Workbenches/it "Workbenches/it") sono contenitori per i comandi di FreeCAD. Possono essere codificati in Python, in C++ o in un mix di entrambi, il che ha il vantaggio di unire la velocità del C++ alla flessibilità di Python. In tutti i casi, tuttavia, l'ambiente verrà avviato da un set di due file Python. Possono essere ambienti di lavoro "interni", inclusi nella distribuzione di FreeCAD, o ambienti di lavoro "esterni", distribuiti tramite l'[Addon Manager](/Std_AddonMgr/it "Std AddonMgr/it") o installati manualmente scaricandoli da qualche repository online. Gli Ambienti di lavoro interni possono essere codificati in C++, Python o una combinazione dei due, mentre i workbench esterni devono essere solo in Python.
...
La cartella utente Mod è una sottocartella della cartella dei dati dell'applicazione utente (si può trovare quest'ultima digitando `App.getUserAppDataDir()` nella [console Python](/Python_console/it "Python console/it")):
...
Si può scegliere qualsiasi licenza che si desidera per il proprio workbench, ma si tenga presente che se ad un certo punto si vuole vedere il proprio workbench integrato e distribuito con il codice sorgente di FreeCAD, deve essere LGPL2+ come nell'esempio sopra. Vedere la [Licenza](/License/it "License/it").

Le funzioni `FreeCAD.addImportType()` e `addEXportType()` consentono di fornire il nome e l'estensione di un tipo di file e un modulo Python responsabile della sua importazione. Nell'esempio sopra, un modulo `importOwn.py` gestirà i file `.own`. Vedere [Frammenti di codice](/Code_snippets/it "Code snippets/it") per altri esempi.
...
* Si deve compilare il plug-in Qt designer che consente di aggiungere le impostazioni delle preferenze con [Qt Designer](/Compile_on_Linux/it#Plug-in_Qt_designer "Compile on Linux/it")
...
Per distribuire il proprio workbench Python, si può semplicemente ospitare i file in una posizione e istruire gli utenti a scaricarli e inserirli manualmente nella loro directory Mod, oppure si possono ospitare in un repository git online (GitHub, GitLab, Framagit e Debian Salsa sono posizioni attualmente supportate) e configurarle per l'installazione con l'[Addon Manager](/Std_AddonMgr/it "Std AddonMgr/it"). Le istruzioni per l'inclusione nell'elenco dei componenti aggiuntivi ufficiali di FreeCAD sono disponibili nel [repository GitHub dei componenti aggiuntivi di FreeCAD](https://github.com/FreeCAD/FreeCAD-addons/blob/master/README.md). Per utilizzare Addon Manager, è necessario includere un [package.xml metadata file](/Package_Metadata/it "Package Metadata/it"), che istruisce Addon Manager su come trovare l'icona del proprio ambiente e consente la visualizzazione di una descrizione, numero di versione, ecc. Può anche essere utilizzato per specificare altri workbench o pacchetti Python da cui dipende l'ambiente in cui è bloccato o che è destinato a sostituire.

Per una guida rapida su come creare un file package.xml di base e aggiungere un workbench all'[Addon Manager](/Std_AddonMgr/it "Std AddonMgr/it") vedere: [Aggiungere Workbench in Addon Manager](/Add_Workbench_to_Addon_Manager/it "Add Workbench to Addon Manager/it").
...
* [Traduzione di un ambiente di lavoro esterno](/Translating_an_external_workbench/it "Translating an external workbench/it")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Workbench_creation/it&oldid=1495264>"