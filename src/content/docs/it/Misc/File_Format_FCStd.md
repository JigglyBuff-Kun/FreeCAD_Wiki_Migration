...
Se FreeCAD viene eseguito in modalità console (senza GUI), viene utilizzato solo questo Document.xml.
...
Per ogni oggetto descritto nel Document.xml, c'è un corrispondente oggetto in GuiDocument.xml il quale descrive la rappresentazione visiva di quell'oggetto (colore, larghezza di linea, ecc.)
...
Queste sono le forme [B-rep](http://en.wikipedia.org/wiki/Boundary_representation "wikipedia:Boundary representation") di tutti gli oggetti che hanno una forma Parte in Document.xml.
...
Per incorporare altri tipi di file all'interno di un file FCStd, bisogna prima creare un [oggetto da script](/Scripted_objects/it "Scripted objects/it") dalla [console Python](/Python_console/it "Python console/it") e assegnargli una proprietà `App::PropertyFileIncluded`.

Quindi nell'[editor delle proprietà](/Property_editor/it "Property editor/it") si può andare alla proprietà aggiunta e scegliere un file nel computer. Una volta salvato il file FCStd, il file assegnato alla proprietà Dati**PropertyFileIncluded** verrà compresso all'interno di `.FCStd`. Quando il documento viene ripristinato, lo stesso file verrà ripristinato con la proprietà Dati**PropertyFileIncluded**.
...
* Vedere [Utilità di progetto](/Std_ProjectUtil/it "Std ProjectUtil/it").

Retrieved from "<http://wiki.freecad.org/index.php?title=File_Format_FCStd/it&oldid=1409710>"