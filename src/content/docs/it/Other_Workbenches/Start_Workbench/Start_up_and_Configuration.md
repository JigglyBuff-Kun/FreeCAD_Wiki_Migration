...
---
:::caution
In FreeCAD versione 0.20 la posizione predefinita dei file di configurazione, dati e cache del programma è stata modificata per Linux.VedereNote sulla versione 0.20per ulteriori informazioni. Questa pagina non è stata ancora aggiornata di conseguenza.
:::
...
| Opzione lunga | [Nome della variabile di configurazione](#Set_di_configurazione) corrispondente | Sinossi |
| --- | --- | --- |
...
Di solito FreeCAD è costruito con due file eseguibili: uno con l'interfaccia grafica chiamato FreeCAD o freecad e uno headless chiamato FreeCADCmd o freecadcmd.
FreeCAD può essere utilizzato in modalità console utilizzando l'opzione `--console` (che è il comportamento predefinito di FreeCADCmd):
...
Per ulteriori informazioni sulla modalità console o headless, fare riferimento a [FreeCAD Headless](/index.php?title=Headless_FreeCAD/it&action=edit&redlink=1 "Headless FreeCAD/it (page does not exist)").
...
Vedere [Eseguire automaticamente una macro all'avvio](/Macro_at_Startup/it "Macro at Startup/it") su come impostare una macro da eseguire automaticamente all'avvio di FreeCAD.

## Variabili d'ambiente

FreeCAD supporta le seguenti variabili di ambiente, che possono essere utilizzate per configurare le directory: [disponibile dalla versione 0.19](/Release_notes_0.19/it "Release notes 0.19/it")

| Variabile d'ambiente | Corrispondente [nome della variabile di configurazione](#Set_di_configurazione) | Sinossi |
| --- | --- | --- |
...
Le suddette variabili d'ambiente sono pensate per essere usate per realizzare un ambiente FreeCAD *portatile*. Per un esempio su come utilizzare le variabili di ambiente dalla riga di comando su Linux, fare riferimento a [note per gli utenti Linux sulla pagina dei download](/Download/it#Note_per_gli_utenti_GNU/Linux "Download/it").

### `HOME`

FreeCAD utilizza [Qt](/Third_Party_Libraries/it#Qt "Third Party Libraries/it"), che rispetta la variabile ambientale `HOME`. Pertanto, l'impostazione `HOME` può essere utilizzata per specificare la directory di base dei file di configurazione relativi a Qt (`.config/FreeCAD/FreeCAD.conf`).
...
| --- | --- | --- | --- |
| UserAppData | Percorso dove FreeCAD archivia i dati dell'applicazione relativi all'utente. | C:\Documents and Settings\username\AppData\FreeCAD  ---  *Percorso breve :* %APPDATA%\FreeCAD | /home/username/.config/.FreeCAD  ---  *Percorso breve :* ~/.FreeCAD |
| UserParameter | File in cui FreeCAD archivia i dati dell'applicazione relativi all'utente. | C:\Documents and Settings\username\AppData\FreeCAD\user.cfg  ---  *Percorso breve :* %APPDATA%\FreeCAD\user.cfg | /home/username/.FreeCAD/user.cfg  ---  *Percorso breve :* ~/.config/FreeCAD/user.cfg or $HOME/.config/.FreeCAD/user.cfg |
| SystemParameter | File in cui FreeCAD archivia i dati relativi all'applicazione. | C:\Documents and Settings\username\AppData\FreeCAD\system.cfg  ---  *Percorso breve :* %APPDATA%\FreeCAD\system.cfg | /home/username/.config/.FreeCAD/system.cfg  ---  *Percorso breve :* ~/.FreeCAD/system.cfg or $HOME/.FreeCAD/system.cfg |
| UserHomePath | Percorso home dell'utente corrente | C:\Documents and Settings\username  ---  *Percorso breve :* %USERPROFILE% | /home/username  ---  *Percorso breve :* ~ |

Nota: per le distribuzioni Linux, un file di configurazione aggiuntivo relativo a [Qt](/Third_Party_Tools/it#Qt-Toolkit "Third Party Tools/it") può esistere nel percorso /home/username/.config/FreeCAD/FreeCAD.conf.
...
| --- | --- | --- | --- |
| AppHomePath | Percorso in cui è installato FreeCAD | c:/Progam Files/FreeCAD\_0.19 | /user/local/FreeCAD\_0.19 |
...
| AppTempPath | Percorso della directory temporanea. Può essere fornito con la variabile d'ambiente `TMPDIR` o con  [Modifica parametri](/Std_DlgParameter/it "Std DlgParameter/it"): **Strumenti → Modifica parametri... → BaseApp → Preferences → General → TempPath** |  | /tmp/ (predefinito) |
...
Queste voci di configurazione sono legate al meccanismo di marchiatura di FreeCAD. Vedere [Marchiatura](/Branding/it "Branding/it") per maggiori dettagli.
...
Le voci del set di configurazione possono essere interrogate con il **config var name** (vedi tabelle sopra) dalla [console Python](/Python_console/it "Python console/it"). Per esempio:
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Start_up_and_Configuration/it&oldid=1501501>"