...
| 0.17                                                                                                                                                                                |
| Vedere anche                                                                                                                                                                        |
| [Reinforcement](/Reinforcement_Workbench/it "Reinforcement Workbench/it"), [Armatura](/Arch_Rebar/it "Arch Rebar/it"), [Distinta dei ferri](/Arch_Rebar_BOM/it "Arch Rebar BOM/it") |
|                                                                                                                                                                                     |

## Descrizione

Lo strumento [Armatura diritta](/Arch_Rebar_Straight/it "Arch Rebar Straight/it") consente all'utente di creare un set di staffe d'armatura all'interno di un oggetto [Struttura](/Arch_Structure/it "Arch Structure/it").
...
Due serie di barre di rinforzo diritte all'interno di una [Struttura](/Arch_Structure/it "Arch Structure/it")

## Utilizzo

1. Selezionare una qualsiasi faccia dell'oggetto ![](/images/Arch_Structure.svg) [Struttura](/Arch_Structure/it "Arch Structure/it").
2. Quindi selezionare ![](/images/Arch_Rebar_Straight.svg) [Armatura diritta](/Arch_Rebar_Straight/it "Arch Rebar Straight/it") dagli strumenti dell'armatura.
...
7. Se è selezionato Passo, l'utente può anche optare per [Passo personalizzato](/Custom_Spacing/it "Custom Spacing/it").
8. Usa lato selezionato viene utilizzato per verificare o modificare la faccia per la distribuzione dell'armatura.
...
- Dati**Orientamento**: Decide l'orientamento dell'armatura (es. in basso, in alto, a destra e a sinistra).
- Dati**Copriferro anteriore**: La distanza tra l'armatura e la faccia selezionata.
- Dati**Copriferro lato destro**: La distanza tra l'estremità destra dell'armatura e la faccia destra della struttura.
- Dati**Copriferro lato sinistro**: La distanza tra l'estremità sinistra dell'armatura e la faccia sinistra della struttura.
- Dati**Copriferro in direzione**: Queste proprietà consentono all'utente di specificare il copriferro superiore o inferiore.
- Dati**Copriferro inferiore** : La distanza tra l'armatura e la faccia inferiore della struttura.
- Dati**Copriferro superiore**: La distanza tra l'armatura e la faccia superiore della struttura.
- Dati**Quantità**: La quantità di armature.
- Dati**Passo**: La distanza tra gli assi di ciascuna barra.
...
Lo strumento Armatura diritta può essere usato nelle [macro](/Macros/it "Macros/it") e dalla console [Python](/Python/it "Python/it") utilizzando la seguente funzione:
...
- `f_cover`, `coverAlong`, `rt_cover`, e `lb_cover` sono le distanze di offset interne per gli elementi dell'armatura rispetto alle facce della struttura.
  - `f_cover` è l'offset del copriferro frontale.
  - `coverAlong` è una tupla `(position, value)` che definisce il valore di offset in una posizione (top, bottom, left, right) che dipende da `orientation`.
  - `rt_cover` è l'offset del copriferro destro o superiore, a seconda del valore di `coverAlong` e `orientation`.
  - `lb_cover` è l'offset del copriferro sinistro o inferiore, a seconda del valore di `coverAlong` e `orientation`.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Reinforcement_StraightRebar/it&oldid=1433688>"
