---
title: Distinta e sagomatura dei ferri
---
|  |
| --- |
| Distinta e sagomatura dei ferri |
| Posizione nel menu |
...
| 0.19 |
| Vedere anche |
| [Distinta dei ferri](/Arch_Rebar_BOM/it "Arch Rebar BOM/it"), [Disegno\_dell'armatura](/Arch_Rebar_Drawing_Dimensioning/it#Disegno_dell'armatura "Arch Rebar Drawing Dimensioning/it") |
|  |
...
* Dati**Reinforcement Group By**: Reinforcement Group By specifies how reinforcement objects should be grouped in Bar Bending Schedule i.e. "Host" or "Mark".
* Dati**Rebar Length Type**: Rebar Length Type specifies the type of rebar length used for BOM calculations i.e. "RealLength" or "LengthWithSharpEdges".
* Dati**Column Headers**: A dictionary with column\_data as key and tuple (column\_display\_header, column\_sequence) as value.
* Dati**Column Units**: A dictionary with keys: "Diameter", "RebarLength", "RebarsTotalLength" and their corresponding units as value.
* Dati**Font Family**: Font Family of text in Bar Bending Schedule SVG.
* Dati**Font Size**: Font size in mm.
* Dati**Column Width**: Width of each column in Bar Bending Schedule SVG.
* Dati**Row Height**: Height of each row in Bar Bending Schedule SVG.
* Dati**SVG Output File**: The output file to write generated rebar shape cut list SVG.

**Rebar Shape column data:** The data related to the Rebar Shape column in Bar Bending Schedule

* Dati**Column Header**: The column header for the rebar shape column.
* Dati**Stirrup Extended Edge Offset**: The offset of extended end edges of the stirrup, so that end edges of the stirrup with a 90-degree bent angle do not overlap with stirrup edges.
* Dati**Rebars Stroke Width**: The stroke-width of rebars in the rebar shape column.
* Dati**Rebars Color Style**: The color style of rebars.

**Rebar Shape Column Dimension Data:** The data related to rebar shape dimensions in the Rebar Shape column

* Dati**Include Dimensions**: If True, then each rebar edge dimensions and bent angle dimensions will be included in the rebar shape cut list.
* Dati**Include Units in Dimension Label**: If it is True, then rebar edge length units will be shown in dimension label.
* Dati**Rebar Edge Dimension Units**: The units to be used for rebar edge length dimensions.
* Dati**Rebar Edge Dimension Precision**: The number of decimals that should be shown for rebar edge length as a dimension label.
* Dati**Dimension Font Family**: The font-family of dimension text.
* Dati**Dimension Font Size**: The font-size of dimension text.
* Dati**Bent Angle Dimension Exclude List**: The list of bent angles to not include their dimensions.
* Dati**Helical Rebar Dimension Label Format**: The format of the helical rebar dimension label. e.g. "%L,r=%R,pitch=%P" where %L -> Length of helical rebar, %R -> Helix radius of helical rebar, %P -> Helix pitch of helical rebar.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Reinforcement_BarBendingSchedule/it&oldid=1372673>"