...
Questa pagina raccoglie le informazioni sui risolutori di elementi finiti utilizzati dall'ambiente [FEM](/FEM_Workbench "FEM Workbench"). L'interfaccia tra un solutore e FreeCAD in pre-elaborazione e post-elaborazione viene eseguita tramite file di testo. Ciò significa che in teoria qualsiasi solutore che può essere configurato e controllato tramite un file di testo è in grado di lavorare con FreeCAD; affinché questa comunicazione funzioni devono essere programmati un parser e uno scrittore appropriati dei file di input e di output.
...
Il repository [FreeCAD-dependencies](https://github.com/luzpaz/FreeCAD-dependencies) tiene traccia delle dipendenze di FreeCAD su molte distribuzioni Linux. La pagina [FEM.md](https://github.com/luzpaz/FreeCAD-dependencies/blob/master/FC-Workbenches/FEM.md) esamina i risolutori FEA open source che potrebbero essere utilizzati con [FEM](/FEM_Workbench/it "FEM Workbench/it"). La pagina mostra la versione di un particolare solutore nel repository di una particolare distribuzione Linux. Questa informazione è utile per sapere se un risolutore è aggiornato o non lo è e deve essere aggiornato.
...
Questo è il primo solutore che è stato integrato per funzionare con FEM. CalculiX è progettato principalmente per analisi statiche, termomeccaniche e modali. Ulteriori informazioni su questo risolutore sono disponibili in [FEM CalculiX](/FEM_CalculiX/it "FEM CalculiX/it").

### Elmer

Il solutore multifisico Elmer è stato integrato in FreeCAD come progetto di [Google Summer of Code 2017](/Google_Summer_of_Code_2017 "Google Summer of Code 2017"): [main website](https://www.csc.fi/web/elmer), [community portal](http://www.elmerfem.org./), [code repository](https://github.com/ElmerCSC/elmerfem), [Elmer Integration (GSoC) - Activity Log](https://forum.freecadweb.org/viewtopic.php?f=18&t=22576) (discussione nel forum).
...
Questi solutori non sono integrati nell'ambiente [FEM](/FEM_Workbench/it "FEM Workbench/it"), il che significa che hanno bisogno di un'interfaccia separata per impostare un progetto di simulazione. Ciò si ottiene attraverso le [macro](/Macros/it "Macros/it") o gli [ambienti esterni](/External_workbenches/it "External workbenches/it").
...
DesignSPHysics può essere installato tramite [Addon manager](/Std_AddonMgr/it "Std AddonMgr/it").
...
[EM](/EM_Workbench/it "EM Workbench/it") è un ambiente di lavoro esterno che è stato creato per servire da front-end per questi risolutori elettromagnetici. FastHenry è completamente supportato per l'analisi di impedenza magneto-quasistatica 3D, mentre FasterCap è accessibile attraverso alcune macro Python.
...
L'ambiente EM può essere installato tramite [Addon manager](/Std_AddonMgr/it "Std AddonMgr/it").
...
fcFEM è impacchettato come una libreria python e una macro e può essere scaricato dal [github repository](https://github.com/HarryvL/fcFEM). Alla fine sarà disponibile dal [Addon manager](/Std_AddonMgr/it "Std AddonMgr/it"), o sarà distribuito come parte di FreeCAD stesso.
...
Entrambi gli ambienti possono essere installati tramite [Addon manager](/Std_AddonMgr/it "Std AddonMgr/it"), ed entrambi hanno un posto per la discussione nel subforum [CfdOF / CFD](https://forum.freecadweb.org/viewforum.php?f=37).
...
[FenicsSolver](https://github.com/qingfengxia/FenicsSolver) è una piattaforma di simulazione per affrontare problemi multi-corpo, multi-fisica (accoppiati) e multi-scala. Si spera di integrare il solutore FEniCS sia in [FEM](/FEM_Workbench/it "FEM Workbench/it") che nell'[ambiente esterno](/External_workbenches/it "External workbenches/it") Cfd , quindi il sistema risultante funziona come un'alternativa gratuita a Comsol o Moose. FenicsSolver è sviluppato dallo stesso autore di Cfd.
...
[Frame3DD](http://frame3dd.sourceforge.net/) è un pacchetto software per l'analisi strutturale statica e dinamica di strutture 2D e 3D, [repository principale](https://github.com/pslack/frame3dd). Un lettore preliminare per i file di input è stato annunciato nei forum:
[Test read data from Frame3DD file.](https://forum.freecadweb.org/viewtopic.php?f=24&t=19428) Argomento generale del forum in FEM
...
È stato suggerito di integrare questa libreria in FreeCAD come parte di un [progetto](https://forum.freecadweb.org/viewtopic.php?f=8&t=35493) [Google Summer of Code](/Google_Summer_of_Code "Google Summer of Code").
...
Retrieved from "<http://wiki.freecad.org/index.php?title=FEM_Solver/it&oldid=1351069>"