...
Ci sono diverse possibilità di creare una mesh nell'ambiente [FEM](/FEM_Workbench/it "FEM Workbench/it"):

- Lo strumento [Gmsh](/FEM_MeshGmshFromShape/it "FEM MeshGmshFromShape/it") dall'interfaccia grafica utente.
- Lo strumento [Netgen](/FEM_MeshNetgenFromShape/it "FEM MeshNetgenFromShape/it") dall'interfaccia grafica utente.
...
- Creazione manuale della mesh tramite uno script [Python](/Python/it "Python/it").

Gli strumenti Gmsh e Netgen supportano i corpi mesh creati in [Part](/Part_Workbench/it "Part Workbench/it") e [PartDesign](/PartDesign_Workbench/it "PartDesign Workbench/it"), nonché le copie semplici di questi solidi. In generale, qualsiasi ambiente di lavoro che genera oggetti solidi, come [Arch](/Arch_Workbench "Arch Workbench"), può essere utilizzato come base da cui creare delle mesh. Notare che una mesh utilizzata per FEA è diversa da una mesh creata o importata dall'ambiente [Mesh](/Mesh_Workbench/it "Mesh Workbench/it").
...
Gli strumenti [Gmsh](/FEM_MeshGmshFromShape/it "FEM MeshGmshFromShape/it") e [Netgen](/FEM_MeshNetgenFromShape/it "FEM MeshNetgenFromShape/it") sono strumenti utili per rendere rapidamente mesh un corpo, ma non espongono le funzionalità complete di questi programmi; essi normalmente creano mesh triangolari, che potrebbero non essere l'ideale per alcuni tipi di analisi. Se si vuole avere più controllo sulla mesh creata (usare solo quadrilateri, numero e dimensione di elementi precisi, risoluzione variabile della mesh, ecc.), si dovrebbe usare questi programmi esternamente, produrre un file mesh in un formato supportato ( .inp, .unv, .vtk, .z88) e importare questo file in FreeCAD.

Precedentemente, Netgen era incluso in FreeCAD e poteva essere utilizzato immediatamente. Ora, sia Netgen che Gmsh devono essere installati prima di poter essere utilizzati da [FEM](/FEM_Workbench/it "FEM Workbench/it"). Per le istruzioni fare riferimento a [Installare FEM](/FEM_Install/it "FEM Install/it").
...
* ![](/images/Edit_Cancel.svg) "-" significa che le specifiche del formato non supportano questo tipo di elemento, quindi FreeCAD non può supportarlo.

- "?" significa che non è noto se il formato supporta questo tipo di elemento.

## Tipi di elementi FEM

Maggiori informazioni sui tipi di elementi FEM e la loro struttura dei dati all'interno di FreeCAD si trovano nella pagina [Tipi di elementi FEM](/FEM_Element_Types/it "FEM Element Types/it").
...
_Vedere anche:_ [Script di base per FreeCAD](/FreeCAD_Scripting_Basics/it "FreeCAD Scripting Basics/it").
...
Raddoppiare il fattore dello spostamento mostrato (_Nota per i redattori: rimosso nelle versioni più recenti?_)
...
Retrieved from "<http://wiki.freecad.org/index.php?title=FEM_Mesh/it&oldid=1456914>"
