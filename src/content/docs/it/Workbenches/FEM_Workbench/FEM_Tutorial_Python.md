...
Questo tutorial ha lo scopo di mostrare come una semplice analisi agli elementi finiti ("FEA"*) nell'[Ambiente FEM](/FEM_Workbench/it "FEM Workbench/it") ![](/images/Workbench_FEM.svg) di FreeCAD viene eseguita utilizzando Python. Per questo esempio verrà utilizzato il modello del tutorial [FEM CalculiX Trave a sbalzo 3D](/FEM_CalculiX_Cantilever_3D/it "FEM CalculiX Cantilever 3D/it").*
...
* Un ambiente FEM funzionante. Eseguire l'analisi [FEM CalculiX Trave a sbalzo 3D](/FEM_CalculiX_Cantilever_3D/it "FEM CalculiX Cantilever 3D/it") per verificarlo.
...
**Nota:** Controllare la sezione [Informazioni aggiuntive](#Informazioni_aggiuntive) di seguito per sapere come creare uno script per la generazione di mesh con GMSH o oggetti mesh Netgen.
...
Per eseguire l'analisi con Python, è necessario creare un'istanza del modulo `ccxtools`. Una volta che questa istanza è stata creata, ci sono due modi per eseguire l'analisi:.

1. Eseguire tutti i processi contemporaneamente (vedere la sottostante sezione ["Tutto in uno"](#Eseguire_l.27analisi_tutto_in_uno))
2. Eseguire i processi uno dopo l'altro (vedere la sottostante sezione ["Passo a passo"](#Passo_a_passo))

Se nel documento è presente una sola analisi e un solo risolutore nell'analisi, non è necessario passare alcun oggetto a `fea init`. Il metodo `init` della classe `fea` attiverà l'analisi **se la GUI è attiva**.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=FEM_Tutorial_Python/it&oldid=1251687>"