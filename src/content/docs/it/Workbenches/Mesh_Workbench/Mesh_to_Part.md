...
Convertire gli oggetti mesh in oggetti di livello superiore, come sono gli oggetti gestiti dal [Ambiente Parte](/Part_Workbench/it "Part Workbench/it") di FreeCAD non è un'operazione facile. L'oggetto Mesh può contenere migliaia di triangoli (per esempio quando è generato da uno scanner 3D), e manipolare solidi costituiti dallo stesso numero di facce sarebbe estremamente difficile. Quindi, in genere, si desidera ottimizzare l'oggetto durante la conversione.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Mesh_to_Part/it&oldid=1192607>"