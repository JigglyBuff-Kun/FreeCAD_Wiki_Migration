---
title: Strumenti Modifica parametri
---
|  |
| --- |
| Modifica parametri |
...
Lavorare con l'Editor dei parametri richiede una certa esperienza. Per i parametri più comuni al suo posto dovrebbe essere utilizzato l'[Editor delle preferenze](/Preferences_Editor/it "Preferences Editor/it").
...
1. Selezionare l'opzione **Strumenti → ![](/images/Std_DlgParameter.svg) Modifica parametri...** dal menu.
2. Viene visualizzata la finestra di dialogo **Editor parametri**. Per ulteriori informazioni, vedere le [Opzioni](#Opzioni).
...
4. Premere il pulsante Chiudi per chiudere la finestra di dialogo ed ultimare il comando.
...
2. I passaggi successivi sono simili a quelli per un [Nuovo elemento string](#Nuovo_elemento_string)

#### Nuovo elemento integer

1. Seleziona l'opzione **Nuovo elemento integer** o **Nuovo → Nuovo elemento integer** dal menu contestuale.
2. I passaggi successivi sono simili a quelli per un [Nuovo elemento string](#New_string_item/it)
...
4. Selezionare una o più caselle di controllo **Gruppi**, **Nomi** e **Valori**. Si noti che verranno cercati solo i valori stringa.
5. Facoltativamente (de)selezionare la casella di controllo **Controlla solo stringa intera**.
6. Premere il pulsante Trova successivo per selezionare il primo gruppo con una corrispondenza. I parametri corrispondenti non sono evidenziati individualmente. Se lo si desidera, ripetere l'operazione fino a quando non si trovano ulteriori corrispondenze.
...
* La pagina [Ottimizzare l'installazione](/Fine-tuning/it "Fine-tuning/it") elenca una serie di parametri che potrebbero essere di interesse.

## Script

Vedere anche: [Autogenerated API documentation](https://freecad.github.io/SourceDoc/) e [Script di base per FreeCAD](/FreeCAD_Scripting_Basics/it "FreeCAD Scripting Basics/it").

È possibile accedere alle preferenze dagli script Python utilizzando il percorso corrispondente in Modifica parametri. Ad esempio, la preferenza **Modifica → Preferenze → Importa-Esporta → DXF → Opzioni di importazione → Unisci geometria** appare in **Strumenti → Modifica parametri → BaseApp → Preferences → Mod → Draft → dxfCreatePart** che ha impostazione `Boolean`. È quindi possibile accedervi in ​​Python utilizzando il seguente codice:
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Std_DlgParameter/it&oldid=1459045>"