...
| Struttura                                                                        |
| Posizione nel menu                                                               |
| Arch → Strumenti di Struttura → Struttura                                        |
| Ambiente                                                                         |
...
| -                                                                                |
| Vedere anche                                                                     |
| [Muro](/Arch_Wall/it "Arch Wall/it"), [Armature](/Arch_Rebar/it "Arch Rebar/it") |
|                                                                                  |
...
- Premere il pulsante ![](/images/Arch_Structure.svg) Struttura, oppure premere i tasti S e poi T
...
- Gli elementi strutturali condividono le proprietà e i comportamenti comuni di tutti i [Componenti Arch](/Arch_Component/it "Arch Component/it")
...
- Dati**Tool**: un percorso di estrusione opzionale, che può essere qualsiasi tipo di linea. Se questa proprietà è vuota, l'estrusione è diritta e viene creata nella direzione indicata dalla proprietà Normal
- Dati**Normal**: specifica la direzione in cui la faccia di base di questa struttura è estrusa. Se questa proprietà viene mantenuta su (0,0,0), la direzione è automaticamente impostata sulla direzione normale della faccia di base.
- Dati**Face Maker**: specifica il tipo di algoritmo di generazione della faccia da utilizzare per creare il profilo. Le scelte sono None, Simple, Cheese e Bullseye.
- Dati**Length**: la lunghezza della struttura (utilizzata solo se non si basa su un profilo)
- Dati**Width**: la larghezza della struttura (utilizzata solo se non si basa su un profilo)
- Dati**Height**: l'altezza della struttura (o la lunghezza di estrusione quando si basa su un profilo). Se l'altezza non è data, e la struttura viene inserita in un oggetto [Piano](/Arch_Floor/it "Arch Floor/it") la cui altezza è definita, la struttura assume automaticamente il valore dell'altezza del piano.
- Dati**Nodes Offset**: un offset opzionale tra la linea centrale e la linea dei nodi

### Vista

- Vista**Nodes Type**: il tipo di nodi strutturali di questo oggetto, lineare o area.
- Vista**Show Nodes**: mostra o nasconde i nodi strutturali.
...
- Quando si modifica un oggetto Struttura (con doppio clic), nella vista Azioni sono disponibili alcuni strumenti nodo per:
  ...
  - Modificare i nodi graficamente, che funziona nello stesso modo di [Modifica di Draft](/Draft_Edit/it "Draft Edit/it")
...
Lo strumento Struttura può può essere utilizzato nelle [macro](/Macros/it "Macros/it") e dalla [Python](/Python/it "Python/it") console utilizzando la seguente funzione:
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Structure/it&oldid=1539771>"
