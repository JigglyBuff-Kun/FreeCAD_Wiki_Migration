...
BIM înseamnă [Building Information Modeling](https://en.wikipedia.org/wiki/Building_information_modeling).
...
- În **snapping toolbar**, asigurați-vă că butonul ![](/images/Draft_Snap_Grid.svg) [grid snap](/Draft_ToggleGrid "Draft ToggleGrid") este activat , astfel ca noi să putem utiliza grila cât mai mult posibil.
- Definiți planul ca fiind **XY** [Working Plane](/Draft_SelectPlane "Draft SelectPlane")
- Desenați patru linii cu instrumentul ![](/images/Draft_Line.svg) [Draft Line](/Draft_Line "Draft Line") Puteți introduce coordonatele manual sau pur și simplu alegeți punctele de pe grilă cu mouse-ul:
...
- Selectați prima linie, apoi apăsați butonul ![](/images/Arch_Wall.png) [Wall](/Arch_Wall "Arch Wall") .
...
- Apăsați butonul ![](/images/Arch_Add.png) [Add](/Arch_Add "Arch Add"). Cei patru pereții sunt acum transformați într-unul singur:

![](/images/Exercise_arch_05.jpg)

The individual walls are however still accessible, by expanding the wall in the tree view.

- Let's now place a door. In FreeCAD, doors are considered a special case of windows, so this is done using the [Window](/Arch_Window "Arch Window") tool.
...
- Set the [Working Plane](/Draft_SelectPlane "Draft SelectPlane") to **auto** so we are not restricted to the ground plane
- Press the ![](/images/Arch_Window.png) [Window](/Arch_Window "Arch Window") button.
- In the window creation panel, select the **Simple door** preset, and set its **Width** to 0.9m and its **Height** to 2.1m
- Make sure the ![](/images/Draft_Snap_Near.svg) [Near snap](/Draft_Snap_Near "Draft Snap Near") location is turned on, so we can snap on faces
...
- Create a ![](/images/Draft_Rectangle.png) [rectangle](/Draft_Rectangle "Draft Rectangle") with a **length** of 5m, a height of **4m**, and place it at position x:-0.5m, y:-0.5m, z:0.
- Select the rectangle
- Click the ![](/images/Arch_Structure.png) [structure](/Arch_Structure "Arch Structure") tool to create a slab from the rectangle
...
- Press the ![](/images/Draft_SelectPlane.png) [working plane](/Draft_SelectPlane "Draft SelectPlane") button. The working plane is now set to that face.
- Create a ![](/images/Draft_Rectangle.png) [rectangle](/Draft_Rectangle "Draft Rectangle"), snapping to two opposite points of the border beams:
...
- Press the ![](/images/Arch_Floor.png) [Floor](/Arch_Floor "Arch Floor") button
- Select the floor we just created
- Press the ![](/images/Arch_Building.png) [Building](/Arch_Building "Arch Building") button
...
We will now place some dimensions. Unlike the [previous chapter](/Manual:Generating_2D_drawings "Manual:Generating 2D drawings"), where we drew all the dimensions directly on the Drawing sheet, we will use another method here, and place [Draft dimensions](/Draft_Dimension "Draft Dimension") directly in the 3D model. These dimensions will then be placed on the Drawing sheet automatically. We will first make two groups for our dimensions, one for the dimensions that will appear in the plan view, and another for those that appear on the elevation.
...
- [Atelierul Arch](/Arch_Workbench/ro "Arch Workbench/ro")
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Manual:BIM_modeling/ro&oldid=1522223>"
