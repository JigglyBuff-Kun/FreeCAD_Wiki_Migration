---
title: Tutorial de programare Python
---
## Introduction
...
* Din [FreeCAD python interpreter](/FreeCAD_Scripting_Basics "FreeCAD Scripting Basics"), unde puteți emite comenzi simple ca într-o interfață "command line"-style interface
* Din [macros](/Macros "Macros"), care sunt o modalitate convenabilă de a adăuga rapid o unealtă lipsă la interfața FreeCAD
* De la scripturi externe, care pot fi folosite pentru a programa lucruri mult mai complexe. ca întreg sistemul de ateliere [Workbenches](/Workbenches "Workbenches").

În acest tutorial, vom lucra pentru început pe câteva exemple simple, dar există, de asemenea, mult mai multă [documentation about python scripting](/Power_users_hub "Power users hub") disponibilă pe acest wiki. Dacă sunteți total novice în limbajul Python și doriți să înțelegeți cum funcționează, avem de asemenea o bază [introduction to Python](/Introduction_to_Python "Introduction to Python").
...
O altă caracteristică comună a obiectelor FreeCAD este [placement](/Placement "Placement"). Fiecare obiect are atribute de plasare, care conține poziția (baza) și orientarea (rotirea) obiectului. Este ușor de manipulat, de exemplu pentru a ne mișca obiectul:
...
[Sketcher](/Sketcher_Workbench "Sketcher Workbench") și [Draft](/Draft_Workbench "Draft Workbench") amândouă utilizează modulul [Part](/Part_Workbench "Part Workbench") pentru a crea și manipula geometria lor, care este Boundary Representation - BRep [Mesh](/Mesh_Workbench "Mesh Workbench") este total independentă și își ocupă propriile obiecte. Mai multe despre acestea sunt în cele de mai jos.
...
Careacteristicile FreeCAD features sunt în multe module, ca de exemplu [Sketcher](/Sketcher_Workbench "Sketcher Workbench") ori [Draft](/Draft_Workbench "Draft Workbench"),
...
[Draft Workbench](/Draft_Workbench "Draft Workbench") adaugă câteva tipuri de obiecte parametrice 2D parametric (care sunt toate obiecte Part) ca de exemplu linile și cercurile, și oferă, de asemenea, câteva funcții generice care funcționează nu numai pe obiectele create de Draft, ci și pe orice obiect Part. Pentru a explora ceea ce este disponibil, pur și simplu faceți:
...
Acum, că aveți o bună înțelegere a principiilor de bază, unde vom păstra scripturile noastre Python și cum le vom lansa cu ușurință de pe FreeCAD? Există un mecanism ușor pentru acesta, numit [Macros](/Macros "Macros"). O macrocomandă este pur și simplu un script python, care poate fi apoi adăugat la o bară de instrumente și poate fi lansat printr-un simplu click de mouse. FreeCAD vă oferă un editor simplu de text (Macro -> Macros -> Creare) unde puteți scrie sau lipi scripturi. După ce s-a terminat, Instrumentele -> Customize -> Macro-urile vă permit să definiți un buton pentru acesta, care poate fi adăugat la barele de instrumente.
...
Acum sunteți gata pentru mai multă programare/script FreeCAD în profunzime. Deplasați-vă la [Power users hub](/Power_users_hub "Power users hub")!

[Top](#top)

Retrieved from "<http://wiki.freecad.org/index.php?title=Python_scripting_tutorial/ro&oldid=1270951>"