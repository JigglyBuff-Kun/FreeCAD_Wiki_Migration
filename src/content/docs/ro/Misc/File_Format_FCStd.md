---
title: Formatul fișierului FCStd
---
## Overview
...
FCStd este un fișier standard tip fișier zip, conținând fișiere[one or more](#Contents) îmtr-o [Structure](#structure) specifică. Ca atare, este posibilă dezarhivarea unui fișier FCStd utilizând un instrument de decompresie zip obișnuit. FreeCAD conține un "Project Utility" pentru rearhivarea fișierelor FCStd, utilizara sa este descrisă mai jos în [#Change the source of the file .FCStd](#Change_the_source_of_the_file_.FCStd) .
...
These are the [B-rep](http://en.wikipedia.org/wiki/Boundary_representation "wikipedia:Boundary representation") shapes of all objects that have a Part shape in the Document.xml. Each object, even if it is parametric, has its shape stored as an individual .brep file, so it can be accessed by components without the need to recalculate the shape.

#### Șabloane/\*.svg

In the Templates folder are stored the template svg files used in [Drawing](/Drawing_Workbench "Drawing Workbench") pages.

These are the template svg files used in [TechDraw](/TechDraw_Workbench "TechDraw Workbench") pages.

### Structură

Structure of a typical FCStd file:

Structure of a typical .FCStd file. The extension can be changed to .zip to explore it like a normal directory. The Document.xml and GuiDocument.xml are at the root of the archive, together with any number of .brp (BREP) files. One subdirectory may hold the thumbnail, and another the SVG templates used by [TechDraw](/TechDraw_Workbench "TechDraw Workbench").
...
In order to embed other file types inside a FCStd file, you must first create a [scripted object](/Scripted_objects "Scripted objects") from the [Python console](/Python_console "Python console"), and give it an `App::PropertyFileIncluded` property.

Then in the [property editor](/Property_editor "Property editor") you can go to the added property and choose a file in the computer. Once the FCStd file is saved, the file assigned to the Date**PropertyFileIncluded** property will be packed inside the `.FCStd`. When the document is restored, the same file will be restored with the Date**PropertyFileIncluded** property.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=File_Format_FCStd/ro&oldid=1409715>"