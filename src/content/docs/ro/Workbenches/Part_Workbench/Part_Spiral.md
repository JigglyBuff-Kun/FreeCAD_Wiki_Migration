...
| Part Spiral |
| Menu location |
| [Part](/Part_Workbench "Part Workbench") → [Create Primitives](/Part_CreatePrimitives "Part CreatePrimitives") → Spiral |
| Workbenches |
...
| - |
| See also |
| [Create Primitives](/Part_CreatePrimitives "Part CreatePrimitives") |
|  |

## Descriere

Un element primitiv geometric este Spiral care este disponibil din dialogul Create Primitives în Atelierul de lucru Part.

A ![](/images/Part_Spiral.svg) **Part Spiral** is a parametric shape that can be created with the ![](/images/Part_Primitives.svg) [Part Primitives](/Part_Primitives "Part Primitives") command. In the coordinate system defined by its Date**Placement** property, the spiral lies on the XY plane with its center at the origin and its start point on the X axis. It widens as it turns counterclockwise.
...
* Date**Growth** (`Length`): The distance between two consecutive turns of the spiral. The default is `1mm`.
* Date**Radius** (`Length`): The start radius of the spiral, the distance between its center and its start point. Can be `0mm`. The default is `1mm`.
* Date**Rotations** (`QuantityConstraint`): The number of rotations, or turns, of the spiral. The default is `2`.
* Date**Segment Length** (`QuantityConstraint`): The number of turns per spiral subdivision. The default is `1`, meaning each full turn of the spiral is a separate segment. Use `0` to suppress subdivision.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Part_Spiral/ro&oldid=1106615>"