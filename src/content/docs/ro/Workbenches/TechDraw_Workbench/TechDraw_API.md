...
title: TechDraw API
---
Aceste funcții fac parte din [TechDraw Workbench](/TechDraw_Workbench "TechDraw Workbench") și pot fi utilizate în script-uri și [macro](/Macros "Macros") sau din interpretorul [Python](/Python "Python"), odată ce modulul  `TechDraw </ code> a fost importat. Exemple bune de programare-script de bază TechDraw pot fi găsite în unit test scripts.`

`Good examples of basic TechDraw scripting can be found in the unit test scripts.``A se vedea TechDrawGui API pentru mai mule funcții.``Exempluː`````
import FreeCAD
import TechDraw
...
`````![](/images/2/20/Method.png) EdgeWalker(listOfEdges, [bool])
...
![](/images/2/20/Method.png) findOuterWire(listOfEdges)

Description: Finds the OuterWire (largest) of a list of edges (that form a planar graph).

Returns: Outer wire

![](/images/2/20/Method.png) findShapeOutline(TopoShape, scale, direction)

Description: Project shape in direction and find outer wire of result.

Returns: Outline wire``![](/images/2/20/Method.png) viewPartAsDxf(DrawViewPart)
...
`````![](/images/2/20/Method.png) viewPartAsSvg(DrawViewPart)
...
`````![](/images/2/20/Method.png) writeDXFView(DrawViewPart, FileName)

Description: Save the DrawViewPart in Dxf.

Returns: File``Example:`````
import TechDraw
TechDraw.writeDXFView(myPart,myFileName)

`````![](/images/2/20/Method.png) writeDXFPage(DrawPage, FileName)
...
`````### DrawViewPart Cosmetics``#### CosmeticVertex (CV) routines accessible from Python``dvp = App.ActiveDocument.View #CV's belong to views.
...
`````#### CosmeticEdge (CE) routines accessible from Python``dvp = App.ActiveDocument.View #CE's belong to views.
...
`````#### CenterLine (CL) routines accessible from Python``Makes a new CenterLine  
...
`````### DrawViewPart Geometry``[topoShapeEdge] = dvp.getVisibleEdges()
...
Retrieved from "<http://wiki.freecad.org/index.php?title=TechDraw_API/ro&oldid=1298842>"