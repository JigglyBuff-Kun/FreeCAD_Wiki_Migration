...
The link operation changes the Date**MeasureType** property of the dimension from `Projected` to `True`.
...
There is currently no way to break a link; you can change the Date**MeasureType** back to `Projected` so that the dimension will use the projected value instead of the linked value.
...
The TechDraw LinkDimension tool is not directly usable in macros, but changing the Date**References 3D** property can accomplish the same result.

Retrieved from "<http://wiki.freecad.org/index.php?title=TechDraw_LinkDimension/ro&oldid=1486076>"