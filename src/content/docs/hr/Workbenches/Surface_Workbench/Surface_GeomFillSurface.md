---
title: Surface GeomFillSurface/hr
---
|  |
| --- |
...
* Podaci**Fill Type** (`Enumeration`): the applied filling algorithm; Stretch, the style with the flattest patches; [`Coons`](https://en.wikipedia.org/wiki/Coons_patch), a rounded style with less depth than Curved; Curved, the style with the most rounded patches.
* Podaci**Boundary List** (`LinkSubList`): a list of edges that will be used to build the surface.
* Podaci ((hidden))**Reversed List** (`BoolList`):
...
When the surface's Pregled**Lighting** property is `One side`, a face will be painted completely black if its normal direction points into the [3D view](/3D_view "3D view") (away from the current viewer), indicating a flipped face with respect to the other colored faces.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Surface_GeomFillSurface/hr&oldid=1463767>"