...
Може да намерите прозореца на настройките на FreeCAD в менюто Edit → Preferences.

FreeCAD е разделен на модули, всеки модул отговорен за един [работен плот](/Workbenches/bg "Workbenches/bg"). Също така FreeCAD зарежда всеки модул чак когато започнете да го използвате. Това включва и зареждане на настройките за модула чак когато започнете да го използвате.
...
Без да имате заредени плотове, ще имате достъп само до две секции с настройки - общи настройки на програмата, и настройки на изгледа (изборазяването на обекти).
...
Когато заредите FreeCAD в началото, и не сте отворили работен плот все още, то ще имате минимален брои секции за настройки в прозореца на настройките. Като започнете да използвате различни модули/работни плотове, нови секции ще се появяват в прозореца на настройките, които ще ви разрешават да конфигурирате поведението на всеки новозареден работен плот.
...
| **Two-side rendering** | If checked, the default value for the Вид**Lighting** property is `Two side` instead of `One side`. `Two side` means the color of the interior side of faces is the same as the color of the exterior side. `One side` means their color is either the [backlight color](#3D_View), if enabled, or black. |
| **Show bounding-box for highlighted or selected meshes** | If checked, the default value for the Вид**Selection Style** property is `BoundBox` instead of `Shape`. `BoundBox` means a highlighted bounding box is displayed if meshes are highlighted or selected. `Shape` means the shape itself is then highlighted. |
...
Модулът Draft има собствена секция с [настройки](/index.php?title=Draft_Preferences/bg&action=edit&redlink=1 "Draft Preferences/bg (page does not exist)").
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Preferences_Editor/bg&oldid=1542521>"