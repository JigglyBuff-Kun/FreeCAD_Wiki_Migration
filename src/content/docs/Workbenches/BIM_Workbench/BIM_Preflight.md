...
![](/src/assets/images/BIM_preflight_screenshot.png)
...
- **IFC4 support**: Tests if the IFC import/export system of FreeCAD is working correctly, and if the IFC4 format is available (which requires an up-to-date version of [IfcOpenShell](/Arch_IFC "Arch IFC") and enabled in the preferences.

### Project organization tests

- **Project hierarchy**: The IFC format requires at least one [building](/Arch_Building "Arch Building") and it is also a common practice to have one [site](/Arch_Site "Arch Site") and at least one [level (storey)](/Arch_BuildingPart "Arch BuildingPart"). This test checks if these are present in the model
- **Buildings**: Checks that all buildings found in the model are part of a [site](/Arch_Site "Arch Site")
- **Levels**: Checks that all [levels](/Arch_BuildingPart "Arch BuildingPart") (building storeys) found in the model are prt of a building
...
- **Explicit quantities**: Checks that all objects with length, width or height properties are [marked for explicit export](/BIM_IfcQuantities "BIM IfcQuantities") of these quantities
...
- **Standards**: Checks that all BIM objects have a [standard code](/BIM_Classification "BIM Classification")

### Compatibility tests

- **Extrusions**: Checks that all BIM objects are linear extrusions
- **Standard cases**: Checks that all [walls](/Arch_Wall "Arch Wall") and [structural elements](/Arch_Structure "Arch Structure") are standard cases, as defined by the IFC schema
...
Retrieved from "<http://wiki.freecad.org/index.php?title=BIM_Preflight/en&oldid=1498576>"
