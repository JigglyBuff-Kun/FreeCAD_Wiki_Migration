...
This is the in-game tutorial of the [BIM Workbench](/BIM_Workbench "BIM Workbench"). It is not meant to be read here on the wiki, but rather to be started from inside FreeCAD. After [activating](/Std_Workbench "Std Workbench") the BIM Workbench it can be found in the menu under **Help → BIM tutorial**. It includes a series of steps to be completed by the user. Each step is terminated by an instance of the [{{BIMTutorialAction|descr|goal1|test1|goal2|test2}}](/Template:BIMTutorialAction "Template:BIMTutorialAction") template, which informs of the condition that need to be met. Images should be 300px wide. No SVG images should be used on this page, as they are not supported by the QTextBrowser widget
...
This tutorial is written for the [most recent development version of FreeCAD available](/Download "Download") (currently 1.1.0dev). The BIM Workbench, however, is made to be compatible with any version of FreeCAD. If you are using a FreeCAD version older than the one indicated here, some BIM tools might look different, work differently or even be unavailable. Refer to the [documentation](/BIM_Workbench "BIM Workbench") to know more in case of doubt.
...
_Open the BIM setup screen_

True if hasattr(FreeCADGui,"BIMSetupDialog") else False

_Set preferred working units to centimeters and the default size of a grid square to 10 cm_
...
The [BIM Workbench](/BIM_Workbench "BIM Workbench") contains tools borrowed from other workbenches such as [Draft](/Draft_Workbench "Draft Workbench") or [Part](/Part_Workbench "Part Workbench"), as well as its own tools. These are organized in several categories. Each category has a menu and a toolbar. Take a moment to explore the contents of the menus described below.
...
These tools turn [snapping](/Draft_Snap "Draft Snap") positions on/off. Like in most BIM applications, each additional snapping position adds calculation time when drawing, so it is best to only keep the ones you need turned on.
...
Each tool contained in these menus has its own documentation page that describes in detail how it works and what options are available. They are listed on the [BIM Workbench documentation](/BIM_Workbench "BIM Workbench") page, which is also accessible from the **Help** menu, or by using menu **Help → What's this?** and clicking on any toolbar button.
...
There are many ways to create BIM objects in FreeCAD. You can use the native [BIM tools](/BIM_Workbench "BIM Workbench") from this workbench, or use any other FreeCAD tool from other [workbenches](/Workbenches "Workbenches"). Both the _2D Drafting_ and the _3D/BIM_ tools from this workbench, unlike other workbenches such as Part Design, make extensive use of **working planes** and **snapping**.

The [working plane](/Draft_SelectPlane "Draft SelectPlane") is where your next objects will be created. You can set it to one of the basic orthogonal planes (ground, front, side), or use any selected face to define the current working plane. You can also use [Working Plane Proxies](/Draft_WorkingPlaneProxy "Draft WorkingPlaneProxy") from menu **Utils** to store a specific working plane position inside your model. [Building Parts](/Arch_BuildingPart "Arch BuildingPart") also contain an implicit working plane position. Changing the current working plane is done by pressing the working plane button on the BIM toolbar. The **grid** always reflects where the working plane is.
...
_Set the working plane in "Top" (XY) mode_
...
Let's start building our pavilion by creating some walls. Walls can be made either directly with the [wall](/Arch_Wall "Arch Wall") tool, or by first drawing 2D objects such as [lines](/Draft_Line "Draft Line"), [wires](/Draft_Wire "Draft Wire") (polylines) or [sketches](/Sketcher_NewSketch "Sketcher NewSketch"), that will define the baseline of our walls. When you have such a baseline object selected, pressing the Wall tool will automatically convert it into a wall.
...
_Create two orthogonal wall objects_

bool(len([o for o in FreeCAD.ActiveDocument.Objects if "MakeBlocks" in o.PropertiesList]) == 2)

_Set their height to 2.50 meters and width to 20 centimeters_
...
Once our rectangular _profile_ is drawn, we can follow the same method as in method one to create a slab (select it, press the **Structure** button, adjust its properties).

**Tutorial steps**

_Create a rectangle_

bool(len([o for o in FreeCAD.ActiveDocument.Objects if "Rectangle" in o.Name]) == 1)

_Create a 20 cm thick slab_
...
Let's add a metallic column to give better support to our slab. Make sure the working plane is in _Top_ mode, let's start by putting ourselves in top view (press key **2**), and turn the slab off, so we see better what's underneath. Select the slab, and press the **Space** key to turn its display off.
...
_Create a CHS tubular column_

bool(len([o for o in FreeCAD.ActiveDocument.Objects if "CHS" in o.Label]) == 1)

_Add a 20 cm x 20 cm plate to the column_
...
_Add the four other root BIM objects to it_
...
IFC import/export operations in FreeCAD are performed by an external piece of software called [IfcOpenShell](http://www.ifcopenshell.org/). Read the [Arch IFC](/Arch_IFC "Arch IFC") page to learn more about how to install it.
...
_Open the BIM preflight tool and run all the tests_
...
Retrieved from "<http://wiki.freecad.org/index.php?title=BIM_ingame_tutorial/en&oldid=1551609>"
