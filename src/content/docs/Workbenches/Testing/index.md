...
The [Test Framework Workbench](/Testing "Testing") is not really a modelling workbench, but it contains a set of [Python](/Python "Python") scripts to perform different tests on the core components of FreeCAD, in order to debug problems. See also [debugging](/Debugging "Debugging").
...
- Code source is copied from post by FreeCAD forum user, _sgrogan_, in the [unit tests per python](https://forum.freecadweb.org/viewtopic.php?style=3&p=153251#p153251) topic, with credit there given to forum user, _wmayer_.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Testing/en&oldid=1404418>"
