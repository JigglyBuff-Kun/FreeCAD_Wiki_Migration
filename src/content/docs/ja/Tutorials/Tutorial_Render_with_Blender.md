---
title: Blenderを使ったレンダリングのチュートリアル
---
|  |
| --- |
| Tutorial |
...
このチュートリアルでは、[Blender](https://www.blender.org/)を使って、FreeCADで作成したパーツやアセンブリからレンダリング画像を作成する方法を紹介します。ユーザは既にFreeCADでパーツを作成あるいはインポートし終えている状態から始めて、それをレンダリングのためのBlenderにエクスポートします。

Bleander 2.80のEEVEEまたはCyclesエンジンを使ってレンダリング画像を生成します。以下では、FreeCADおよびBlenderの両方で、より早く作業を進めるために使うことのできる[Python](/Python "Python")コマンドについても示します。

Joko Engineeringによる一連の動画「
[Render Solidworks and FreeCAD Models in Blender](https://www.youtube.com/watch?v=U7e6-Wfv2b0)」で、同様の手順について説明しています。

## FreeCAD

1. [Partワークベンチ](/Part_Workbench "Part Workbench")または[PartDesignワークベンチ](/PartDesign_Workbench "PartDesign Workbench")で作った部品からなるアセンブリ、または、例えば[Archワークベンチ](/Arch_Workbench "Arch Workbench")などを使って、ソリッドなモデルを作成します。最終的なレンダリング画像でほしいものと概ね合致した色や材質を、個々の部品に設定します。

![](/images/01_T03_FreeCAD_Blender_model.png)

FreeCADで作成し、色及び材質を設定したアセンブリの例

2. もしモデルが非常に精細なものの場合は、物体のビュー**Deviation**を`0.1`から`0.01`の間、あるいはさらにそれよりも小さな値にします。小さな値を設定すると、エクスポートするメッシュがより細かくなり、レンダリング結果もより高品質になります。

![](/images/02_T03_FreeCAD_Blender_deviation.png)

FreeCADで作成した部品のDeviationプロパティー。十分な解像度を持たせるためには、この値を小さくする必要がります。

3. `Part`を選択してから、**ファイル → エクスポート**またはCtrl+Eで、`Wavefront OBJ`形式のファイルにエクスポートします。

あるいは、[Python](/Python "Python")コンソールからコマンドを入力してエクスポートすることも可能です。エクスポートするオブジェクトのリストを定義してから、エクスポートのための関数をファイル名を与えて呼び出します。
...
*注意：*OBJにエスクポートすると、ファイルが2つ作られます。1つ目のファイル（例えば`assembly.obj`）にはメッシュの情報が入っています。2つ目のファイル（例えば`assembly.mtl`）には材質の情報（ほとんどの場合は色の情報）が入っています。

*注意2：*もしOBJファイルが空の場合は、個々のボディーを個別にエクスポートする必要があります。この場合は、Partの下の個々のボディーを選択し、個別にエクスポートしてください。
...
4. Blenderを開きます。Shift+F4を押して、`タイムライン`パネルを`Pythonコンソール`に切り替えておきます。こうしておくと、コマンドを入力して結果を確認するのが簡単になります。このパネルを分割し、片側をコンソールに、もう片方を`情報`パネルにしてもいいでしょう。こうすると、GUI上の操作に対応するPythonコードを確認することができます。

EEVEEエンジンを使っていることを確認してください。`プロパティ`パネルの`レンダー`の中の`レンダーエンジン`を`Eevee`にしてくだい。
...
5. メニューから**ファイル → インポート → Wavefront (.obj)**を選んでモデルをインポートします。

あるいは、`Pythonコンソール`にコマンドを入力することでインポートすることもできます。
...
もしボディーが非常に大きく見える場合は、単位を変更し適切な縮尺で表示されるようにする必要があるかもしれません。

`プロパティ`パネルの`シーン`の中の`単位`で、`単位系Unit System`あるいは`単位の倍率`あるいは`長さ`を適切に設定します。

小さいものの場合は、長さを`ミリメートル`にするのがいいでしょう。建物のモデルと言った大きいものの場合は長さを`メートル`に倍率を`0.001`にするのがいいでしょう。適切な倍率をいろいろと試してみてください。

これも`Pythonコンソール`から変更できます。
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Tutorial_Render_with_Blender/ja&oldid=1541537>"