...
LibPackは上記のすべてのライブラリを一緒に梱包した便利なパッケージです。現在、Windowsプラットフォームでは[ダウンロード](/Download "Download")ページから利用可能です！あなたがLinuxで作業していれば、LibPackを必要としません。代わりにお使いのLinuxディストリビューションのパッケージリポジトリを利用するべきです。
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Third_Party_Libraries/ja&oldid=1457088>"