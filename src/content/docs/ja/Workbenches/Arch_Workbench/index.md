...
Arch ワークベンチは![](/images/Workbench_Draft.svg) [Draft ワークベンチ](/Draft_Workbench/ja "Draft Workbench/ja")のすべてのツールを受け継いでおり、2 次元のオブジェクトから 3 次元のパラメトリックな建築用オブジェクトを作成することが可能です。しかしそれだけではなく![](/images/Workbench_Part.svg) [Part](/Part_Workbench/ja "Part Workbench/ja")や![](/images/Workbench_PartDesign.svg) [PartDesign](/PartDesign_Workbench/ja "PartDesign Workbench/ja")といった他のワークベンチにより作成されたソリッドの図形を使うことも可能です。

FreeCAD の BIM 関連機能は徐々にこの Arch ワークベンチと![](/images/Workbench_BIM.svg) [BIM ワークベンチ](/BIM_Workbench/ja "BIM Workbench/ja")（![](/images/Std_AddonMgr.svg) [Addon Manager](/Std_AddonMgr "Std AddonMgr")から導入可能）に分担されるように実装していっています。前者が基本的な建築ツールを担い、後者はより直接的な BIM の作業工程を担っており、直感的かつ使いやすいよう Arch のツール群の上に新たなインターフェースを追加して成り立っているものです。詳しくは[FreeCAD BIM migration guide](https://yorik.uncreated.net/blog/2020-010-freecad-bim-guide)をご覧ください。

Draft、Arch 及び BIM の各ワークベンチの開発者達はより大きな[OSArch コミュニティー](https://osarch.org)とも連携しており、共に建築土木の設計をフリーソフトのみを用いて改良するという究極の目標に向かって努力しています。
...
- [IFC](/index.php?title=Arch_IFC/jp&action=edit&redlink=1 "Arch IFC/jp (page does not exist)") : Industry Foundation Classes 形式（インポートのみ）
- [DAE](/index.php?title=Arch_DAE/jp&action=edit&redlink=1 "Arch DAE/jp (page does not exist)") : Collada メッシュ形式
- [OBJ](/index.php?title=Arch_OBJ/jp&action=edit&redlink=1 "Arch OBJ/jp (page does not exist)") : Obj メッシュ形式（エクスポートのみ）
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Arch_Workbench/ja&oldid=1433531>"
