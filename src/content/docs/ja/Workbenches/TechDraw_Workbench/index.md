...
![](/images/Workbench_TechDraw.svg) テックドロー（TechDraw、技術製図）ワークベンチは、3D モデルを基に基本的な技術図面を製図するためのものです。[パート](/Part_Workbench/ja "Part Workbench/ja")、[パートデザイン](/PartDesign_Workbench/ja "PartDesign Workbench/ja")、[BIM](/BIM_Workbench/ja "BIM Workbench/ja")といったワークベンチで作成したモデルや、他のアプリケーションからインポートしたモデルを基に製図ができます。図面はページとして管理され、各ページには Part::Features、PartDesign::Bodies、App::Part グループ、その他ドキュメント上のオブジェクトのグループといった描画可能なオブジェクトのビューを複数個配置できます。作成した図面は、ドキュメンテーション、製造指示、契約、許認可申請などに使えます。
...
Retrieved from "<http://wiki.freecad.org/index.php?title=TechDraw_Workbench/ja&oldid=1544563>"
