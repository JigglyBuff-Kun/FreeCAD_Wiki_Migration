...
| - |
| See also |
| [부품설계 작업대에서 모따기](/PartDesign_Chamfer/ko "PartDesign Chamfer/ko") |
|  |

## 설명

![](/images/PartDesign_Fillet.svg) **부품설계 작업대에서 모깎기** 도구는 개체의 선택된 모서리를 둥글게 깎아냅니다. [트리 뷰](/Tree_view/ko "Tree view/ko")에 해당 표현이 있는 문서에 **모깎기** 개체를 추가합니다.
...
* 부품설계 작업대에서의 모깎기를 [부품 작업대에서의 모깎기](/index.php?title=Part_Fillet/ko&action=edit&redlink=1 "Part Fillet/ko (page does not exist)")와 혼동하지 마세요. 둘의 차이를 알지 못하는 경우 [부품 작업대의 모깎기](/index.php?title=Part_Fillet/ko&action=edit&redlink=1 "Part Fillet/ko (page does not exist)")를 부품설계의 몸통에 사용하면 안 됩니다. [부품과 부품설계](/Part_and_PartDesign "Part and PartDesign")를 참조하세요.
...
* 데이터**Support Transform** (`Bool`): If `true` the filleted shape of the additive/subtractive parent feature will be used when the fillet object is included in a [pattern](/PartDesign_Workbench#Transformation_tools "PartDesign Workbench"), else only the shape of the fillet itself will be used. The default is `false`.
...
* 데이터**Radius** (`QuantityConstraint`): The fillet radius. The default is `1 mm`.
* 데이터**Use All Edges** (`Bool`): If `true` all edges of the feature are filleted, and the edges specified by 데이터**Base** are ignored. The default is `false`.
...
고체 몸통에서 작동하는 모깎기, 모따기 및 기타 도형특징은 FreeCAD가 사용하는 기본 [OpenCASCADE](/OpenCASCADE "OpenCASCADE") 기술(OCCT) 커널에 따라 달라집니다. OCCT 커널은 두 면이 만나는 동시에 동시에 날카로운 모서리를 처리하는 데 어려움을 겪는 경우가 있습니다. 이 경우 FreeCAD는 설명 없이 충돌할 수 있습니다.
...
Edge numbers are not completely stable, therefore it is advisable that you finish the main design work of your solid body before applying features like fillets and chamfers, otherwise edges could change names and filleted edges would likely become invalid. When the 데이터**Use All Edges** property is `true` there is some protection from this. Because in such cases all the edges of the base object are used and there is no dependence on individual edge names.

[위상 명명 문제](/Topological_naming_problem/ko "Topological naming problem/ko")에 대해 더 자세히 알아보세요.

Retrieved from "<http://wiki.freecad.org/index.php?title=PartDesign_Fillet/ko&oldid=1515562>"