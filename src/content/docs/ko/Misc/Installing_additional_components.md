...
여러분의 운영체제 ([윈도우](/Installing_on_Windows "Installing on Windows"), [리눅스](/Installing_on_Linux "Installing on Linux") 또는 [맥킨토시](/Installing_on_Mac "Installing on Mac")) 에 프리캐드를 설치한 뒤에, 아래에 설명되어 있는 부속 기능들 중에 필요한 것이 있을 겁니다.

# 도움말 파일

다음을 읽어 보세요 ☞ [도움말 파일 설치](/Installing_Helpfile "Installing Helpfile").

## 외부 작업대

FreeCAD와 함께 번들로 제공되는 기본 [작업대](/Workbenches/ko "Workbenches/ko") 외에도 커뮤니티 구성원이 만든 유용한 [외부 작업대](/External_workbenches/ko "External workbenches/ko") 들이 많이 있습니다.
...
이 섹션의 모든 소프트웨어는 [표준 가져오기](/Std_Import/ko "Std Import/ko") 또는 [표준 내보내기](/Std_Export/ko "Std Export/ko") 도구에서 사용됩니다.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Installing_additional_components/ko&oldid=1476433>"