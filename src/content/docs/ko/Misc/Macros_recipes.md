...
이 페이지에는 FreeCAD 설치에 기능을 추가할 수 있는 [매크로](/Macros "Macros")가 나열되어 있습니다.

매크로를 작성하고 이 페이지의 매크로 분류 중 하나에 추가하고 싶다면, [매크로 문서](/Macro_documentation "Macro documentation")로 이동하여 매크로를 올바르게 문서화하는 방법에 대해 자세히 알아보세요.
...
- ![](/images/Macro_CenterOfMass.png) [질량중심 매크로](/Macro_CenterOfMass/ko "Macro CenterOfMass/ko"): 선택한 밀도로 선택된 여러 개체의 총 질량과 질량 중심을 보여 줍니다.
...
- ![](/images/FCInfo.png) [FCInfo 매크로](/Macro_FCInfo/ko "Macro FCInfo/ko"): 선택한 형상에 대한 일련의 정보를 제공하며 길이, 경사(도, 라디안, 등급) 형상, 표면, 부피 및 다양한 국제 단위와 앵글로색슨 단위로 선택한 밀도로 형태의 무게를 변환하여 표시할 수 있습니다.
...
- ![](/images/Easy-alias-icon.png) [EasyAlias 매크로](/Macro_EasyAlias/ko "Macro EasyAlias/ko"): FreeCAD 스프레드시트 작업대에서 빠르게 별칭을 만듭니다. 이는 오른쪽 다음 열의 인접 셀에 대한 별칭을 생성하기 위해 한 열의 레이블을 사용합니다, 예를 들어, 열 A의 라벨은 열 B의 셀에 대한 별칭이 됩니다.
...
자세한 설명은 [매크로 설치 방법](/How_to_install_macros/ko "How to install macros/ko")을 참조하고, 도구 모음에 매크로를 추가하여 쉽게 액세스할 수 있도록 하려면 [도구 모음 사용자 지정](/index.php?title=Customize_Toolbars/ko&action=edit&redlink=1 "Customize Toolbars/ko (page does not exist)")을 참조하세요.

많은 매크로를 설치하는 것은 새로운 작업대를 설치하는 것과 같습니다; 이것은 [추가 작업대를 설치하는 방법](/How_to_install_additional_workbenches/ko "How to install additional workbenches/ko")을 참조하세요.

### 자동 설치

**도구 → Addon manager**에서 [애드온 관리자](/Std_AddonMgr/ko "Std AddonMgr/ko")를 사용하여 [FreeCAD-macros](https://github.com/FreeCAD/FreeCAD-macros) 저장소에 포함된 매크로를 설치하세요. [introduced in 0.17](/Release_notes_0.17 "Release notes 0.17")

### 수동 설치

[애드온 관리자](/Std_AddonMgr/ko "Std AddonMgr/ko")를 사용하지 않으면 매크로를 수동으로 설치할 수 있습니다.

- 해당 매크로 페이지에서 [파이썬](/Python/ko "Python/ko") 코드를 복사합니다.
- 매크로 메뉴 **매크로 → 매크로**를 열고 만들기를 누른 다음 이름을 지정합니다.
- 복사한 파이썬 코드를 붙여넣습니다.
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Macros_recipes/ko&oldid=1534321>"
