...
쉬운 스크립팅 방법의 첫 번째 장애물은 메뉴 항목이나 도구 모음 영역의 아이콘을 통해 FreeCAD 내부 Python 편집기에 직접 액세스할 수 있는 방법이 없다는 것입니다. 하지만 FreeCAD가 내부 Python 편집기에서 `.py` 확장자를 가진 파일을 연다는 것을 알고 있으면 가장 간단한 방법은 즐겨 사용하는 텍스트 편집기에서 생성한 다음 일반적인 명령인 **File → Open)을 사용하여 여는 것입니다.** .
...
You will see some magic things, a new document is open named "Wiki\_example" and you will see in the 3d view a [Cube](/Part_Box "Part Box"), like the one in the image below.
...
Not that amazing? Yes, but we have to start somewhere, we can do the same thing with a [Cylinder](/Part_Cylinder "Part Cylinder"), add these lines of code after the `my_box()` function and before the line: `# objects definition`.
...
Placement Concept is relatively complex, see [Aeroplane Tutorial](/Aeroplane "Aeroplane") for a more deep explanation.
...
But over other consideration, one thing is crucial, geometry **reference point**, in other words the point from which the object is modeled by FreeCAD, as described in this table, copied from [Placement](/Placement "Placement"):
...
|  |  |  |
| --- | --- | --- |
| Airplane example | Airplane rotated | Placement property |
...
Retrieved from "<http://wiki.freecad.org/index.php?title=Scripts/ko&oldid=1461211>"